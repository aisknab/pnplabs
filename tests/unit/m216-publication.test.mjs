...
const milestoneId = 'concrete-cook-levin-builder-complete-schedule-iteration';
const theoremName = 'PNP.Concrete.CookLevin.BuilderCompleteScheduleIteration.cook_levin_builder_complete_schedule_iteration_checked_complete';
const theoremHash = '6b6baad967bde7dafa7de704b0d87df6f5f75ce7a6d1d32b778f8c0c92f7262a';
const moduleName = 'PNP.Concrete.CookLevinBuilderCompleteScheduleIteration';

test('M216 public payloads preserve the complete semantic schedule-iteration boundary', () => {
  const milestone = status.formalPublicationMilestones.find((row) => row.id === milestoneId);
  assert.ok(milestone);
  assert.equal(milestone.title, 'Complete semantic Cook-Levin schedule iteration');
  ...
  assert.match(milestone.scope, /structural recursion consumes every verifier-derived post-header schedule opportunity/);
  assert.match(milestone.scope, /complete run equals encodeCNFTokens of the generated formula/);
  assert.match(milestone.scope, /All 12 public declarations/);
  assert.match(milestone.nonClaim, /not one literal raw-machine loop/);
  assert.match(milestone.nonClaim, /does not prove a physical tape-to-tape handoff between successive stages/);
...
test('M216 status and release mirrors retain every load-bearing field', () => {
  const pairs = [
    ['Formalized', true],
    ['AxiomAuditPassed', true],
    ['AuditedDeclarationCount', 12],
  ...
  for (const [suffix, expected] of pairs) {
    assert.equal(status[`leanConcreteCookLevinBuilderCompleteScheduleIteration${suffix}`], expected);
    assert.equal(release.earnedBoundary[`cookLevinBuilderCompleteScheduleIteration${suffix}`], expected);
  }
  assert.equal(release.earnedBoundary.cookLevinBuilderCompleteScheduleIterationCheckedCompleteTheorem, theoremName);
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderCompleteScheduleIterationTheoremKernelTypeSha256, { [theoremName]: theoremHash });
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderCompleteScheduleIterationAxiomClosure, ['Quot.sound', 'propext']);
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderCompleteScheduleIterationProjectAxiomClosure, []);
});

test('M216 progress snapshot remains separate and conservative', () => {
  ...
  assert.ok(history);
  assert.deepEqual(history.formalArtefactCoverage, { earnedRows: 192, totalRows: 194 });
