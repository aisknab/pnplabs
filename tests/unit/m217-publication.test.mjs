...
test('M217 active surfaces publish the new boundary without erasing M216 history', () => {
  for (const file of ['README.md', 'architecture.html', 'faq.html', 'index.html', 'paper.html', 'status.html']) {
    const text = readFileSync(file, 'utf8');
    assert.match(text, /193 of 195/);
    assert.match(text, /35%/);
    assert.match(text, /fixed collision-free 64-rule machine/i);
    assert.doesNotMatch(text, /M217[^\n]*close(?:s|d) (?:a )?(?:fixed checkpoint|global gate)/i);
  }
  ...
  assert.equal(release.artifacts.report.pageCount, 156);
});
