...
test('current status binds the compiled inventory and fails the concrete gate closed', async () => {
  ...
  const pccMinCheckedPacketBN6BCELDerivedFamily = status.formalPublicationMilestones
    .find((row) => row.id === 'pccmin-checked-packet-bn6-bcel-derived-family');
  assert.ok(pccMinCheckedPacketBN6BCELDerivedFamily);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.earned, true);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.allPresent, true);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.allAssumptionFree, false);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.axiomClosureUsesOnlyLeanStandardAllowlist, true);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.allKernelTypesMatch, true);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.sourceClosureFingerprintMatches, true);
  assert.deepEqual(pccMinCheckedPacketBN6BCELDerivedFamily.requiredTheorems, [
    'PNP.DirectWire.pccmin_checked_packet_bn6_bcel_derived_family_route_or_zeroslack_checked_complete',
  ]);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.theoremRows.length, 1);
  assert.equal(pccMinCheckedPacketBN6BCELDerivedFamily.theoremRows[0].actualKernelTypeSha256,
    '2d50a967736e342be58071bd6c39783abf0cc18d67ef398fdf22803e6d4bd76a');
  assert.deepEqual(pccMinCheckedPacketBN6BCELDerivedFamily.theoremRows[0].axioms,
    ['Quot.sound', 'propext']);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyFormalized, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyAxiomAuditPassed, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyAuditedDeclarationCount, 14);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyEndpointProjectAssumptionFree, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyConstructsFamilyFromBCEL, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyDerivesCarrier, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyDerivesCutValue, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyEliminatesDuplicateMismatchRoutes, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyRetainsActivationMismatchRoute, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyDerivesConditionalZeroSlack, true);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyUnconditionalZeroSlack, false);
  assert.equal(status.leanPCCMinCheckedPacketBN6BCELDerivedFamilyPolynomialRuntimeProved, false);
  assert.equal(release.earnedBoundary.pccMinCheckedPacketBN6BCELDerivedFamilyCheckedCompleteTheorem,
    'PNP.DirectWire.pccmin_checked_packet_bn6_bcel_derived_family_route_or_zeroslack_checked_complete');
  assert.deepEqual(release.earnedBoundary.pccMinCheckedPacketBN6BCELDerivedFamilyAxiomClosure,
    ['Quot.sound', 'propext']);
  assert.deepEqual(
    release.earnedBoundary.pccMinCheckedPacketBN6BCELDerivedFamilyProjectAxiomClosure,
    []
  );
  ...
  const builderPostHeaderRawDivider = status.formalPublicationMilestones
    .find((row) => row.id === "concrete-cook-levin-builder-post-header-raw-divider");
  assert.ok(builderPostHeaderRawDivider);
  assert.equal(builderPostHeaderRawDivider.earned, true);
  assert.equal(builderPostHeaderRawDivider.allPresent, true);
  assert.equal(builderPostHeaderRawDivider.allAssumptionFree, false);
  assert.equal(builderPostHeaderRawDivider.axiomClosureUsesOnlyLeanStandardAllowlist, true);
  assert.equal(builderPostHeaderRawDivider.allKernelTypesMatch, true);
  assert.equal(builderPostHeaderRawDivider.sourceClosureFingerprintMatches, true);
  assert.match(builderPostHeaderRawDivider.scope, /fixed 99-rule deterministic work machine/u);
  assert.match(builderPostHeaderRawDivider.scope, /explicit quadratic bound/u);
  assert.match(builderPostHeaderRawDivider.nonClaim, /standalone fixed unary divider/u);
  assert.match(builderPostHeaderRawDivider.nonClaim, /not spliced onto M209/u);
  assert.match(builderPostHeaderRawDivider.nonClaim, /does not emit or append/u);
  assert.deepEqual(builderPostHeaderRawDivider.requiredTheorems, ["PNP.Concrete.CookLevin.BuilderPostHeaderRawDivider.cook_levin_builder_post_header_raw_divider_checked_complete"]);
  assert.equal(builderPostHeaderRawDivider.theoremRows.length, 1);
  assert.equal(builderPostHeaderRawDivider.theoremRows[0].actualKernelTypeSha256, "49eafb1e2702755c9a011c7dfcb37897242ab96021b03f64ea75fa621160ea75");
  ...
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawDividerExactQuotientRemainderDecodeFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawDividerExternalUnaryEncodedSizeQuadraticBoundFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawDividerFailClosedBoundaryTimeoutFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawDividerRawBodyTokenEmissionFormalized, false);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerCheckedCompleteTheorem, "PNP.Concrete.CookLevin.BuilderPostHeaderRawDivider.cook_levin_builder_post_header_raw_divider_checked_complete");
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerTheoremKernelTypeSha256, {
    "PNP.Concrete.CookLevin.BuilderPostHeaderRawDivider.cook_levin_builder_post_header_raw_divider_checked_complete": "49eafb1e2702755c9a011c7dfcb37897242ab96021b03f64ea75fa621160ea75",
  });
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerAxiomClosure, ["Quot.sound", "propext"]);
  ...
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerExactQuotientRemainderDecodeFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerExternalUnaryEncodedSizeQuadraticBoundFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerFailClosedBoundaryTimeoutFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawDividerRawBodyTokenEmissionFormalized, false);
  assert.ok(index.earnedMilestones.includes(builderPostHeaderRawDivider.id));

  const builderPostHeaderRawLaunch = status.formalPublicationMilestones
    .find((row) => row.id === "concrete-cook-levin-builder-post-header-raw-launch");
  assert.ok(builderPostHeaderRawLaunch);
  assert.equal(builderPostHeaderRawLaunch.earned, true);
  assert.equal(builderPostHeaderRawLaunch.allPresent, true);
  assert.equal(builderPostHeaderRawLaunch.allAssumptionFree, false);
  assert.equal(builderPostHeaderRawLaunch.axiomClosureUsesOnlyLeanStandardAllowlist, true);
  assert.equal(builderPostHeaderRawLaunch.allKernelTypesMatch, true);
  assert.equal(builderPostHeaderRawLaunch.sourceClosureFingerprintMatches, true);
  assert.match(builderPostHeaderRawLaunch.scope, /executable Lean orchestration/u);
  assert.match(builderPostHeaderRawLaunch.scope, /summed compiled work/u);
  assert.match(builderPostHeaderRawLaunch.nonClaim, /not a literal raw tape-to-tape bridge/u);
  assert.match(builderPostHeaderRawLaunch.nonClaim, /does not emit or append/u);
  assert.deepEqual(builderPostHeaderRawLaunch.requiredTheorems, ["PNP.Concrete.CookLevin.BuilderPostHeaderRawLaunch.cook_levin_builder_post_header_raw_launch_checked_complete"]);
  assert.equal(builderPostHeaderRawLaunch.theoremRows.length, 1);
  assert.equal(builderPostHeaderRawLaunch.theoremRows[0].actualKernelTypeSha256, "0361f9b8c05cea70ce199c78a74e55473765ae3e49d39565a46696915558fda1");
  assert.deepEqual(builderPostHeaderRawLaunch.theoremRows[0].axioms, ["Quot.sound", "propext"]);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchAxiomAuditPassed, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchAuditedDeclarationCount, 24);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchExactHandoffFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchAllRoutesFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchSourceSizePolynomialBoundFormalized, true);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchLiteralTapeBridgeFormalized, false);
  assert.equal(status.leanConcreteCookLevinBuilderPostHeaderRawLaunchRawBodyTokenEmissionFormalized, false);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchCheckedCompleteTheorem, "PNP.Concrete.CookLevin.BuilderPostHeaderRawLaunch.cook_levin_builder_post_header_raw_launch_checked_complete");
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchTheoremKernelTypeSha256, {
    "PNP.Concrete.CookLevin.BuilderPostHeaderRawLaunch.cook_levin_builder_post_header_raw_launch_checked_complete": "0361f9b8c05cea70ce199c78a74e55473765ae3e49d39565a46696915558fda1",
  });
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchAxiomClosure, ["Quot.sound", "propext"]);
  assert.deepEqual(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchProjectAxiomClosure, []);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchAxiomAuditPassed, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchAuditedDeclarationCount, 24);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchExactHandoffFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchAllRoutesFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchSourceSizePolynomialBoundFormalized, true);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchLiteralTapeBridgeFormalized, false);
  assert.equal(release.earnedBoundary.cookLevinBuilderPostHeaderRawLaunchRawBodyTokenEmissionFormalized, false);
  assert.ok(index.earnedMilestones.includes(builderPostHeaderRawLaunch.id));
