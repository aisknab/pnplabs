...
test("current updates cover every milestone earned after the exact 39-milestone baseline", async () => {
  ...
  assert.equal(model.earnedCount, 62);
  assert.equal(model.entries.length, 23);
  ...
  assert.deepEqual(model.entries.map((entry) => entry.milestone.requiredTheorems.length), [
    11, 8, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 39, 39, 39, 41, 92, 75, 40
  ]);
  assert.equal(model.progressEstimatePercent, 48);
  assert.deepEqual(data.entries.map((entry) => entry.progressEstimatePercent), [48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 30, null, null, null, null, null]);
...
test("HTML puts two plain-language paragraphs before one collapsed source-derived technical dropdown", async () => {
  ...
  assert.match(html, /About 48% of the known formalisation work/u);
  assert.match(html, /<progress[^>]+max="100"[^>]+value="48"/u);
...
test("Atom output has stable IDs, canonical timestamps, escaped text, and no duplicated technical prose", async () => {
  ...
  assert.match(feed, /Editorial progress estimate at publication: 48 percent/u);
  assert.match(feed, /data-progress-estimate-percent=&quot;48&quot;/u);
...
test("progress SVG is deterministic, accessible, themed, and free of active content", async () => {
  ...
  assert.match(svg, /Proof reconstruction progress estimate: 48 percent/u);
  assert.match(svg, /#6f193c/u);
  assert.match(svg, /#168b87/u);
  assert.match(svg, /48% ESTIMATED/u);
...
test("progress estimates are editorial and may decrease between tracked milestones", async () => {
  ...
  assert.equal(model.entries[0].progressEstimatePercent, 48);
...
test("updates are discoverable from every public HTML page and the locked-down static surface", async () => {
  ...
  assert.match(await readFile(path.join(repositoryRoot, "assets\/proof-progress.svg"), "utf8"), /48% ESTIMATED/u);
});
