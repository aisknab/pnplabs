...
test("current updates cover every milestone earned after the exact 39-milestone baseline", async () => {
  ...
  assert.equal(model.earnedCount, 74);
  assert.equal(model.entries.length, 35);
  ...
  assert.deepEqual(model.entries.map((entry) => entry.milestone.requiredTheorems.length), [
    13, 10, 14, 28, 18, 5, 22, 20, 11, 8, 2, 5, 11, 8, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 39, 39, 39, 41, 92, 75, 40
  ]);
  assert.equal(model.progressEstimatePercent, 60);
  assert.deepEqual(data.entries.map((entry) => entry.progressEstimatePercent), [60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 30, null, null, null, null, null]);
...
test("HTML puts two plain-language paragraphs before one collapsed source-derived technical dropdown", async () => {
  ...
  assert.match(html, /About 60% of the known formalisation work/u);
  assert.match(html, /<progress[^>]+max="100"[^>]+value="60"/u);
...
test("Atom output has stable IDs, canonical timestamps, escaped text, and no duplicated technical prose", async () => {
  ...
  assert.match(feed, /Editorial progress estimate at publication: 60 percent/u);
...
test("progress SVG is deterministic, accessible, themed, and free of active content", async () => {
  ...
  assert.match(svg, /Proof reconstruction progress estimate: 60 percent/u);
  assert.match(svg, /#6f193c/u);
  assert.match(svg, /#168b87/u);
  assert.match(svg, /60% ESTIMATED/u);
...
test("progress estimates are editorial and may decrease between tracked milestones", async () => {
  ...
  assert.equal(model.entries[0].progressEstimatePercent, 60);
...
test("updates are discoverable from every public HTML page and the locked-down static surface", async () => {
  ...
  assert.match(await readFile(path.join(repositoryRoot, "assets\/proof-progress.svg"), "utf8"), /60% ESTIMATED/u);
});
