...
test("current updates cover every milestone earned after the exact 39-milestone baseline", async () => {
  ...
  assert.equal(model.earnedCount, 94);
  assert.equal(model.entries.length, 55);
  assert.deepEqual(model.entries.map((entry) => entry.earnedOrdinal), [
    94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83, 82, 81, 80, 79, 78, 77, 76, 75, 74, 73, 72, 71, 70, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40
  ]);
  assert.deepEqual(model.entries.map((entry) => entry.milestone.requiredTheorems.length), [
    12, 36, 20, 28, 20, 19, 30, 27, 24, 23, 28, 26, 23, 22, 21, 14, 7, 4, 14, 12, 13, 10, 14, 28, 18, 5, 22, 20, 11, 8, 2, 5, 11, 8, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 39, 39, 39, 41, 92, 75, 40
  ]);
  assert.equal(model.progressEstimatePercent, 79);
  assert.deepEqual(data.entries.map((entry) => entry.progressEstimatePercent), [79, 78, 77, 76, 75, 74, 73, 72, 72, 71, 70, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50, 49, 48, 47, 46, 45, 44, 43, 42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 32, 30, null, null, null, null, null]);
...
test("HTML puts two plain-language paragraphs before one collapsed source-derived technical dropdown", async () => {
  ...
  assert.match(html, /Reviewed theorem pins:<\/strong> 12/u);
  assert.match(html, /About 79% of the known formalisation work/u);
  assert.match(html, /<progress[^>]+max="100"[^>]+value="79"/u);
...
test("Atom output has stable IDs, canonical timestamps, escaped text, and no duplicated technical prose", async () => {
  ...
  assert.match(feed, /Editorial progress estimate at publication: 79 percent/u);
...
test("progress SVG is deterministic, accessible, themed, and free of active content", async () => {
  ...
  assert.match(svg, /Proof reconstruction progress estimate: 79 percent/u);
  assert.match(svg, /#6f193c/u);
  assert.match(svg, /#168b87/u);
  assert.match(svg, /79% ESTIMATED/u);
...
test("progress estimates are editorial and may decrease between tracked milestones", async () => {
  ...
  assert.equal(model.entries[0].progressEstimatePercent, 79);
...
test("updates are discoverable from every public HTML page and the locked-down static surface", async () => {
  ...
  assert.match(await readFile(path.join(repositoryRoot, "assets\/proof-progress.svg"), "utf8"), /79% ESTIMATED/u);
});
