...
test('plain-language orientation is static and available before technical depth', async () => {
  ...
  for (const question of [
    ...
    'What does the 87% tracker mean?',
    'How can I follow new milestones?',
  ]) assert.ok(faq.includes(question), question);
  assert.match(faq, /87% of the known formal reconstruction workload is complete/u);
...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 104 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 87% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="87"[^>]+aria-label="Estimated proof reconstruction progress: 87 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 87 percent"/u);
  assert.match(home, /data-proof-progress="87"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 87%;/u);
  assert.match(minCss, /\.proof-tape-graphic\{--proof-progress:87%/u);
