...
test('plain-language orientation is static and available before technical depth', async () => {
  ...
  assert.match(paper, /The current 52-page report is generated from the compiled Lean inventory/u);
...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 55 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 38% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="38"[^>]+aria-label="Estimated proof reconstruction progress: 38 percent"/u);
  assert.match(await read('index.html'), /alt="Estimated proof reconstruction progress: 38 percent"/u);
