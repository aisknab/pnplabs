...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 66 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 49% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="49"[^>]+aria-label="Estimated proof reconstruction progress: 49 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 49 percent"/u);
  assert.match(home, /data-proof-progress="49"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 49%;/u);
