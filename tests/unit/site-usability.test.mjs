...
test('plain-language orientation is static and available before technical depth', async () => {
  ...
  assert.match(paper, /The current 81-page report is generated from the compiled Lean inventory/u);
...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 99 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 81% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="81"[^>]+aria-label="Estimated proof reconstruction progress: 81 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 81 percent"/u);
  assert.match(home, /data-proof-progress="81"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 81%;/u);
