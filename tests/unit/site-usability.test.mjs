...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 72% of the known formalisation work/u);
  ...
  assert.match(svg, /72% ESTIMATED/u);
  assert.match(updates, /<progress[^>]+value="72"[^>]+aria-label="Estimated proof reconstruction progress: 72 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 72 percent"/u);
  assert.match(home, /data-proof-progress="72"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 72%;/u);
