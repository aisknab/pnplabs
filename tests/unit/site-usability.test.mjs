...
test('plain-language orientation is static and available before technical depth', async () => {
  ...
  for (const question of [
    ...
    'What does the 89% tracker mean?',
    'How can I follow new milestones?',
  ]) assert.ok(faq.includes(question), question);
  assert.match(faq, /89% of the known formal reconstruction workload is complete/u);
  ...
  assert.match(paper, /The current 84-page report is generated from the compiled Lean inventory/u);
...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 106 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 89% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="89"[^>]+aria-label="Estimated proof reconstruction progress: 89 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 89 percent"/u);
  assert.match(home, /data-proof-progress="89"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 89%;/u);
  assert.match(minCss, /\.proof-tape-graphic\{--proof-progress:89%/u);
