...
test('technical disclosures announce their controls and remain usable without JavaScript', async () => {
  ...
  assert.match(status, /Show all 78 formal milestone records/u);
...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 61% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="61"[^>]+aria-label="Estimated proof reconstruction progress: 61 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 61 percent"/u);
  assert.match(home, /data-proof-progress="61"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 61%;/u);
