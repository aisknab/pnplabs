...
test('updates expose a provider-free feed and a clearly qualified progress estimate', async () => {
  ...
  assert.match(updates, /About 82% of the known formalisation work/u);
  ...
  assert.match(updates, /<progress[^>]+value="82"[^>]+aria-label="Estimated proof reconstruction progress: 82 percent"/u);
  ...
  assert.match(home, /role="img" aria-label="Estimated proof reconstruction progress: 82 percent"/u);
  assert.match(home, /data-proof-progress="82"/u);
  ...
  assert.match(css, /\.proof-tape-graphic \{\s*--proof-progress: 82%;/u);
