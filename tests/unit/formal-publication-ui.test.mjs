...
test('site validator accepts only the exact current inventory/status boundary', () => {
  assert.equal(createHash('sha256').update(inventoryBytes).digest('hex'), 'a273572123347ada7b362371a3f30fe17c8c0ac359e3d440d6fd4583eaffeedf');
  ...
  assert.equal(status.formalPublicationMilestones.filter((row) => row.earned).length, 43);
...
test('browser loader pins the raw status bytes before parsing', () => {
  assert.match(source, /const STATUS_SHA256 = '66385722de2685d469eba26619080dc8659e3c87dcdaf5e677da7cf08ff9943a'/);
...
test('static pages remain conservative and distinguish current from historical reports', () => {
  ...
  assert.match(statusPage, /9,758/);
  assert.match(statusPage, /Forty-three scoped milestones/);
  ...
  assert.match(reportPage, /forty-two-page report generated from the compiled Lean inventory/i);
