...
test('site validator accepts only the exact current inventory/status boundary', () => {
  assert.equal(createHash('sha256').update(inventoryBytes).digest('hex'), 'f3e8f9024eeaf12bc26854355b378fc14d0d9e777cab545539573f48b476ff0b');
  ...
  assert.equal(status.formalPublicationMilestones.filter((row) => row.earned).length, 47);
...
test('browser loader pins the raw status bytes before parsing', () => {
  assert.match(source, /const STATUS_SHA256 = 'a30ee68df44c5d1b02f243388d43ef20f310ee608c18d5c84c9c4fb4daf41f8e'/);
...
test('static pages remain conservative and distinguish current from historical reports', () => {
  ...
  assert.match(statusPage, /10,300/);
  assert.match(statusPage, /Forty-seven scoped milestones/);
  ...
  assert.match(reportPage, /current 46-page report is generated from the compiled Lean inventory/i);
