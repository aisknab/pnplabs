...
test('one-command upload payload exposes the simple verifier flow', async () => {
  const payload = await readJson('public/pnp-one-command-upload.json');

  assert.equal(payload.kind, 'PNPOneCommandVerifierUpload0');
  assert.equal(payload.status, 'one-command-verifier-upload-ready');
...
test('public pages and issue template surface the one-command upload flow', async () => {
  const joined = [
