...
test('M221 active surfaces publish the Finish request without overclaiming', () => {
  ...
  for (const file of [
    ...
    assert.match(text, /Finish request/i);
  ...
  assert.match(statusPage,
    /data-milestone-id="concrete-cook-levin-builder-physical-classifier-pipeline"/);
  assert.equal(release.artifacts.report.pageCount, 160);
});
