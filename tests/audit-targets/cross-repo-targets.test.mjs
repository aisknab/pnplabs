...
function makeProject(t) {
  ...
  assert.equal(statusPayload.formalPublicationMilestones.length, 97, "synthetic status must match the published milestone count");
  assert.equal(statusPayload.formalPublicationMilestones.filter((row) => row.earned === true).length, 94, "synthetic status must match the published earned-milestone count");
  ...
  const inventoryPayload = {
    ...
    declarationCount: 25571,
    theoremCount: 13587,
    assumptionFreeTheoremCount: 7043,
    excludedPrivateDeclarationCount: 14779,
    sourceClosureModuleCount: 233,
    axiomCount: 4,
  ...
  assert.equal(inventoryPayload.milestoneCandidates.length, 2432, "synthetic inventory must match the published reviewed-candidate count");
