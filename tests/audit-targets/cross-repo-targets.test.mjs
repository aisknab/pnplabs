...
function makeProject(t) {
  ...
  assert.equal(statusPayload.formalPublicationMilestones.length, 95, "synthetic status must match the published milestone count");
  assert.equal(statusPayload.formalPublicationMilestones.filter((row) => row.earned === true).length, 92, "synthetic status must match the published earned-milestone count");
  ...
  const inventoryPayload = {
    ...
    declarationCount: 25099,
    theoremCount: 13423,
    assumptionFreeTheoremCount: 7025,
    excludedPrivateDeclarationCount: 14705,
    sourceClosureModuleCount: 231,
    axiomCount: 4,
  ...
  assert.equal(inventoryPayload.milestoneCandidates.length, 2397, "synthetic inventory must match the published reviewed-candidate count");
