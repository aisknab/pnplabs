...
function makeProject(t) {
  ...
  assert.equal(statusPayload.formalPublicationMilestones.length, 98, "synthetic status must match the published milestone count");
  assert.equal(statusPayload.formalPublicationMilestones.filter((row) => row.earned === true).length, 95, "synthetic status must match the published earned-milestone count");
  ...
  const inventoryPayload = {
    ...
    declarationCount: 25863,
    theoremCount: 13665,
    assumptionFreeTheoremCount: 7079,
    excludedPrivateDeclarationCount: 14904,
    sourceClosureModuleCount: 235,
    axiomCount: 4,
  ...
  assert.equal(inventoryPayload.milestoneCandidates.length, 2449, "synthetic inventory must match the published reviewed-candidate count");
