...
function makeProject(t) {
  ...
  assert.equal(statusPayload.formalPublicationMilestones.filter((row) => row.earned === true).length, 98, "synthetic status must match the published earned-milestone count");
  assert.equal(statusPayload.formalPublicationMilestones.filter((row) => row.status === "not-formalized").length, 3, "synthetic status must retain every fail-closed milestone");
  const status = json(statusPayload);
  const inventoryPayload = {
    kind: "PNPLeanTheoremInventory0",
    coordinate: publishedInventory.coordinate,
    declarationCount: 26539,
    theoremCount: 13883,
    assumptionFreeTheoremCount: 7159,
    excludedPrivateDeclarationCount: 14935,
    sourceClosureModuleCount: 239,
    axiomCount: 4,
  ...
  assert.equal(inventoryPayload.milestoneCandidates.length, 2486, "synthetic inventory must match the published reviewed-candidate count");
