...
const REVIEWED_CORE_COMMIT = "a23dd28052c4bbeee281ed0ce691ef7350ef2215";
const REVIEWED_CORE_TREE = "bceb595ee6783770a47270ada6c492a627b4f26a";
const REVIEWED_PROOF_COMMIT = "ae61c50516a57065e650499c008f637d210d7bdb";
...
function validateReleaseManifest(manifest, expectedIdentity, failures) {
  ...
  if (manifest.artifacts?.report?.pageCount !== 20) failures.push("formal-publication report must have exactly twenty pages");
  ...
  if (!bodyStartHashes || Object.keys(bodyStartHashes).length !== 40 || !Object.entries(BUILDER_BODY_START_PREFIX_THEOREMS).every(([name, evidence]) => bodyStartHashes[name] === evidence.hash)) failures.push("formal-publication Cook-Levin builder body-start-prefix fingerprint mismatch");
...
function validateCurrentPayloads(contents, failures) {
  ...
  if (inventoryBuffer) {
    ...
    if (inventory.declarationCount !== 7500 || inventory.theoremCount !== 3536 || inventory.assumptionFreeTheoremCount !== 2795 || inventory.excludedPrivateDeclarationCount !== 2124 || inventory.sourceClosureModuleCount !== 68 || inventory.axiomCount !== 4) failures.push("public inventory count boundary mismatch");
    ...
    if (inventory.milestoneCandidates?.length !== 454) failures.push("public inventory reviewed theorem-candidate count mismatch");
  }
}
