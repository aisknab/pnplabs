...
const REVIEWED_CORE_COMMIT = "5fe3ffec94ad37b0f62321bde9764518d3a4e548";
const REVIEWED_CORE_TREE = "ecf8bd94a5f8f93f685c7d2b56be910c44bd30dc";
...
function validateReleaseManifest(manifest, expectedIdentity, failures) {
  ...
  if (manifest.artifacts?.report?.pageCount !== 32) failures.push("formal-publication report must have exactly thirty-two pages");
...
function validateCurrentPayloads(contents, failures) {
  ...
  if (inventoryBuffer) {
    ...
    if (inventory.declarationCount !== 8599 || inventory.theoremCount !== 4359 || inventory.assumptionFreeTheoremCount !== 3031 || inventory.excludedPrivateDeclarationCount !== 2833 || inventory.sourceClosureModuleCount !== 77 || inventory.axiomCount !== 4) failures.push("public inventory count boundary mismatch");
    ...
    if (inventory.milestoneCandidates?.length !== 878) failures.push("public inventory reviewed theorem-candidate count mismatch");
  }
}
