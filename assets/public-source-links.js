...
if(footer&&!footer.querySelector('a[href="https://github.com/aisknab/pnp"]')){
...
if(page==='index.html'){
  ...
  if(actions&&!actions.querySelector(`a[href="${repo}"]`)) actions.insertBefore(link('Browse source/checker',repo,'btn secondary'),actions.children[1]);
  ...
  if(request){request.textContent='Open public source';request.href=repo;}
}
...
if(page==='review.html'){
  ...
  if(lede)lede.textContent='The source/checker repository and pinned release tags are public. Use the links below for direct inspection, and use the contact channels for technical questions, institutional coordination, current-status questions, and responsible security disclosure.';
  const access=byText('h3','Review and source repository access');
  if(access){access.textContent='Review questions and reproduction support';const p=access.nextElementSibling;if(p)p.textContent='For mathematical findings, checker questions, reproduction help, or institutional review coordination. Source access is public and does not require email approval.';}
  const institutional=byText('h3','Institutional review');
  if(institutional?.nextElementSibling)institutional.nextElementSibling.textContent='Current status, internal acceptance record, external verification status, public source and release availability, and public citation language.';
  const contact=document.querySelector('#contact');
  if(contact&&!document.querySelector('#public-source')){
    const section=document.createElement('section');
    section.id='public-source';section.className='section compact';
...
if(page==='faq.html'){
  ...
  if(list&&!document.querySelector('#public-source-faq')){
    ...
    const contact=[...list.querySelectorAll('details')].find(d=>d.textContent.includes('source repository access requests'));
    list.insertBefore(details,contact||null);
    if(contact){contact.querySelector('summary').textContent='Where do technical findings and security disclosures go?';contact.querySelector('p').innerHTML='Source access is public and requires no approval. Technical findings, reproduction questions, and review coordination go to <a href="mailto:review@pnplabs.com.au">review@pnplabs.com.au</a>. Security issues go to <a href="mailto:security@pnplabs.com.au">security@pnplabs.com.au</a>.﻿';}
  }
}
