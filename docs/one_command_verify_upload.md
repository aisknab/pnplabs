...
These commands expose current status, rebuild the compiled inventory and generated report, build the pinned Lean `PNP` library root, and audit supporting declarations. They reproduce the earned concrete `CNFSAT ∈ NP` theorem, literal all-input and sequential compilers, recursive raw-machine refinement, the typed locked-NAND semantic threshold, exact strict codecs, concrete parser/emitter machines and polynomial reductions, the finite residual sequence through typed PkgC restoration, same-key cancellation, exact ambient-BN4-ledger embedding and residual reduction, the finite BN6 packet bridge, the complete finite Packet selector pipeline through conditional gain coverage, the generic finite Packet charge-surplus kernel, and the checked unit-charge blueprint realizer. The locked-NAND theorem `PNP.Main.locked_nand_threshold : ReducesTo CNFSAT EncodedLockedNANDThreshold` remains a polynomial many-one reduction, not a polynomial-time target decider or an NP-hardness or NP-completeness result. The newest result derives canonical unit-charge gate-occurrence ledgers and constructively validates exact occurrence multiplicity, a nonempty unmatched current-gate remainder, and semantic equivalence for every supplied blueprint in one finite Packet family. Acceptance instantiates the charge-surplus kernel and yields a genuine strict equivalent gain and strict residual descent without accepting a gate inequality; the exhaustive scan visits every original blueprint atom behind every canonical handle. The grouped family, candidates, blueprints, pairings, and unmatched lists remain proof-bearing inputs and are not derived from terminal data. Family-local validator silence is not a typed blocker, global no-gain result, semantic minimum, or unconditional `ZeroSlack`; the result does not prove selector faithfulness or compatibility, establish encoded-size or runtime bounds, close HB/rank routing, complete global PkgC, unconditional `ZeroSlack`, PCCMin, or the root theorem. `PNP.Main.p_eq_np` is absent; four project-specific axioms and five blockers remain.
