...
These commands expose current status, rebuild the compiled inventory and generated report, build the pinned Lean `PNP` library root, and audit supporting declarations. They reproduce the earned concrete `CNFSAT ∈ NP` theorem, literal all-input and sequential compilers, recursive raw-machine refinement, the typed locked-NAND semantic threshold, exact strict codecs, concrete parser/emitter machines and polynomial reductions, the finite residual sequence through typed PkgC restoration, same-key cancellation, exact ambient-BN4-ledger embedding and residual reduction, the finite BN6 packet bridge, payload-backed Packet selector seeds, exact finite grouped-footprint universe membership, and canonical finite selector handles. The locked-NAND theorem `PNP.Main.locked_nand_threshold : ReducesTo CNFSAT EncodedLockedNANDThreshold` remains a polynomial many-one reduction, not a polynomial-time target decider or an NP-hardness or NP-completeness result. The newest result gives every payload selector in an arbitrary finite explicit grouped BN6 family one unique input-relative handle, proves decoding injective, and retains payload, carrier-containment, and minimum-size evidence. The grouped family remains proof-bearing input. These list-position handles are not the manuscript's bit encoding or a polynomially enumerable selector universe and do not prove manuscript-level selector faithfulness or compatibility, construct a selector realizer or route, derive or group the family from terminal data, establish polynomial encoding length or runtime, complete PkgC, ZeroSlack or PCCMin, or the root theorem. `PNP.Main.p_eq_np` is absent; four project-specific axioms and five blockers remain.
