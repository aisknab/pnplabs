...
core commit `a09062d02e0c06704f57efe1bdb99024fbcfc618`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the eighteen-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `a09062d02e0c06704f57efe1bdb99024fbcfc618` | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results, the direct formula cursor, and the literal 184-rule first-token prefix |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 365 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Twenty-three earned scoped milestones, three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current eighteen-page non-claiming report; mirrored by this site |
