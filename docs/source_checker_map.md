...
core commit `fdd4e10c36155f079edc72f44fd59f0e8767dad6`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the sixty-three-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `fdd4e10c36155f079edc72f44fd59f0e8767dad6` (tree `34313d90e30ff0940ce4624d6d590b0b65df7b7d`) | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results and its bounded builder prefix, plus the locked-NAND typed threshold, strict version-zero encoding round trips, complete-candidate bytes, and fail-closed pure semantic construction |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 1,988 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Sixty-six earned scoped milestones—including the arbitrary-circuit locked-NAND threshold and its encoded semantic boundary—three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current sixty-three-page non-claiming report; mirrored by this site |
...
- The key mathematical review targets remain locked-NAND threshold correctness, exact codec round trips and normalization semantics, the distinction between a pure construction and bounded parser/emitter machines, the missing concrete polynomial reduction, residual-band exact minimization, no-hidden-minimization discipline, proof-ref soundness, and public theorem boundary discipline.
