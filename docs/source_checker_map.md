...
core commit `9db71188ff4dafbefc09f5f0120fdf37c229bb4d`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the thirty-four-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `9db71188ff4dafbefc09f5f0120fdf37c229bb4d` | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results, both complete fixed clauses, clause-two padding traversal, and the complete negative literal on variable zero in clause three |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 960 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Thirty-six earned scoped milestones, three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current thirty-four-page non-claiming report; mirrored by this site |
