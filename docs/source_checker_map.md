...
core commit `97ca87c3588deab60b8126c204d4d63b10dc2c85`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 113-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | 137 of 139 scoped milestone rows are earned. The newest result computes canonical Packet frontier and obligation acceptance from corresponding exact fields of supplied typed source and selector terminal BN5 coordinates while retaining grouped colour, positive charge, the internal source route, authoritative rank, and exact RankWF descent computation. A frontier first route is exactly frontier inequality; an obligation first route is prior frontier equality with obligation inequality; the classifier also cannot return colour, charge, exactRoute, or rank; a final descent route carries actual nondecrease; and the positive Packet/HB endpoint carries exact failure evidence. The BN5 coordinates, finite rank map, before/after ranks, activation, direction, budget, activity bits, dependency rows, blocker semantics, semantic dependency completeness, exhaustive claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, and unmatched lists remain supplied inputs or open obligations. The coordinates are not constructed from terminal data, and the result does not prove the manuscript's complete BN5 or Packet adequacy bridge. It does not prove the three remaining routes' external manuscript semantics or decreasing complete global coverage, guarantee a decreasing transition, construct the no-lower ledger, derive positive slack or those inputs from terminal data, establish full external selector compatibility, close unconditional HB negative closure, provide encoded-size or runtime bounds, complete global PkgC, prove unconditional ZeroSlack, or establish polynomial PCCMin. Two global rows, four project axioms, and five blockers remain. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 113-page non-claiming report; mirrored by this site |
