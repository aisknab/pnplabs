...
core commit `7dc26600f55762bf7a65b3fc6512a76aba23d9d0`. Its generated status payload is current
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 163 of 165 scoped milestone rows. The newest result gives the report-facing residual-band endpoint concrete fail-closed encoded exact-minimum semantics and replaces its caller-supplied compatibility edge with a compiled identity reduction. It removes one project axiom but does not supply polynomial residual minimisation, residual-band promise bounds, a target decider, SAT hardness transport, unconditional ZeroSlack, exact polynomial PCCMin, or the root theorem. Two global rows, two project axioms, and five blockers remain. The row ratio is not proof completion. |
| Fixed-weight progress | `public/pnp-proof-progress.json` | Exact mirror of the canonical 100-point checkpoint ledger, including the separate risk-weighted estimate, uncertainty, evidence coverage, five global gates, two project axioms, root state, and publication gate. Its validator resolves checkpoint evidence against status and compiled declarations. |
