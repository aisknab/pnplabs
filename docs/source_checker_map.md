...
core commit `ff003dc1e86367a072e783d7fb8e32d374feed05`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the ninety-nine-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `ff003dc1e86367a072e783d7fb8e32d374feed05` (tree `31387d243fbcfa8fa62f683377c238861b0830f9`) | Exact source tree reviewed and merged through computed BN2 square legitimacy, the canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through conditional gain coverage, the generic finite Packet charge-surplus strict-gain kernel, the checked unit-charge blueprint realizer, the checked finite Packet typed-realizer contract, exact-rank acyclicity and total-table coverage for a supplied finite HN and budget dependency system, and checked active-dependency closure for every supplied HN/BUD activity table |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 2,719 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | One hundred and twenty-four earned scoped milestones through a checked active-dependency closure that combines supplied active-to-active rows with strict exact-rank descent, proves every supplied HN/BUD activity bit false, and removes those typed-bot branches; activity bits, dependency rows, rank mapping, blocker activity, blocker semantics, semantic dependency completeness, gain exclusion, lower-seed closure, rank assignment, faithfulness predicate, claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, unmatched lists, and gain-coverage certificate remain supplied inputs or open obligations; the result does not establish selector compatibility, rank-complete selector silence, the full HB negative closure, encoded-size or runtime bounds, complete global PkgC or global routing, or polynomial PCCMin; two unearned global milestones, four project axioms, and five blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current ninety-nine-page non-claiming report; mirrored by this site |
