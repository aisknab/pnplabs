...
core commit `71f0e479cfd23126abfe19bda4763f514cf4d3e5`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the thirty-three-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `71f0e479cfd23126abfe19bda4763f514cf4d3e5` | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results, both complete fixed clauses, clause-two padding traversal, and the fixed separator beginning clause three |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 912 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Thirty-five earned scoped milestones, three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current thirty-three-page non-claiming report; mirrored by this site |
