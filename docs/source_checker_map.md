...
core commit `79d36936abf796a3f306cede1b762aabc6907cd2`. Its generated status payload is current
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 204 of 206 scoped milestone rows. For every concrete verifier problem and every coordinate in its complete post-header schedule, M228 runs M226's terminal-joined classifier without a staged request cell. A fixed 20-rule relay reads the physical body-or-Finish terminal, crosses the exact blank-free classifier prefix, writes a collision-free body-pending marker for every body route, and writes the canonical Finish request only for the unique Finish route. A fixed 65-rule conditional reflected dispatcher rejects the body-pending marker and sends Finish to the exact next canonical emitted prefix. The collision-free 823-rule composition has exact work, six-for-one compiled, one-step-short and source-input-size polynomial evidence. All 91 public declarations are axiom-audited: 50 have empty closure, 18 use only propext, and 23 use only propext and Quot.sound, with no project axiom or Classical.choice. The unique Finish request is derived and dispatched, while every body route stops at an explicit non-request pending marker. Body-token and padding request synthesis, successive physical schedule configurations, one repeated raw-machine builder loop, builder `FunctionProgram.RawRefinement`, and the packaged `PolynomialReduction` remain open. No fixed checkpoint or global gate closes, so the risk-weighted estimate remains 35% while formal artefact coverage becomes 204 of 206. Two global rows and all five global gates remain open; no project-specific axioms remain. The row ratio is not proof completion. |
