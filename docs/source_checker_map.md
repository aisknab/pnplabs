...
core commit `b69c7419158154187c2adc6991ebe31aef6538f9`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 104-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | 130 of 132 scoped milestone rows are earned. The newest result proves every returned canonical Packet route equivalent to the unique earliest failed supplied field, with every preceding field accepted; rejection is equivalent to exact failure existence, and the positive Packet/HB endpoint carries both route and proof. Payload fields, activity bits, dependency rows, rank mapping and assignment, blocker semantics, semantic dependency completeness, exhaustive claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, and unmatched lists remain supplied inputs or open obligations. The result proves semantics only for those existing Boolean fields; it does not prove their external manuscript semantics or decreasing complete global coverage, positive slack, terminal derivation of those inputs, full external selector compatibility, unconditional HB negative closure, encoded-size or runtime bounds, complete global PkgC, unconditional ZeroSlack, or polynomial PCCMin. Two global rows, four project axioms, and five blockers remain. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 104-page non-claiming report; mirrored by this site |
