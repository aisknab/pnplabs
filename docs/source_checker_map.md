...
core commit `0dbad6e14d82bf1f6f1d7b315dc896bdceb325a6`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 134-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 167 of 169 scoped milestone rows. The newest result enforces HResolve before BudgetResolve, scans every selector in every canonical finite rank row, returns gains immediately, and requires complete typed silence before the supplied ZeroSlack closure. The normalizer, resolvers, rows, realizer, blocker semantics, ZeroSlack proof, and exhaustive reference fixture remain supplied, so the result does not construct executable PCCMin, derive terminal objects, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 134-page non-claiming report; mirrored by this site |
