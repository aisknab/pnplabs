...
core commit `a23dd28052c4bbeee281ed0ce691ef7350ef2215`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the twenty-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `a23dd28052c4bbeee281ed0ce691ef7350ef2215` | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results, the direct formula cursor, and the literal `T^FormulaWidth F Sep` body-start builder prefix |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 454 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Twenty-five earned scoped milestones, three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current twenty-page non-claiming report; mirrored by this site |
