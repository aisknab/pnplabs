...
core commit `02a8fc9ea8fb7c338b9c6a484020fe6dfc754872`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 141-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `02a8fc9ea8fb7c338b9c6a484020fe6dfc754872` (tree `cbb00cdfdd0e002dfd0db55594dc5f0e997f0866`) | Exact source tree reviewed and merged through the concrete and terminal chain, the finite Packet/HB and sidecar layers, the report-facing compatibility links, typed PCCPack reflection, the proof-bearing PCCMin orchestration, the checked conditional ZeroSlack bridges, the BCEL-derived BN6 skeleton, canonical positive-cell grouping, exact raw cut-ledger conservation, and the sparse V53 constant-cut basis equivalent to every proper-cut equation. |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 3,139 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 175 of 177 scoped milestone rows. The newest result proves a sparse V53 constant-cut basis equivalent to the complete equation on every nonempty proper cut and gives a total classifier without carrier-subset enumeration. Acceptance derives constant activation and conditional ZeroSlack; rejection remains a structural diagnostic. Terminal data, raw cells and payloads, tables, ranks, route-clear proof, resolvers, normalizer, blocker semantics, upstream construction, and complete encoded-size bounds remain supplied. The result does not force acceptance or route rejection to a gain, complete PkgC/BN3 through BN6 integration, prove manuscript-wide SaturatePositive or BCELReady, construct executable PCCMin, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 141-page non-claiming report; mirrored by this site |
