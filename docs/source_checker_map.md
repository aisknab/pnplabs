...
core commit `fb9674949679a137b7ce790c1522efaecbe240ce`. Its generated status payload is current
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `fb9674949679a137b7ce790c1522efaecbe240ce` | Exact source tree reviewed and merged through concrete CNF-SAT verifier correctness, NP membership, the all-input local framer, and the four-stage compiler's exact canonical-pair verdict, output, no-timeout, and external polynomial bounds |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 157 reviewed milestone candidates; canonical counts |
