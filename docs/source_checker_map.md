...
core commit `171a90983820e1c6bba2fcf85203843b4bd3f5da`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 145-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `171a90983820e1c6bba2fcf85203843b4bd3f5da` (tree `513bbcf670bdfb02ae2640949aa8ba37c372fbcd`) | Exact source tree reviewed and merged through the concrete and terminal chain, finite Packet/HB and sidecar layers, report-facing compatibility links, typed PCCPack reflection, proof-bearing PCCMin orchestration, checked conditional ZeroSlack bridges, BCEL-derived BN6 skeleton, canonical positive-cell grouping, exact raw cut-ledger conservation, sparse V53 cut basis and activation route, PkgC cancellation-or-cellization over supplied active V54 consumers, source-derived integration with the checked sparse BCEL/Packet classifier, and computed multiplicity-preserving extraction from a supplied ambient BN4 ledger. |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 3,143 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 179 of 181 scoped milestone rows. The newest result computes multiplicity-preserving, order-independent extraction of generated PkgC cancellation cells from an arbitrary supplied ambient BN4 ledger, returning the exact remainder and residual reduction or proof that no exact embedding exists, while preserving M202's conditional source-routing branches. The ambient ledger, terminal data, checked readiness, source systems and cuts, payloads, restorer, downstream tables, HB evidence, route-clear proof, selector silence, upstream construction, and complete encoded-size bounds remain supplied or open. Neither a surviving remainder nor no-embedding result is a verified global gain or rank decrease. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 145-page non-claiming report; mirrored by this site |
