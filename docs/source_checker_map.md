...
core commit `63ba53d76da0878b0d3b2834b891afcc6ef0fe0e`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 132-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `63ba53d76da0878b0d3b2834b891afcc6ef0fe0e` (tree `98103f568ccb68561094d5516ada08c099f770a7`) | Exact source tree reviewed and merged through the concrete and terminal chain, the complete finite Packet selector pipeline, checked realizer and dependency layers, exact earliest-route semantics, all reflected local Packet fields, checked budget/HB and semantic/HN bindings, the Packet no-lower ledger, supplied-family HResolve routing, the terminal-derived support and budget resolvers, their same-candidate finite Packet composition, a deterministic maximal H-disjoint subfamily over supplied footprints, an exact certified-path minimum, a maximal H-disjoint certified-path family, proof-bearing HResolve, Budget, Selector/HB, Packet/budget no-lower, BCEL/Packet no-lower, and same-family Selector/HB/Packet/BCEL coherence ZeroSlack sidecars, the finite BCEL/Packet results, and the concrete report-facing locked-NAND compatibility link. |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 3,120 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 162 of 164 scoped milestone rows. The newest result makes the report-facing SAT and locked-NAND interfaces exact names for the concrete finite-pipeline model and directly reuses its checked all-input reduction. It removes one project axiom and caller-supplied reduction edge but does not supply a target decider, SAT hardness transport, unconditional residual minimisation, ZeroSlack, exact polynomial PCCMin, or the root theorem. Two global rows, three project axioms, and five blockers remain. The row ratio is not proof completion. |
| Fixed-weight progress | `public/pnp-proof-progress.json` | Exact mirror of the canonical 100-point checkpoint ledger, including the separate risk-weighted estimate, uncertainty, evidence coverage, five global gates, three project axioms, root state, and publication gate. Its validator resolves checkpoint evidence against status and compiled declarations. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 132-page non-claiming report; mirrored by this site |
...
- The key mathematical review targets remain locked-NAND threshold correctness, exact codec/parser/emitter/compiler behavior and reduction composition, the M186 compatibility/axiom-removal boundary, the missing target decider and SAT NP-hardness or CNF-SAT NP-completeness transport, residual-band exact minimization, no-hidden-minimization discipline, proof-ref soundness, and public theorem boundary discipline.
