...
core commit `c225e91eeb469cd87f0c52c9731074e8b66fc573`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the seventy-five-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `c225e91eeb469cd87f0c52c9731074e8b66fc573` (tree `9616b6c150124561a7f88672aa20a34bba1a4aae`) | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results and its bounded builder prefix, the locked-NAND typed threshold and concrete compiler/reduction chain, the universal residual-gain bound and global stopping specification, the terminal full-carrier and quotient/full firewall bridges, exhaustive attained full/quotient projection minima, the exact signed four-corner projection-transfer identity, finite terminal saturation under explicitly supplied rule-tagged dependencies, executable exact boundary/interface wire completion, arbitrary finite terminal-support extraction with exact open semantics and whole-circuit recovery, governed proper-positive support search, saturated support-square meet/join closure, and exact governed boundary, interface, and ten-role profile completion for every computed square corner inside an explicit finite dependency system |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 2,240 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Eighty-three earned scoped milestones, including the arbitrary-circuit locked-NAND threshold, its encoded semantic boundary, strict-v0 parser/emitter, concrete polynomial reductions, the universal gain-chain bound, global semantic stopping criterion, direct-wire terminal full-carrier bridge, computed terminal quotient/full mode firewall, exhaustive attained projection minima, exact signed four-corner transfer identity, finite terminal saturation closure, executable exact boundary/interface wire completion, arbitrary finite terminal-support extraction, governed proper-positive support search, saturated support-square meet/join closure, and governed boundary, interface, and ten-role profile completion for computed square corners; three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current seventy-five-page non-claiming report; mirrored by this site |
