...
core commit `6e7124f7edf19b2ab077b9b52ed267cb6ecb00e3`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 133-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 165 of 167 scoped milestone rows. The newest result proves total, terminating, semantics-preserving strict-gain recursion, exact-minimum output, zero residual slack, and a starting-slack iteration bound under an explicit proof-bearing total oracle. The oracle remains supplied and the regression fixture uses exhaustive reference minimisation, so the result does not construct executable PCCMin, derive terminal routes, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 133-page non-claiming report; mirrored by this site |
