...
core commit `eb631703727b4121447eab01060712343b5e22f1`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 138-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 172 of 174 scoped milestone rows. The newest result constructs the BN6 family carrier, duplicate-freedom, cut value, and positivity from the checked BCEL nucleus, makes M195's carrier and cut-value mismatch routes impossible, and leaves either an exact activation mismatch or conditional ZeroSlack. The terminal problem, positive premise, checked finite BCEL-ready certificate, grouped cells and payloads, exact grouping proofs, tables, ranks, route-clear proof, resolvers, normalizer, blocker semantics, encoded-size bounds, and exhaustive powerset classifier remain supplied. The remaining mismatch is not a constructed gain or globally decreasing transition. The result does not complete PkgC/BN3 through BN6 integration, prove manuscript-wide SaturatePositive or BCELReady, derive the supplied data from arbitrary input, construct executable PCCMin, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 138-page non-claiming report; mirrored by this site |
