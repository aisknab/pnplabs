...
core commit `3112ca90d74d58116dc53ea5300082d0caa63c0e`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the forty-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `3112ca90d74d58116dc53ea5300082d0caa63c0e` | Exact source tree reviewed and merged through concrete CNF-SAT NP membership, raw-machine compilation, Cook-Levin semantic/size/schedule results, all three complete fixed clauses, clause-two and clause-three padding traversal, the separator beginning clause four, and the complete first negative literal in clause four |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 1221 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | Forty-one earned scoped milestones, three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current forty-page non-claiming report; mirrored by this site |
