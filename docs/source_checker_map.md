...
core commit `d3794beaf85242ea6d5c4f141f74e52876261c4f`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the ninety-eight-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | One hundred and twenty-three earned scoped milestones through a total HN/BUD dependency table that materializes its graph from every row and derives exact representation coverage, well-founded induction, and cycle exclusion; the table, rank mapping, blocker semantics, semantic dependency completeness, local invariant premise, rank assignment, faithfulness predicate, claims, activity tables, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, unmatched lists, and gain-coverage certificate remain supplied inputs or open obligations; the result does not establish selector compatibility, rank-complete selector silence, the full HB negative closure, encoded-size or runtime bounds, complete global PkgC or global routing, or polynomial PCCMin; two unearned global milestones, four project axioms, and five blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current ninety-eight-page non-claiming report; mirrored by this site |
