...
core commit `289ccde2874be3f2f4684470602cad073858fac0`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 115-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| --- | --- | --- |
| Merged formalization | `aisknab/pnp` commit `289ccde2874be3f2f4684470602cad073858fac0` (tree `f0e0d8ac1d1e538deb8ae2e75de50988c7abae48`) | Exact source tree reviewed and merged through computed BN2 square legitimacy, the canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through conditional gain coverage, the generic finite Packet charge-surplus strict-gain kernel, the checked realizer and dependency layers, exact earliest-route semantics, and successive reflection of descent, rank, exact source route, charge, colour, frontier, obligation, activation, and direction. Direction acceptance is computed from equality of explicit typed source and selector direction values; a direction route carries the three prior equalities plus typed-direction inequality; and colour, charge, exactRoute, and rank remain excluded. |
| Compiled declaration evidence | `public/pnp-theorem-inventory.json` in that commit | Names, modules, kinds, and axiom closures for all public declarations; raw kernel types for 2,945 reviewed milestone candidates; canonical counts |
...
| Generated status | `public/pnp-status.json` | 139 of 141 scoped milestone rows are earned. The newest result computes Packet direction acceptance from equality of explicit typed source and selector direction values while retaining computed BN5 frontier, obligation, BN4 activation, grouped colour, positive charge, the internal source route, authoritative rank, and exact RankWF descent computation. A direction first route is prior frontier, obligation, and activation equality with typed-direction inequality; the classifier also cannot return colour, charge, exactRoute, or rank; a final descent route carries actual nondecrease; and the positive Packet/HB endpoint carries exact failure evidence. The direction values, BN5 coordinates, finite rank map, before/after ranks, budget, activity bits, dependency rows, blocker semantics, semantic dependency completeness, exhaustive claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, and unmatched lists remain supplied inputs or open obligations. The direction values are not constructed from terminal data, and the result does not prove the manuscript's complete Dir(u) or Packet adequacy bridge. It does not prove the sole remaining budget route's external manuscript semantics or decreasing complete global coverage, guarantee a decreasing transition, construct the no-lower ledger, derive positive slack or those inputs from terminal data, establish full external selector compatibility, close unconditional HB negative closure, provide encoded-size or runtime bounds, complete global PkgC, prove unconditional ZeroSlack, or establish polynomial PCCMin. Two global rows, four project axioms, and five blockers remain. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 115-page non-claiming report; mirrored by this site |
