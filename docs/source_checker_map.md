...
core commit `34713d47d7e00298aeb532dc9d5c69e57d11f296`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the seventy-seven-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | Eighty-nine earned scoped milestones, including the arbitrary-circuit locked-NAND threshold, its encoded semantic boundary, strict-v0 parser/emitter, concrete polynomial reductions, the universal gain-chain bound, global semantic stopping criterion, direct-wire terminal full-carrier bridge, computed terminal quotient/full mode firewall, exhaustive attained projection minima, exact signed four-corner transfer identity, finite terminal saturation closure, executable exact boundary/interface wire completion, arbitrary finite terminal-support extraction, governed proper-positive support search, saturated support-square meet/join closure, governed boundary, interface, and ten-role profile completion for computed square corners, exact governed frontier gluing, governed projection commutation, side-tight four-corner minimum arithmetic, checked structural carrier transport, four-corner optimum carrier compatibility, and the coherent-tuple-or-first-failure classifier; three unearned global milestones, four project axioms, and six blockers |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current seventy-seven-page non-claiming report; mirrored by this site |
