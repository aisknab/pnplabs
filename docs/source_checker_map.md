...
core commit `df4f4d830f6a0fd44af51edb0be178652d1b9417`. Its generated status payload is current
...
| Generated status | `public/pnp-status.json` | Eighty-eight earned scoped milestones, including the arbitrary-circuit locked-NAND threshold, its encoded semantic boundary, strict-v0 parser/emitter, concrete polynomial reductions, the universal gain-chain bound, global semantic stopping criterion, direct-wire terminal full-carrier bridge, computed terminal quotient/full mode firewall, exhaustive attained projection minima, exact signed four-corner transfer identity, finite terminal saturation closure, executable exact boundary/interface wire completion, arbitrary finite terminal-support extraction, governed proper-positive support search, saturated support-square meet/join closure, governed boundary, interface, and ten-role profile completion for computed square corners, exact governed frontier gluing, governed projection commutation, side-tight four-corner minimum arithmetic, checked structural carrier transport, and four-corner optimum carrier compatibility; three unearned global milestones, four project axioms, and six blockers |
