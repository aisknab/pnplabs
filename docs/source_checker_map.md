...
core commit `26547c1a3c05de0c907021320352a46a8145dbaa`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 108-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | 133 of 135 scoped milestone rows are earned. The newest result follows every canonical Packet handle to its exact grouped cell and original positive payload atom, marks that internal source route clear by construction, copies the authoritative handle rank, and retains exact RankWF descent computation. The canonical classifier can return neither exactRoute nor rank, a final descent route carries actual nondecrease, and the positive Packet/HB endpoint carries exact failure evidence. The finite rank map, seven semantic Boolean fields, before/after ranks, activity bits, dependency rows, blocker semantics, semantic dependency completeness, exhaustive claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, and unmatched lists remain supplied inputs or open obligations. The internal source route is not an external exact minimum. The result does not prove the seven remaining routes' external manuscript semantics or decreasing complete global coverage, guarantee a decreasing transition, construct the no-lower ledger, derive positive slack or those inputs from terminal data, establish full external selector compatibility, close unconditional HB negative closure, provide encoded-size or runtime bounds, complete global PkgC, prove unconditional ZeroSlack, or establish polynomial PCCMin. Two global rows, four project axioms, and five blockers remain. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 108-page non-claiming report; mirrored by this site |
