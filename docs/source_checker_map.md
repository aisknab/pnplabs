...
core commit `8bc8d36fea5abf3f486e9dbfe7cfccea5c6b34e0`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 103-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | 128 of 130 scoped milestone rows are earned. The newest constructor replaces the free HB faithfulness function with the canonical positive source-payload computation, preserves the supplied rank map, realizer claims, and HN/BUD activity exactly, and removes the separate binding premise from the Packet-to-HB contradiction. Payload checks, route-clear evidence, activity bits, dependency rows, rank mapping and assignment, blocker semantics, semantic dependency completeness, exhaustive claims, grouped BN6 family, candidate implementations, replacement blueprints, occurrence pairings, and unmatched lists remain supplied inputs or open obligations. The result does not establish positive slack, terminal derivation of those inputs, full external selector compatibility, unconditional HB negative closure, encoded-size or runtime bounds, complete global PkgC or global routing, unconditional ZeroSlack, or polynomial PCCMin. Two global rows, four project axioms, and five blockers remain. |
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 103-page non-claiming report; mirrored by this site |
