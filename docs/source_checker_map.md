...
core commit `c912b110f14386d335d58bb89b1bb9bb643bffb4`. Its generated status payload is current
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 166 of 168 scoped milestone rows. The newest result composes an explicit proof-bearing non-increasing semantic normalizer with the explicit proof-bearing total oracle, lifts later gains to strict gains from the original implementation, and transports exact-minimum and ZeroSlack endpoints through normalization into the checked terminating loop. Both stages remain supplied and the regression fixtures use exhaustive reference minimisation, so the result does not construct executable PCCMin, derive terminal routes, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
