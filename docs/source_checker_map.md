...
core commit `821f05facdda1d41e6c1fb7b2e5206b2e2703715`. Its generated status payload is current
publication-status authority. The compiled theorem inventory is the exact evidence mirror, while
the 136-page TeX/PDF report is derived presentation of the same fail-closed gate.
...
| Generated status | `public/pnp-status.json` | Formal artefact coverage is 170 of 172 scoped milestone rows. Under one explicit constant-activation premise, the newest result carries positive residual slack into the general BN6 Packet theorem, computes a faithful selector from checked route-clear payload data, and uses the M193 checked HB contradiction to derive conditional ZeroSlack before reusing the checked loop. Constant activation, the terminal family, carrier lower bound, payload and rank construction, route-clear proof, claim and HB tables, resolvers, normalizer, blocker semantics, encoded-size bounds, and the exhaustive reference fixture remain supplied. The result does not prove manuscript-wide SaturatePositive or BCELReady, derive the supplied data from arbitrary input, construct executable PCCMin, prove unconditional ZeroSlack or encoded-size polynomial runtime, provide a target decider or SAT hardness transport, or create the root theorem. Two global rows and five blockers remain; no project-specific axioms remain. The row ratio is not proof completion. |
...
| Generated report | root `canonical_proof_report.tex` and `.pdf` | Current 136-page non-claiming report; mirrored by this site |
...
- The key mathematical review targets remain locked-NAND threshold correctness, exact codec/parser/emitter/compiler behavior and reduction composition, the M186 locked-NAND, M187 residual-band, M188 typed PCCPack compatibility/axiom-removal, M189 supplied-total-oracle boundaries, M190 supplied-normalizer/oracle composition boundaries, M191 rank-ordered orchestration boundaries, M192 supplied checked-Packet-table boundaries, M193 checked HB closure, and the M194 supplied constant-activation and route-clear-data boundary. M194 computes the faithful selector consumed by the M193 contradiction, but it does not derive constant activation, the terminal family, carrier lower bound, payload or rank data, route-clear proof, claim or HB tables, supplied stages, blocker semantics, or encoded-size bounds; prove manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack; or establish encoded-size polynomial execution. The missing target decider and SAT NP-hardness or CNFSAT NP-completeness transport, polynomial residual-band minimization and promise bounds, no-hidden-minimization discipline, proof-ref soundness, and public theorem boundary discipline remain open.
