...
The canonical report downloads are now a forty-five-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 10,207 public declarations across 89 modules,
including 5,600 theorem-kind declarations, 3,294 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current forty-five-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1445 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand four hundred forty-five reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Forty-six narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, and exact no-emission traversal through the remaining padding of the first scheduled constraint under external polynomial bounds | Emission of the separator beginning the second scheduled constraint, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `5a2cf2b0bea9568d7361336fa5f8f197246a2f9c`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1445 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns forty-six scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, all four complete fixed clauses, and exact no-emission traversal through the remaining padding of the first scheduled constraint under external polynomial bounds; typed direct-wire semantics; finite reference enumeration/minimum;
...
the manuscript at the pinned 7072f8d source tag, never to the current forty-five-page report.
