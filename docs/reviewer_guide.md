...
The canonical report downloads are now an eighty-one-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 26,485 public declarations across 238 modules,
including 13,860 theorem-kind declarations, 7,139 assumption-free theorem-kind declarations, and four
disclosed project axioms. Exactly 14,928 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current eighty-one-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,468 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,468 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Ninety-seven narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, the whole-support positivity firewall, candidate-derived terminal saturation cost balance, finite interface-exposure and origin/kernel/obligation closure routing, and the finite terminal positive-saturation composition for an explicit proof-bearing positive-slack problem | Treating a local route as a complete global outcome, full Package E acceptance, verified global gain, or global route completeness; treating the finite composition as manuscript-wide `SaturatePositive`; hiding the positive initial full-slack premise; claiming manuscript activation or charge classes, Package E, BCELReady, RankWF, later BCEL/BN2-BN6 conclusions, a complete Cook-Levin formula builder, the manuscript's ZeroSlack/PCCMin construction and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, discharge of the abstract threshold axiom, CNF-SAT in P, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `3f4352d190b44d34866500e672c2ef2af89e08de`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,468 reviewed milestone declarations at its exact kernel type.
...
the manuscript at the pinned 7072f8d source tag, never to the current eighty-one-page report.
