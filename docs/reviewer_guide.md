...
The canonical report downloads are now a ninety-eight-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 28,720 public declarations across 264 modules,
including 14,811 theorem-kind declarations, 7,460 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current ninety-eight-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,712 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,712 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | One hundred and twenty-three narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through conditional gain coverage, the generic finite charge-surplus strict-gain kernel, a checked unit-charge blueprint realizer, a checked finite Packet typed-realizer contract, exact-rank acyclicity for a supplied finite HN and budget dependency graph, and a total HN/BUD dependency table whose graph is materialized from every row | Treating the dependency table, rank mapping, rank assignment, faithfulness predicate, claims, activity tables, grouped family, candidates, blueprints, occurrence pairings, unmatched lists, coverage certificate, or upstream data as derived from terminal candidates; presenting blocker activity as blocker semantics or total row coverage as semantic dependency completeness, rank-complete selector silence, the full HB negative closure, or unconditional ZeroSlack; claiming the local invariant premise, selector compatibility, a circuit-size bound, polynomial enumeration, a completed global gain-or-blocker route, complete PkgC or decreasing global routing, polynomial PCCMin, a target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `d3794beaf85242ea6d5c4f141f74e52876261c4f`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,712 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and twenty-three scoped milestones: the concrete bitstring/machine/cost kernel,
...
derives exact row-to-edge coverage, well-founded induction with an explicit local premise, and cycle exclusion.
...
the manuscript at the pinned 7072f8d source tag, never to the current ninety-eight-page report.
