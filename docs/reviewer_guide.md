...
The canonical report downloads are now a 153-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 32,412 public declarations across 332 modules,
including 16,515 theorem-kind declarations, 7,859 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,453 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 153-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,152 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,152 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 188 of 190 scoped formal milestone rows. For every concrete verifier tableau problem and every natural coordinate, Lean now composes M209's checked raw result reader with M211's fixed positive-width divider. The exact M209 and M211 traces are retained; every M210 body, Finish, and out-of-range result is recovered; every coordinate inside the complete token schedule excludes the out-of-range branch; and the summed compiled work is bounded by one verifier-derived polynomial in source-input length. | Treating the executable raw-launch orchestration as a literal tape-to-tape bridge preserving the complete workspace, as body or Finish token emission, or as the complete formula builder or reduction; treating supplied terminal, source, restoration, ambient, BCEL, Packet/HB, rank, claim, dependency, route-clear, or selector-silence data as terminal-derived; treating a local obstruction as a verified global gain or descent; treating conditional ZeroSlack as unconditional; or treating the row as executable PCCMin, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `3d2cc2eda40f6dcaa7758e96af14b203f69c55cb`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,152 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns 188 scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current 153-page report.
