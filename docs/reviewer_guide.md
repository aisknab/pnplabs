...
The canonical report downloads are now a ninety-five-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 28,414 public declarations across 261 modules,
including 14,690 theorem-kind declarations, 7,390 assumption-free theorem-kind declarations, and four
disclosed project axioms. Exactly 15,043 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current ninety-five-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,689 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,689 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | One hundred and twenty narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through conditional gain coverage, the generic finite charge-surplus strict-gain kernel, and a checked unit-charge blueprint realizer | Treating the grouped family, candidates, blueprints, occurrence pairings, unmatched lists, coverage certificate, or upstream data as derived from terminal candidates; presenting family-local validator silence as a typed blocker, global no-gain result, semantic minimum, or unconditional ZeroSlack; claiming selector faithfulness, a circuit-size bound, polynomial enumeration, typed blocker or rank closure, a completed global gain-or-blocker route, complete PkgC or decreasing global routing, polynomial PCCMin, a target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `5f2d880b953cfc1f2d26b6fc0d6a97d5a1c2b8cf`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,689 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and twenty scoped milestones: the concrete bitstring/machine/cost kernel,
...
one selected source cell; and an exhaustive gain scan across every canonical selector in the supplied family.
...
the manuscript at the pinned 7072f8d source tag, never to the current ninety-five-page report.
