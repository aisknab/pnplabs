...
The canonical report downloads are now a 136-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 30,569 public declarations across 310 modules,
including 15,667 theorem-kind declarations, 7,690 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,151 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 136-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,134 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,134 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 170 of 172 scoped formal milestone rows. Under one explicit constant-activation premise, the newest row carries positive residual slack into the general BN6 Packet theorem, computes a faithful selector from checked route-clear payload data, and uses the M193 checked HB contradiction to derive conditional ZeroSlack before reusing the checked loop. | Treating constant activation, supplied route-clear or terminal data, or other supplied stages as manuscript-wide SaturatePositive, BCELReady, executable PCCMin, unconditional ZeroSlack, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `821f05facdda1d41e6c1fb7b2e5206b2e2703715`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,134 reviewed milestone declarations at its exact kernel type.
...
bijection. The newest layer tests the required total-defect and proper-cut activation equations on that same family.
Full numerical coherence would contradict the existing Packet exclusion, so its deterministic classifier returns a
proof-bearing mismatch. This diagnoses rather than repairs the missing numerical bridge. It does not derive the
supplied family or activation data, establish manuscript-wide SaturatePositive or BCELReady, complete the global
route system, prove unconditional ZeroSlack, or prove polynomial PCCMin.
...
3. The literal target emitter and strict parser/emitter composition are formalized. The standalone
   ...
   faithful selector consumed by the M193 contradiction. Constant activation, the terminal family, carrier
   lower bound, payload and rank construction, route-clear proof, claim and HB tables, normalizer, resolvers,
   blocker semantics, and encoded-size bounds remain supplied, so the route still does not prove
   manuscript-wide SaturatePositive or BCELReady,
   construct executable PCCMin, prove unconditional ZeroSlack, decide
...
the manuscript at the pinned 7072f8d source tag, never to the current 136-page report.
