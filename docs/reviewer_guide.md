...
The canonical report downloads are now a ninety-four-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 28,241 public declarations across 260 modules,
including 14,628 theorem-kind declarations, 7,370 assumption-free theorem-kind declarations, and four
disclosed project axioms. Exactly 15,033 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current ninety-four-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,676 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,676 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | One hundred and nineteen narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through conditional gain coverage, and a generic finite charge-surplus strict-gain kernel | Treating the finite ledgers, pairing, unmatched witness, gate accounting, semantics, coverage certificate, upstream data, candidate implementations, or grouped family as derived from terminal candidates; presenting the charge-surplus kernel as a complete realizer or unconditional ZeroSlack; claiming selector faithfulness, a circuit-size bound, polynomial enumeration, typed blocker or rank closure, a completed global gain-or-blocker route, complete PkgC or decreasing global routing, polynomial PCCMin, a target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `77b275efe586e68339f6dd61abb5a2d3abcdce68`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,676 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and nineteen scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current ninety-four-page report.
