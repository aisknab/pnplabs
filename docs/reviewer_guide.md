...
The canonical report downloads are now a sixty-six-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 20,965 public declarations across 185 modules,
including 11,430 theorem-kind declarations, 5,968 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current sixty-six-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,035 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,035 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Sixty-nine narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence and its bounded builder prefix, the locked-NAND typed threshold and strict-v0 semantic encoding, one literal polynomial-time source parser, one literal exact target emitter, and their concrete `EncodedNANDSAT`-to-`EncodedLockedNANDThreshold` polynomial reduction | A complete formula builder, identification with ordinary CNF-SAT or NP-hardness, discharge of the abstract threshold axiom, complete residual search, CNF-SAT in P, NP-completeness, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `03f62a5465c1eacd399671121123a3891d8b3e67`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,035 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns sixty-nine scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current sixty-six-page report.
