...
The canonical report downloads are now a fifty-five-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 11,055 public declarations across 97 modules,
including 6,294 theorem-kind declarations, 3,428 assumption-free theorem-kind declarations, and four
disclosed project axioms. Four thousand two hundred seventy-eight private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current fifty-five-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1729 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand seven hundred twenty-nine reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Fifty-four narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, traversal through the remaining first-constraint padding, the complete first literal of the second scheduled constraint, its width-selected successor token, and the next padding-or-unary opportunity under an external polynomial bound | The following padding or second unary opportunity, traversal of the second constraint, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `5aba133715790bae354e584c0d8606c19bb3ab8b`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1729 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns fifty-four scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current fifty-five-page report.
