...
Lean theorem inventory. The inventory contains 27,659 public declarations across 248 modules,
including 14,395 theorem-kind declarations, 7,336 assumption-free theorem-kind declarations, and four
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,566 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,566 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | One hundred and seven narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the candidate-derived finite BN3 request envelope, the finite BN4 activation-exact cancellation kernel, the finite BN5 full-shadow localization kernel, the finite PkgC separating-consumer restoration dichotomy and typed restoration realization, the V54 consumer-antichain normal form, the V53 constant-cut hypergraph rigidity classification, and the finite BN6 grouped hypergraph-packet bridge with payload witnesses | Treating explicit BN4 or BN5 inputs, the PkgC antichain or typed restoration operation, or the BN6 survivor family, grouping, payloads, singletonization proofs, and constant-cut equation as derived from terminal candidates; treating the typed restoration operation as fully semantically adequate; treating complete BN5 matching or PkgC coverage as a BN4/BN5 contradiction; treating a local PkgC route as a complete global outcome; treating either PkgC classifier as the full historical PkgC theorem or route silence; treating the BN6 bridge as the full historical BN6 or Packet selector and realizer result; treating the finite BN3 envelope as polynomial; claiming a complete decreasing global route system, global ZeroSlack, or polynomial PCCMin; treating the concrete reduction as a target decider or hardness theorem; treating RankWF as proof that current routes decrease; treating a local route as Package E acceptance; claiming manuscript-wide `SaturatePositive`, BCELReady, a complete Cook-Levin formula builder, CNF-SAT in P, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `fe2c8ceb024d0a1afcb2a79a21015eb2969c37bd`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,566 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and seven scoped milestones: the concrete bitstring/machine/cost kernel,
