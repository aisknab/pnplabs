...
The canonical report downloads are now a 135-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 30,498 public declarations across 309 modules,
including 15,649 theorem-kind declarations, 7,690 assumption-free theorem-kind declarations, and no
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 135-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,133 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,133 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 169 of 171 scoped formal milestone rows. The newest row derives complete selector silence from checked Packet claims and exact rank rows, uses checked HB no-outcome closure to eliminate every canonical faithful handle, and reaches conditional ZeroSlack through one explicit positive-slack bridge before reusing the M192 selector construction and loop. | Treating the supplied positive-slack bridge or other supplied stages and data as executable PCCMin, unconditional ZeroSlack, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `9f2e7f022d1483702a1ead06392ac2dafe86120b`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,133 reviewed milestone declarations at its exact kernel type.
...
3. The literal target emitter and strict parser/emitter composition are formalized. The standalone
   ...
   ZeroSlack. The terminal family, rank assignment, claim table, positive-slack bridge, normalizer,
   resolvers, blocker semantics, and encoded-size bounds remain supplied, so the route still does not
...
the manuscript at the pinned 7072f8d source tag, never to the current 135-page report.
