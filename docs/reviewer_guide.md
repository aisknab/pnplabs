...
The canonical report downloads are now a sixty-nine-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 23,601 public declarations across 210 modules,
including 12,818 theorem-kind declarations, 6,776 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current sixty-nine-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,093 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,093 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Seventy-two narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence and its bounded builder prefix, the locked-NAND typed threshold and strict-v0 semantic encoding, one literal polynomial-time source parser, one literal exact target emitter, their concrete `EncodedNANDSAT`-to-`EncodedLockedNANDThreshold` polynomial reduction, the CNF-to-NAND semantic compiler, its fixed all-input finite-machine direct and composed polynomial reductions, and the universal verified residual-gain-chain bound | A complete Cook-Levin formula builder, SAT NP-hardness or CNF-SAT NP-completeness, discharge of the abstract threshold axiom, complete residual search, CNF-SAT in P, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `3894e5e2dd3f34c2fe19f6eb0b9e39119ad05403`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,093 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns seventy-two scoped milestones: the concrete bitstring/machine/cost kernel,
...
at most its starting residual slack, hence at most four for the complete locked-NAND candidate.
...
2. The exact `X/T/O/R/L/z` carrier, both trace-equivalence directions, complete baseline plus
   ...
   verified steps. This verifies and bounds a disclosed chain, but does not generate gains, prove
   completeness or stopping, construct `ZeroSlack`, compute an exact minimizer, or establish
   polynomial checker/PCCMin runtime. Strict-v0 codecs now prove normalized-circuit and complete-instance
...
the manuscript at the pinned 7072f8d source tag, never to the current sixty-nine-page report.
