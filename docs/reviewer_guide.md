...
Lean theorem inventory. The inventory contains 28,193 public declarations across 259 modules,
including 14,607 theorem-kind declarations, 7,359 assumption-free theorem-kind declarations, and four
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,668 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,668 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | One hundred and eighteen narrowly scoped formal milestones through computed BN2 square legitimacy, a canonical positive terminal BCEL anchor nucleus, candidate-derived terminal saturation and finite routing, the fixed residual RankWF, the report-facing concrete locked-NAND threshold reduction, the finite BN3, BN4, BN5, PkgC, V54, V53, and grouped BN6 packet chain, the complete finite Packet selector pipeline through exhaustive supplied-universe gain scanning, and a conditional source-gain-or-ZeroSlack bridge under an explicit proof-bearing global gain-coverage certificate | Treating the certificate, upstream data, candidate implementations, or grouped family as derived from terminal candidates; presenting the conditional result as unconditional ZeroSlack; claiming selector faithfulness, a circuit-size bound, polynomial enumeration, typed blocker or rank closure, a completed global gain-or-blocker route, complete PkgC or decreasing global routing, polynomial PCCMin, a target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `f149a972c366b96a3db704aaa1a23679abc21b6a`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,668 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and eighteen scoped milestones: the concrete bitstring/machine/cost kernel,
