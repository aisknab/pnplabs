...
The canonical report downloads are now a forty-one-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 9,661 public declarations across 85 modules,
including 5,173 theorem-kind declarations, 3,215 assumption-free theorem-kind declarations, and four
disclosed project axioms. Three thousand five hundred nine private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current forty-one-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1295 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand two hundred ninety-five reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Forty-two narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, all three complete fixed clauses, clause-two and clause-three padding traversal, the separator beginning clause four, and the complete first and second negative literals in clause four under external polynomial bounds | Emission of the following `Finish`, completion of clause four, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `3a56b27add47f7991b670e6e0fb9bb302d78cd04`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1295 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns forty-two scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current forty-one-page report.
