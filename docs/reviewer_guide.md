...
Lean theorem inventory. The inventory contains 5,125 public declarations across 47 modules,
including 2,168 theorem-kind declarations, 2,067 assumption-free theorem-kind declarations, and four
disclosed project axioms. Nine hundred and sixty-one private compiler auxiliaries are excluded explicitly.
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 149 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One hundred forty-nine reviewed theorem-type fingerprints, empty axiom closures, and the complete Lean-source digest | Nine narrowly scoped formal milestones, including `CNFSAT ∈ NP` and the four-stage compiler's exact canonical-pair verdict, output, no-timeout, and external polynomial bounds | Malformed-input behavior, uniform all-input raw refinement, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `cc3c004f3498a0f28b8afda688802d322b9e5c21`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 149 reviewed milestone declarations at its exact kernel type.
...
including collision-free state namespaces and one full four-stage raw compiler for proof-bearing
...
1. The charged-pipeline model and concrete CNF-SAT language are formalized. For every proof-bearing polynomial-time target machine and canonical `BitString.pair`, the ordinary paired-input execution now runs through framing, simulation, represented handoff, terminal launch, and raw-output packing in one finite machine with exact verdict and `machineOutput`, no timeout, and explicit external bounds `B(m)` and `R(m)`. Malformed-input behavior, uniform all-input raw refinement, the concrete complexity machine link, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider are absent.
