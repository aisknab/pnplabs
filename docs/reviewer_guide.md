...
The canonical report downloads are now a thirty-seven-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 9,251 public declarations across 82 modules,
including 4,864 theorem-kind declarations, 3,140 assumption-free theorem-kind declarations, and four
disclosed project axioms. Three thousand two hundred five private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current thirty-seven-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1126 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand one hundred twenty-six reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Thirty-nine narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, all three complete fixed clauses, and clause-two and clause-three padding traversal under external polynomial bounds | Emission of the fourth-clause separator, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `783941d428aa43cc2fb6763ed5f58fd9dab9ec44`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1126 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns thirty-nine scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, all three complete fixed clauses, and the remaining clause-two padding run under external polynomial bounds; typed direct-wire semantics; finite reference enumeration/minimum;
...
the manuscript at the pinned 7072f8d source tag, never to the current thirty-seven-page report.
