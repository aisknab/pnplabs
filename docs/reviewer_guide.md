...
The canonical report downloads are now a forty-six-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 10,300 public declarations across 90 modules,
including 5,678 theorem-kind declarations, 3,306 assumption-free theorem-kind declarations, and four
disclosed project axioms. Three thousand eight hundred twenty-eight private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current forty-six-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1479 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand four hundred seventy-nine reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Forty-seven narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, traversal through the remaining first-constraint padding, and emission of the single separator beginning the second scheduled constraint under external polynomial bounds | The following `T` and first literal, traversal of the second constraint, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `8ac394c0fb19fbaabec883a498a1ad35d730b77f`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1479 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns forty-seven scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current forty-six-page report.
