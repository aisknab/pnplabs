...
The canonical report downloads are now a twenty-eight-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 8,229 public declarations across 74 modules,
including 4,080 theorem-kind declarations, 2,964 assumption-free theorem-kind declarations, and four
disclosed project axioms. Two thousand five hundred eighty-five private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current twenty-eight-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 729 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Seven hundred twenty-nine reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Thirty-one narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, the complete first clause, and the complete negative literal on variable zero beginning clause two under an external polynomial bound | A general dynamic formula cursor, completed clause two, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `bcabe7b58b7d72e738471f036f1c44b905b949eb`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 729 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns thirty-one scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current twenty-eight-page report.
