...
The canonical report downloads are now a 120-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 29,347 public declarations across 284 modules,
including 15,206 theorem-kind declarations, 7,526 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 120-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,989 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,989 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 143 of 145 scoped formal milestone rows. The newest executable checker scans every canonical Packet handle and accepts exactly when no fully computed first route is residual nondecrease. A supplied positive Packet conclusion, checked semantic/HN and budget/HB bindings, selector silence, and well-founded HB closure force it to reject; accepting that local row is contradictory. | Treating the grouped family, rank map, residual ranks, typed fields, realizer or dependency tables, bindings, silence, closure checks, candidates, blueprints, occurrence pairings, unmatched lists, or upstream data as terminal-derived; presenting one local row as the complete no-lower ledger, HResolve, BudgetResolve, normalization, named descent routing, saturation, replay, unconditional HB closure, ZeroSlack, or complete global routing; claiming positive slack, a circuit-size bound, polynomial enumeration, complete PkgC, polynomial PCCMin, target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `7a3308520a68b5553cc95a7b56c560a20508111a`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,989 reviewed milestone declarations at its exact kernel type.
...
the manuscript at the pinned 7072f8d source tag, never to the current 120-page report.
