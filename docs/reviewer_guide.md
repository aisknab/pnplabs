...
The canonical report downloads are now a twenty-two-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 7,746 public declarations across 70 modules,
including 3,725 theorem-kind declarations, 2,865 assumption-free theorem-kind declarations, and four
disclosed project axioms. Two thousand two hundred eighty-six private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current twenty-two-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 549 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Five hundred forty-nine reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Twenty-seven narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, the complete width header, and the `T^FormulaWidth F Sep T F T T F T T T F Finish` first-clause prefix under an external polynomial bound | The dynamic cursor controller, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `f5b7b31b549260c6366302e616c71565e8857112`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 549 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns twenty-seven scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, the complete width header, and a literal `T^FormulaWidth F Sep T F T T F T T T F Finish` first-clause composition with a retained next coordinate under an external polynomial bound; typed direct-wire semantics; finite reference enumeration/minimum;
...
1. The charged-pipeline model and concrete CNF-SAT language are formalized. Every proof-bearing function or decision program tree recursively compiles into one literal finite machine. The Cook-Levin construction proves exact semantic equivalence between its generated formula and the verifier language, bounds the actual encoded formula by an external input-size polynomial, supplies an exact answer-independent rectangular schedule, and supplies direct coordinate decoders with exact fuelled traversal. Fixed finite machines prove input preparation and reusable token appending. One literal finite machine emits `T^FormulaWidth F Sep T F T T F T T T F Finish`, matching the canonical prefix through the complete positive first clause on variables zero, one, and two and retaining `formulaVariableSlotBound + 12` as the next coordinate under an external polynomial bound. The retained coordinate is only data: the machine does not implement the dynamic cursor or remaining formula body and is not a complete raw builder, builder `FunctionProgram.RawRefinement`, or packaged polynomial reduction. CNF-SAT NP-completeness and a deterministic polynomial-time CNF-SAT decider are absent.
...
the manuscript at the pinned 7072f8d source tag, never to the current twenty-two-page report.
