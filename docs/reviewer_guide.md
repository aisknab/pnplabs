...
The canonical report downloads are now a nineteen-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 7,418 public declarations across 67 modules,
including 3,473 theorem-kind declarations, 2,764 assumption-free theorem-kind declarations, and four
disclosed project axioms. Two thousand thirty-two private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current nineteen-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 414 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Four hundred fourteen reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Twenty-four narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, and the complete width-header composition under an external polynomial bound | The dynamic cursor controller, formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `62c35edef4bb64f2c665051e61d07c7b17f70ad3`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 414 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns twenty-four scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, the first-token composition, a structurally generated unary polynomial evaluator, and a literal complete width-header composition under an external polynomial bound; typed direct-wire semantics; finite reference enumeration/minimum;
...
the manuscript at the pinned 7072f8d source tag, never to the current nineteen-page report.
