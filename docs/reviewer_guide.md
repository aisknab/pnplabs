...
The canonical report downloads are now a fifty-one-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 10,681 public declarations across 94 modules,
including 5,999 theorem-kind declarations, 3,355 assumption-free theorem-kind declarations, and four
disclosed project axioms. Four thousand one hundred eighteen private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current fifty-one-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1615 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand six hundred fifteen reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Fifty-one narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, traversal through the remaining first-constraint padding, the separator and sign beginning the second scheduled constraint, and all three unary units of its first variable index under external polynomial bounds | The following terminating `F` and rest of that literal, traversal of the second constraint, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `170775d279865370c239919326f1f336cf254b70`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1615 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns fifty-one scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current fifty-one-page report.
