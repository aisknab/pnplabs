...
Lean theorem inventory. The inventory contains 6,459 public declarations across 59 modules,
including 2,883 theorem-kind declarations, 2,495 assumption-free theorem-kind declarations, and four
disclosed project axioms. One thousand and seventy-seven private compiler auxiliaries are excluded explicitly.
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 278 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Two hundred seventy-eight reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Seventeen narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, and the actual encoded-formula size polynomial | A raw formula builder and construction-runtime polynomial, a packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `d335c29ce499acddb16c5fc50afa622a6ea9075f`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 278 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns seventeen scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, and the raw-tape bridge; typed direct-wire semantics; finite reference enumeration/minimum;
...
1. The charged-pipeline model and concrete CNF-SAT language are formalized. Every proof-bearing function or decision program tree recursively compiles into one literal finite machine. The Cook-Levin construction proves exact semantic equivalence between its generated formula and the verifier language and bounds the actual encoded formula by an external input-size polynomial. A raw formula builder, its construction-runtime polynomial, and a packaged polynomial reduction remain absent. CNF-SAT NP-completeness and a deterministic polynomial-time CNF-SAT decider are absent.
