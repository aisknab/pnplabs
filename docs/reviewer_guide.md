...
The canonical report downloads are now a 151-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 32,158 public declarations across 329 modules,
including 16,350 theorem-kind declarations, 7,799 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,311 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 151-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,149 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,149 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 185 of 187 scoped formal milestone rows. For every concrete polynomial-time verifier and every coordinate below its complete direct token schedule, Lean now verifies a fixed 54-rule deterministic work machine that compares that coordinate with the exact problem-derived first-body boundary. Its exact trace accepts precisely the padded-header branch and rejects equality and post-header coordinates. The semantic route agrees definitionally with formulaTokenSlotDirect, the compiled execution is bounded by the verifier-derived polynomial 36 * terminalSlotPolynomial^2, and one-step-short fuel or malformed symbols remains timeout. | Treating the header router as post-header decoding, body-token emission, the complete formula builder or reduction; treating supplied terminal, source, restoration, ambient, BCEL, Packet/HB, rank, claim, dependency, route-clear, or selector-silence data as terminal-derived; treating a local obstruction as a verified global gain or descent; treating conditional ZeroSlack as unconditional; or treating the row as executable PCCMin, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `8529753cac12243c0a618facfca31e2287368469`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,149 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns 185 scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, all four complete fixed clauses, traversal through the remaining first-constraint padding, the complete first literal of its second scheduled constraint, its width-selected successor token, its first six width-selected opportunities, the seventh padding-or-unary opportunity under an external polynomial bound, M208's separate uniform polynomial raw controller that consumes the complete schedule count and reaches its terminal coordinate without decoding or emitting the traversed body tokens, and M209's fixed 54-rule arbitrary-slot router that classifies only the top-level header boundary without decoding or emission; typed direct-wire semantics; finite reference enumeration/minimum;
...
the manuscript at the pinned 7072f8d source tag, never to the current 151-page report.
