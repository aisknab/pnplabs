...
The canonical report downloads are now a twenty-six-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 8,006 public declarations across 72 modules,
including 3,903 theorem-kind declarations, 2,921 assumption-free theorem-kind declarations, and four
disclosed project axioms. Two thousand four hundred forty-four private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current twenty-six-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 633 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Six hundred thirty-three reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Twenty-nine narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, the complete first-clause prefix, and the exact remaining first-clause padding run through the second-clause separator boundary under an external polynomial bound | A general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `bb51e4a9a502f41c360c995946025ef09e9afa97`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 633 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns twenty-nine scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current twenty-six-page report.
