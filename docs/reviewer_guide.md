...
The canonical report downloads are now a 164-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 33,570 public declarations across 345 modules,
including 17,268 theorem-kind declarations, 8,105 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,896 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 164-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,165 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,165 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 201 of 203 scoped formal milestone rows. For every concrete verifier problem and every coordinate in its complete clause-token body rectangle, M225 runs the complete 711-rule classifier while preserving an explicitly staged canonical optional-token request, relays that request through one fixed 14-rule scanner, and executes M223's reflected 64-rule dispatcher. One collision-free 807-rule composition covers populated and padding coordinates and reaches each exact next canonical emitted prefix, with exact work, six-for-one compiled execution, one-step-short nonhalting and one source-input-size polynomial bound. All 80 public declarations are axiom-audited: 32 have empty closure, nine use only propext, and 39 use only propext and Quot.sound, with no project axiom or Classical.choice. | Treating M225's explicitly staged body/padding request as raw request synthesis, a combined body-and-Finish machine, successive physical schedule composition, one repeated raw-machine loop, complete raw builder/refinement, or the packaged reduction; treating supplied terminal, source, restoration, ambient, BCEL, Packet/HB, rank, claim, dependency, route-clear, or selector-silence data as terminal-derived; treating a local obstruction as a verified global gain or descent; treating conditional ZeroSlack as unconditional; or treating the row as executable PCCMin, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `8a9c372156eea1b0f8bc47bd0c7b139b3a2f17b3`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,165 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns 198 scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, all four complete fixed clauses, traversal through the remaining first-constraint padding, the complete first literal of its second scheduled constraint, its width-selected successor token, its first six width-selected opportunities, the seventh padding-or-unary opportunity under an external polynomial bound, M208's separate uniform polynomial raw controller that consumes the complete schedule count and reaches its terminal coordinate without decoding or emitting the traversed body tokens, M209's fixed 54-rule arbitrary-slot router that classifies only the top-level header boundary, M210's structurally recursive all-coordinate semantic post-header decoder with exact shifted-remainder extraction, M211's fixed 99-rule raw unary quotient/remainder divider, M212's all-coordinate checked-reader/divider orchestration, M213's fixed 351-rule literal workspace-preserving tape bridge, M214's fixed 180-rule post-divider route classifier, M215's all-coordinate selected-token launch, M216's complete semantic schedule iteration, M217's fixed 64-rule physical optional-token dispatcher, M218's all-coordinate physical dispatch schedule, M219's unique physical Finish-request handoff, M220's complete suffix-preserving physical classifier pipeline, M221's exact full-classifier Finish-request cell, M222's full-classifier Finish-workspace orientation, and M223's reflected unique Finish dispatch to the complete CNF appender endpoint, still without body-token or padding request derivation, connection of every classifier outcome, successive schedule configurations, or one literal raw loop; typed direct-wire semantics; finite reference enumeration/minimum;
...
exact all-cut activation conservation in the singleton branch. M202 feeds that derived ledger into the checked
sparse BCEL/Packet classifier and reflects its first singleton/pair activation mismatch back to the supplied
source ledger. M203 then computes multiplicity-preserving extraction of the generated cancellation cells from an arbitrary supplied ambient BN4 ledger, giving the exact remainder and residual-ledger reduction or an exact no-embedding proof. M204 removes the typed-restorer premise at one local boundary and classifies a supplied restoration-coordinate universe as a proof-bearing Hall route or complete coverage with a constructed balanced BN4 cancellation subledger and computed ambient remainder. M205 lifts that classifier over an arbitrary finite supplied active source ledger, preserving the first exact obstruction or constructing the complete source-ordered BN6 positive-cell ledger. M206 binds that ledger to one supplied computed BCEL nucleus and enters the checked sparse BN6/BCEL route only after every source singletonizes. M207 proves that the successful exact ambient-embedding branch strictly decreases the chargeSize coordinate of the exact TerminalResidualRank while preserving the complete residual ledger. M208 separately adds a uniform polynomial raw controller for the complete Cook-Levin body schedule without body-token emission. M209 adds a fixed uniform arbitrary-coordinate header router. M210 adds unbounded semantic post-header decoding and exact remainder extraction. M211 adds exact raw unary quotient/remainder computation with a quadratic encoded-size bound, M212 composes it with M209's checked raw result for every coordinate under one polynomial bound, M213 replaces that value-level handoff with a fixed literal workspace-preserving tape bridge, and M214 classifies the physical divider endpoint as an M210 body or Finish route while preserving the exterior and remainder ledger. The classifier still emits no token and does not iterate the complete schedule. The route does not derive the ambient ledger, source systems, cuts, payloads, restorer, downstream tables, HB
evidence, route-clear result, or selector silence from terminal input, turn either diagnostic into a verified
global gain, or map the M200 mismatch into a
...
3. The literal target emitter and strict parser/emitter composition are formalized. The standalone
   ...
   case, derives the raw BN6 positive cells and exact all-cut activation ledger. M202 feeds that ledger into the
   checked sparse BCEL/Packet classifier, retaining cancellation, reflecting a singleton/pair mismatch back to
   the source ledger, or reaching conditional ZeroSlack under supplied checked selector silence. M203 computes the exact multiplicity-preserving extraction of those generated cancellation cells from a supplied ambient BN4 ledger, with a computed remainder and residual reduction or an exact no-embedding proof. M204 classifies supplied exact restoration-coordinate coverage and constructs the balanced cancellation subledger and computed ambient remainder in its coverage branch. M205 scans the full supplied source ledger and reaches BN6 cellization only when every source singletonizes. M206 then carries that exact branch into the supplied checked BN6/BCEL Packet/HB classifier, preserving conditional ZeroSlack or reflecting the first exact small-cut mismatch to the source ledger. M207 proves exact charge-coordinate residual-rank descent for the successful ambient-embedding outcome. M208 separately adds a uniform polynomial raw controller for the complete Cook-Levin body schedule without body-token emission. M209 adds a fixed uniform arbitrary-coordinate header router. M210 adds unbounded semantic post-header decoding and exact remainder extraction. M211 adds exact raw unary quotient/remainder computation with a quadratic encoded-size bound, M212 composes it with M209's checked raw result for every coordinate under one polynomial bound, M213 replaces that value-level handoff with a fixed literal workspace-preserving tape bridge, and M214 classifies the physical divider endpoint as an M210 body or Finish route while preserving the exterior and remainder ledger. The classifier still emits no token and does not iterate the complete schedule. The terminal problem, ambient ledger, source
...
the manuscript at the pinned 7072f8d source tag, never to the current 164-page report.
