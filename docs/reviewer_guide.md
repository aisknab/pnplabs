...
The canonical report downloads are now a 119-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 29,341 public declarations across 283 modules,
including 15,202 theorem-kind declarations, 7,526 assumption-free theorem-kind declarations, and four
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 119-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 2,985 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 2,985 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 142 of 144 scoped formal milestone rows. The newest checked binding maps any frontier, obligation, activation, or direction mismatch to HN activity at the table-owned handle rank; the checked well-founded HB no-outcome closure forces all four fields to agree, and the separate budget/HB binding plus executable selector silence leave only exact residual nondecrease. | Treating the grouped family, typed coordinates, activation atoms, direction or budget values, bindings, rank map, activity environment, dependency table, closure premise, candidates, blueprints, occurrence pairings, unmatched lists, or upstream data as terminal-derived; presenting the result as HN blocker semantics, semantic dependency completeness, complete Packet adequacy, unconditional HB negative closure, a decreasing transition, no-lower contradiction, or complete global routing; claiming positive slack, unconditional ZeroSlack, a circuit-size bound, polynomial enumeration, complete PkgC, polynomial PCCMin, target decider, assumption discharge, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `fe6c20fe6929c9e57c5ab7c483f7c56bfa9e9832`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 2,985 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns one hundred and forty-one scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current 119-page report.
