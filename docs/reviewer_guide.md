...
The canonical report downloads are now a forty-three-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 9,906 public declarations across 87 modules,
including 5,367 theorem-kind declarations, 3,252 assumption-free theorem-kind declarations, and four
disclosed project axioms. Three thousand six hundred thirty private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current forty-three-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1371 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand three hundred seventy-one reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Forty-four narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, and exact clause-two, clause-three, and clause-four padding traversal under external polynomial bounds | Traversal of the intentionally empty fifth clause rectangle, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `5377b99658a756f60a8b36d19896be579761d8cd`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1371 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns forty-four scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current forty-three-page report.
