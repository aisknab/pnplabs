...
The canonical report downloads are now a 139-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 30,900 public declarations across 314 modules,
including 15,787 theorem-kind declarations, 7,690 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,163 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 139-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,137 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,137 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 173 of 175 scoped formal milestone rows. The newest row normalizes supplied raw supports into the computed carrier, constructs singleton consumer systems, coalesces duplicate footprints, preserves positive payload atoms, and reuses M196 to leave either an exact activation mismatch or conditional ZeroSlack. | Treating the supplied raw cells, certificate, table, rank, route-clear, or terminal data, exhaustive powerset classifier, remaining mismatch diagnostic, or other supplied stages as complete PkgC/BN3 through BN6 integration, manuscript-wide SaturatePositive, BCELReady, executable PCCMin, unconditional ZeroSlack, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `54aa7d8c2fa0a728e6e0af9349c78f27f510d989`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,137 reviewed milestone declarations at its exact kernel type.
...
3. The literal target emitter and strict parser/emitter composition are formalized. The standalone
   ...
   preserves every positive payload atom, producing M196's grouped-family input canonically. The terminal
   problem, positive premise, checked finite BCEL-ready certificate, raw positive supports and payloads, tables,
   ranks, route-clear proof, normalizer, resolvers, blocker semantics, encoded-size bounds, and exhaustive
   powerset classifier remain supplied. The remaining activation mismatch is not a constructed gain or
...
the manuscript at the pinned 7072f8d source tag, never to the current 139-page report.
