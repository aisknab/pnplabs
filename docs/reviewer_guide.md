...
The canonical report downloads are now a thirty-six-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 9,117 public declarations across 81 modules,
including 4,764 theorem-kind declarations, 3,122 assumption-free theorem-kind declarations, and four
disclosed project axioms. Three thousand one hundred fifty-one private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current thirty-six-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1089 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand eighty-nine reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Thirty-eight narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, all three complete fixed clauses, and clause-two padding traversal under external polynomial bounds | Clause-three padding traversal, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `ed439e1477e69315006fe7f8bbecac278c024bbf`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1089 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns thirty-eight scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current thirty-six-page report.
