...
Lean theorem inventory. The inventory contains 5,197 public declarations across 48 modules,
including 2,197 theorem-kind declarations, 2,096 assumption-free theorem-kind declarations, and four
disclosed project axioms. One thousand and thirty-two private compiler auxiliaries are excluded explicitly.
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 157 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One hundred fifty-seven reviewed theorem-type fingerprints, empty axiom closures, and the complete Lean-source digest | Nine narrowly scoped formal milestones, including `CNFSAT ∈ NP`, the local all-input framer, and the four-stage compiler's exact canonical-pair verdict, output, no-timeout, and external polynomial bounds | Full-pipeline malformed-input behavior, uniform all-input raw refinement, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `fb9674949679a137b7ce790c1522efaecbe240ce`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 157 reviewed milestone declarations at its exact kernel type.
...
including a finite accepting framer for every raw bitstring, collision-free state namespaces, and one full four-stage raw compiler for proof-bearing
polynomial-time targets on canonical paired inputs; charged-pipeline P/NP/reduction definitions; universal concrete CNF-SAT verifier correctness,
...
1. The charged-pipeline model and concrete CNF-SAT language are formalized. A literal finite framer accepts every raw bitstring—including empty and odd-length inputs—within `6 * m * m + 39 * m + 75` raw steps. For every proof-bearing polynomial-time target machine and canonical `BitString.pair`, the ordinary paired-input execution runs through framing, simulation, represented handoff, terminal launch, and raw-output packing in one finite machine with exact verdict and `machineOutput`, no timeout, and explicit external bounds `B(m)` and `R(m)`. No theorem carries arbitrary non-pair inputs from the local framer endpoint through the remaining stages; full-pipeline malformed-input behavior, uniform all-input raw refinement, the concrete complexity machine link, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider are absent.
