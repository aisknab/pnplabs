...
The canonical report downloads are now a twenty-one-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 7,626 public declarations across 69 modules,
including 3,640 theorem-kind declarations, 2,827 assumption-free theorem-kind declarations, and four
disclosed project axioms. Two thousand two hundred fourteen private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current twenty-one-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 508 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | Five hundred eight reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Twenty-six narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, the complete width header, and the `T^FormulaWidth F Sep T F` first-literal prefix under an external polynomial bound | The dynamic cursor controller, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `977748fa95202c86cf1235e7336e76d908a11eb1`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 508 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns twenty-six scoped milestones: the concrete bitstring/machine/cost kernel,
...
no-timeout and NP membership; Cook-Levin layout, tableau, CNF compilation, finite semantics, the raw-tape bridge, encoded-size bound, exact rectangular formula schedule, direct coordinate cursor with exact fuelled traversal, the complete width header, and a literal `T^FormulaWidth F Sep T F` first-literal composition with a retained next coordinate under an external polynomial bound; typed direct-wire semantics; finite reference enumeration/minimum;
...
the manuscript at the pinned 7072f8d source tag, never to the current twenty-one-page report.
