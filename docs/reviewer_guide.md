...
The canonical report downloads are now a 140-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 30,909 public declarations across 316 modules,
including 15,792 theorem-kind declarations, 7,690 assumption-free theorem-kind declarations, and no
project-specific axioms. Exactly 15,171 private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current 140-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 3,138 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | 3,138 reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | 174 of 176 scoped formal milestone rows. The newest row proves that M197's canonical duplicate-footprint coalescing preserves the exact direct raw crossing-mass sum on every cut and exposes the remaining activation mismatch directly against the supplied raw ledger. | Treating the supplied raw cells, certificate, table, rank, route-clear, or terminal data, exhaustive powerset classifier, raw-ledger mismatch diagnostic, or other supplied stages as complete PkgC/BN3 through BN6 integration, manuscript-wide SaturatePositive, BCELReady, executable PCCMin, unconditional ZeroSlack, polynomial runtime, a target decider, SAT hardness transport, a root theorem, or P = NP |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `ebebf9356116e3b8dca9fbae6999826c026fa18b`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 3,138 reviewed milestone declarations at its exact kernel type.
...
payload atom. M198 proves that this coalescing preserves the exact direct raw crossing-mass sum on every cut, so
the remaining activation mismatch is exposed directly against the raw ledger. This diagnoses rather than repairs
the missing numerical bridge. It does not derive the raw cells or payloads from terminal input, turn the mismatch
into a constructed gain, derive constant activation, establish manuscript-wide SaturatePositive or BCELReady,
complete the global route system, prove unconditional ZeroSlack, or prove polynomial PCCMin.
...
the manuscript at the pinned 7072f8d source tag, never to the current 140-page report.
