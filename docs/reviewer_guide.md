...
The canonical report downloads are now a fifty-page, non-claiming report generated from a compiled
Lean theorem inventory. The inventory contains 10,587 public declarations across 93 modules,
including 5,920 theorem-kind declarations, 3,344 assumption-free theorem-kind declarations, and four
disclosed project axioms. Four thousand forty-three private compiler auxiliaries are excluded explicitly.
...
- [`downloads/canonical_proof_report.pdf`](../downloads/canonical_proof_report.pdf): current fifty-page report;
...
| --- | --- | --- | --- |
| Compiled Lean inventory | Environment constants and `collectAxioms`, exported under the pinned Lean toolchain | Names, modules, kinds, and axiom dependencies for all public declarations; raw kernel types for the 1581 reviewed milestone candidates | A theorem broader than a reviewed candidate's exact type |
| Earned milestones | One thousand five hundred eighty-one reviewed theorem-type fingerprints, permitted axiom closures, and the complete Lean-source digest | Fifty narrowly scoped formal milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, encoded-formula bounds and schedules, a direct coordinate cursor, four complete fixed clauses, traversal through the remaining first-constraint padding, the separator and sign beginning the second scheduled constraint, and the first two unary units of its first variable index under external polynomial bounds | The following third unary `T` and rest of that literal, traversal of the second constraint, a general dynamic formula cursor, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged reduction, CNF-SAT in P, NP-completeness, global locked-NAND construction, complete residual search, or `P = NP` |
...
1. Reproduce the pinned Lean build in `aisknab/pnp` at merged commit
   `2079ea0df337d413c2402a3820087aea4aca9efa`.
2. Re-export the inventory and compare it byte-for-byte with
   `public/pnp-theorem-inventory.json`.
3. Inspect every one of the 1581 reviewed milestone declarations at its exact kernel type.
...
The formal inventory earns fifty scoped milestones: the concrete bitstring/machine/cost kernel,
...
the manuscript at the pinned 7072f8d source tag, never to the current fifty-page report.
