...
for dependencies. Every public row records name, module, kind, and axiom closure; the 108 reviewed
milestone candidates additionally record raw kernel types for publication fingerprinting. The
canonical output records:

- 5,023 public declarations;
- 2,081 theorem-kind declarations;
- 1,980 assumption-free theorem-kind declarations;
- 45 source-closure modules;
- 950 excluded private compiler auxiliaries;
- four project axioms.
...
| --- | --- | --- |
| Concrete machine and cost kernel | Executable bitstrings/codecs, finite rule-list machines, collision-free pipeline namespaces, exact stage launches, accept/reject/stuck trace transport through the internal represented-output handoff, and a separate terminal raw-output packer with ordinary `machineOutput` equality and compiled bound `18*m^2 + 36*m + 6` | Requires a supplied exact target run; the packer is not launched from the bridge endpoint or incorporated into its rule table, and there is no target termination, complete raw refinement, external input-size polynomial, CNF-SAT in P, NP-completeness, or `P = NP` |
