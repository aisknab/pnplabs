...
| Milestone map | Reviewed theorem-name/type pins and fixed source digest | Presence, theorem kind, type match, exact approved axiom closure with no project axiom, source match | Seventy earned scopes only, including the locked-NAND semantic threshold, strict-v0 encoding, the literal polynomial-time parser and emitter, their concrete encoded-language polynomial reduction, and a general semantic CNF-to-NAND compiler; no finite-machine reduction or NP-hardness transport for that compiler, threshold discharge, or final conclusion |
