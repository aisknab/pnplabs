...
| Complexity | Concrete charged-pipeline P/NP/reduction definitions, `CNFSAT ∈ NP`, collision-free pipeline namespaces, and bounded internal stage bridges for supplied exact target traces | Target termination, terminal raw-output packing, ordinary `machineOutput` equality, complete raw refinement, an external input-size polynomial, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider |
