...
| Complexity | Concrete charged-pipeline P/NP/reduction definitions, `CNFSAT ∈ NP`, collision-free pipeline namespaces, and the complete four-stage terminal trace for a caller-supplied exact target execution, with exact verdict and raw `machineOutput` | Target termination, uniform all-input raw refinement, an external input-size polynomial, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider |
