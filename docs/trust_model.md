...
| Milestone map | Reviewed theorem-name/type pins and fixed source digest | Presence, theorem kind, type match, exact approved axiom closure with no project axiom, source match | Sixty-three earned scopes only, including global baseline distinctness and exact reference minimum; no final conclusion |
...
| Locked-NAND | Typed local candidates, five local minima, global carrier/trace equivalence, exact `B/B` and `B+4/B+1` candidate assembly, global `BaselineDistinct`, exact exhaustive reference minimum `B`, conditional six-premise boundary | The two whole-carrier final-output laws, uniform builder, threshold, and remaining premise instantiation |
