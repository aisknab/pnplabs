...
| Complexity | Concrete charged-pipeline P/NP/reduction definitions, `CNFSAT ∈ NP`, collision-free pipeline namespaces, and a complete four-stage raw compiler with exact all-input verdict, output, no-timeout, and external polynomial bounds for every proof-bearing polynomial-time target | Recursive function/decision `RawRefinement`, the concrete complexity machine link, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider |
