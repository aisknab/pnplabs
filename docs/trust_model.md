...
| Milestone map | Reviewed theorem-name/type pins and fixed source digest | Presence, theorem kind, type match, exact approved axiom closure with no project axiom, source match | Nineteen scopes only; no global conclusion |
...
| Complexity | Concrete charged-pipeline P/NP/reduction definitions, `CNFSAT ∈ NP`, all-input and sequential raw compilers, and recursive function/decision refinement into one literal raw machine with exact output/verdict, no-timeout, and external polynomial bounds | CNF-SAT NP-completeness and a deterministic polynomial-time CNF-SAT decider |
