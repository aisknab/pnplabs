...
| Complexity | Concrete charged-pipeline P/NP/reduction definitions, `CNFSAT ∈ NP`, collision-free pipeline namespaces, and a complete four-stage raw compiler with exact canonical-pair verdict, output, no-timeout, and external polynomial bounds for every proof-bearing polynomial-time target | Malformed-input behavior, uniform all-input raw refinement, the concrete complexity machine link, CNF-SAT NP-completeness, and a deterministic polynomial-time CNF-SAT decider |
