...
| --- | --- | --- | --- | --- |
| `npm run verify:seal` | [downloads/release-seal.json](../downloads/release-seal.json), [downloads/SHA256SUMS](../downloads/SHA256SUMS), bundled report files | File byte counts, SHA-256 digests, and manifest/ledger agreement for public report artefacts. | Theorem correctness, PDF truth, source/checker execution, checker soundness, or semantic equivalence of digest-labelled objects. | Missing file, changed bytes, digest mismatch, malformed ledger, or manifest status no longer saying file identity only. |
