...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/52b77de8a79e41527b4fbab788d433ec031da459/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/52b77de8a79e41527b4fbab788d433ec031da459/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 111-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,863 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 136 of 138 scoped milestone rows are earned. The newest scope is canonical Packet typed-frontier route reflection with 23 reviewed theorem pins. Its focused 30-declaration audit has two empty, 17 `propext`-only, and 11 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For explicit typed source and selector signatures, the canonical payload computes frontier acceptance from exact equality while retaining grouped colour, positive charge, the internal source route, authoritative rank, and exact RankWF descent computation. A frontier first route is exactly signature inequality, equal signatures exclude it, the first route can return none of colour, charge, exactRoute, or rank, a final descent route carries actual nondecrease, and the positive Packet/HB endpoint carries exact failure evidence without route-clear or binding premises. The signatures, finite rank map, before/after ranks, obligation, activation, direction, budget, grouped family, exhaustive realizer claims, activity bits, and dependency rows remain proof-bearing inputs. The signatures must not be described as constructed from terminal data, bound to a BN5 coordinate, or as a proof of full manuscript frontier faithfulness. External semantics for the four remaining routes, decreasing complete global coverage, a guaranteed decreasing transition, a no-lower ledger, positive slack, blocker semantics, semantic dependency completeness, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A signature, rank map, payload field, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; supplied signatures are presented as BN5-bound or as full frontier-faithful comparison; route exclusion is presented as proof that a decreasing transition exists or that the remaining routes have external semantics and decreasing globally complete coverage; unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack` is claimed; a no-lower ledger, circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 111 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
