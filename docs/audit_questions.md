...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/9a01a9aee903b0d76132e80d40eed9004dc4eae1/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/9a01a9aee903b0d76132e80d40eed9004dc4eae1/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the seventy-three-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,166 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm seventy-nine earned scoped milestones. The newest scope gives the finite saturation relation a deterministic work-list implementation and computes exactly every incoming boundary and outgoing interface wire for selected nodes in a finite direct-wire circuit. The dependency system remains explicit data and it does not construct or certify a proper governed support square. Three global milestones, four project axioms, and six blockers remain. Proper or governed support construction, support completion in the manuscript's full sense, square legitimacy, BCEL/BN2–BN6, route completeness, `ZeroSlack`, PCCMin exactness and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | Finite executable saturation or crossing-wire completion is presented as proper-support or square construction, a polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require seventy-three PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
