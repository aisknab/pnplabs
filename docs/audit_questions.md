...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/f3ca3346b3cd2b33e1259321297b6149ac5c52db/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/f3ca3346b3cd2b33e1259321297b6149ac5c52db/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 100-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,723 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 125 of 127 scoped milestone rows are earned. The newest scope is a conditional HB selector-silence rank closure with four reviewed theorem pins and a four-declaration focused audit: all four use `Quot.sound` with `propext`; none uses only `propext`, a project axiom, or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The grouped family, selector table, finite ranks, faithfulness predicate, realizer claims, activity bits, dependency rows, rank map, semantic gain exclusion, and coverage certificate must remain proof-bearing inputs. Blocker semantics, semantic dependency completeness, construction from terminal data, selector faithfulness and compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A table, rank map, faithfulness predicate, realizer claim, family, certificate, or semantic premise is hidden or presented as derived; conditional selector silence is presented as selector faithfulness, compatibility, unconditional HB negative closure, unconditional global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 100 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
