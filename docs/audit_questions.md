...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/37bbbd1d978df770087a0cd675b90eba2cfc50ad/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/37bbbd1d978df770087a0cd675b90eba2cfc50ad/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 133-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,128 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 164 of 166 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is typed PCCPack reflection with one reviewed completion-theorem pin and no project axiom at its checked endpoint. It replaces the final two opaque declarations and caller-supplied reflection field with transparent typed generation and structural checking over an explicit loop certificate. Exactly the named `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints become earned; no project axioms remain, while all five blockers remain. A constructed PCCMin certificate, unconditional ZeroSlack, polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied-certificate reflection is presented as a PCCMin construction; row coverage is presented as proof completion; any checkpoint beyond the two recorded M188 axiom removals is awarded; a remaining blocker is hidden; or a final theorem appears. |
