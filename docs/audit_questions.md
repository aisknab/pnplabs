...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/d3794beaf85242ea6d5c4f141f74e52876261c4f/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/d3794beaf85242ea6d5c4f141f74e52876261c4f/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-eight-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,712 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twenty-three earned scoped milestones. The newest scope is a total HN/BUD dependency table with eleven reviewed theorem pins and a 21-declaration focused audit: nine have empty closures and 12 use `Quot.sound` with `propext`; none uses only `propext`, a project axiom, or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The table, rank mapping, selector family, faithfulness predicate, realizer claims, activity tables, grouped family, and blueprints must remain proof-bearing inputs. Blocker semantics, semantic dependency completeness, the local invariant needed to silence active rows, construction from terminal data, selector compatibility, rank-complete selector silence, the full HB negative closure, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A table, rank mapping, faithfulness, claim, activity table, blueprint, family, or semantic premise is hidden or presented as derived; total row coverage is presented as blocker semantics or semantic dependency completeness, the full HB negative closure, global selector silence, or unconditional `ZeroSlack`; the local invariant premise, a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 98 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
