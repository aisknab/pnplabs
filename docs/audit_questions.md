...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/e0d08b9d0f294b64d41035125ce0dab1ee6188d8/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/e0d08b9d0f294b64d41035125ce0dab1ee6188d8/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 143-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,141 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 177 of 179 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope has one reviewed completion-theorem pin whose closure uses only Lean-standard `Quot.sound` and `propext`. It classifies every finite supplied active V54 consumer family into the first exact same-key PkgC cancellation or an all-singletonized branch that derives raw BN6 positive cells, support-size facts, payload order, and exact all-cut activation conservation. The terminal problem, source systems, active cuts, positive payloads, typed restorer, upstream BN3 through BN5 construction, checked BCEL/Packet integration, and complete encoded-size bounds remain supplied or open. A cancellation is not a verified global gain or rank decrease, and no fixed checkpoint changes. Executable terminal-derived stages, complete PkgC through BN6 integration, manuscript-wide SaturatePositive and BCELReady, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied stages or data are presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M201; supplied source systems are described as terminal-derived; local cancellation is presented as a verified global gain or rank decrease; a remaining blocker is hidden; or a final theorem appears. |
