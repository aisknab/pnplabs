...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/ca501ca3df44c8d19933e59fe132612bea5422c2/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/ca501ca3df44c8d19933e59fe132612bea5422c2/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 126-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,064 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 152 of 154 scoped milestone rows are earned. The newest scope is the terminal HResolve certified-path family with seven reviewed theorem pins. One pin uses only `propext` and six use only `Quot.sound` plus `propext`; the focused 13-declaration audit has six empty, one `propext`-only, and six `Quot.sound` plus `propext` closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every duplicate-free finite supplied family of proof-bearing hereditary candidates, the constructor selects a maximal pairwise H-disjoint family, preserves exact certified-path minima for selected candidates, and records a blocker route for each rejection. The candidates, paths, footprints, governed predicates, completeness proofs, and coherence proofs remain supplied. This must not be described as terminal derivation, HN grammar soundness or completeness, LN confluence, ParseOrExit, independent leaf tightness, the full NoHereditary sidecar, blocker-to-HB-rank semantics, full or polynomial HResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A maximal family over supplied candidates is presented as terminal derivation, the full NoHereditary sidecar, full HResolve, or a polynomial algorithm; the supplied candidates, paths, footprints, completeness or coherence proofs, or missing grammar, routing, runtime, or global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 126 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
