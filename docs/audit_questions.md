...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/8b074cc2e8f840009a4ee064dd9dc6299c278004/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/8b074cc2e8f840009a4ee064dd9dc6299c278004/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,993 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 144 of 146 scoped milestone rows are earned. The newest scope is one checked Packet no-lower ledger with four reviewed theorem pins. Its focused six-declaration audit has zero empty, three `propext`-only, and three `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The ledger recomputes five exact Packet checks; a positive Packet forces rejection, and accepted ledger data excludes a positive Packet conclusion. The grouped family, coordinates, typed values, ranks, realizer claims, activity environment, dependency rows, and terminal data remain proof-bearing inputs. This closes the Packet branch only. It must not be described as the complete no-lower ledger, HResolve, BudgetResolve, normalization, other named obstruction routing, saturation, replay, unconditional HB closure, global ZeroSlack, or complete global routing. Positive slack, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; the closed Packet branch is presented as the complete no-lower ledger or global route system; positive slack is claimed; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
