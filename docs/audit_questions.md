...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/8bc8d36fea5abf3f486e9dbfe7cfccea5c6b34e0/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/8bc8d36fea5abf3f486e9dbfe7cfccea5c6b34e0/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,749 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 128 of 130 scoped milestone rows are earned. The newest scope is canonical Packet faithfulness-table construction with eight reviewed theorem pins. Its focused 9-declaration audit has one empty, five `propext`-only, and three `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The constructor computes the HB faithfulness function from canonical positive source payloads and removes the separate binding premise, while the grouped family, ten payload fields, finite rank tags and assignment, route-clear checks, exhaustive realizer claims, activity bits, dependency rows, and rank map remain proof-bearing inputs. Positive slack, blocker semantics, semantic dependency completeness, construction from terminal data, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A rank map, payload field, route-clear check, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; a canonical computed table and conditional contradiction is presented as unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
