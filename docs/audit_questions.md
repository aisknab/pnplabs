...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/f6f78026f9ff3be2f45b8fce859d2c09b6a8d764/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/f6f78026f9ff3be2f45b8fce859d2c09b6a8d764/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 130-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,099 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 158 of 160 scoped milestone rows are earned. The newest scope is the same-family Selector/HB, Packet, and BCEL ZeroSlack coherence sidecar with seven reviewed theorem pins. The focused eleven-declaration audit has one `propext`-only definition and ten `Quot.sound` plus `propext` theorem closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The newest row derives Selector/HB evidence from the exact grouped family, computed realizer table, and dependency table in one supplied accepted M180 certificate, then reuses that certificate for the Packet and dependent M181 BCEL exclusions. Exact identities prevent a detached certificate pairing. The grouped family and all terminal, budget, Packet, realizer, dependency, rank, and BCEL inputs remain supplied. This must not be described as deriving those inputs or constant activation from positive residual slack, completing the manuscript no-lower ledger, establishing unconditional global ZeroSlack, or completing global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked same-family coherence result over supplied data is presented as derived BCELReady or constant activation, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
