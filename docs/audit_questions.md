...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/f149a972c366b96a3db704aaa1a23679abc21b6a/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/f149a972c366b96a3db704aaa1a23679abc21b6a/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-four-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,668 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and eighteen earned scoped milestones. The newest scope is conditional Packet selector gain-or-ZeroSlack coverage with 7 reviewed theorem pins and a 14-declaration focused audit: 7 use only `propext`, and 7 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The gain-coverage certificate, candidate implementations, and grouped BN6 family must remain proof-bearing inputs. The result must be presented as conditional rather than unconditional `ZeroSlack`. Certificate construction from terminal data, selector faithfulness or compatibility, replacement-candidate construction or polynomial enumeration, encoded-size and runtime bounds, typed blockers or HB/rank closure, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit coverage premise is hidden or presented as derived; the conditional result is presented as unconditional `ZeroSlack`; a complete manuscript gain-or-blocker realizer, circuit-size bound, polynomial generation, typed blocker or rank closure, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
