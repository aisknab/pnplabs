...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/7dc26600f55762bf7a65b3fc6512a76aba23d9d0/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/7dc26600f55762bf7a65b3fc6512a76aba23d9d0/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,127 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 163 of 165 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports 33%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is concrete residual-band compatibility with seven reviewed theorem pins and no project axiom at its checked endpoint. It replaces a caller-supplied edge with a compiled identity reduction and removes `PNP.ResidualBandExactMinimization` from the active project-axiom inventory. Exactly the named `axiom-remove-residual-band-minimum` checkpoint becomes earned; two project axioms and five blockers remain. Polynomial residual minimisation, residual-band promise bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, unconditional ZeroSlack, exact polynomial PCCMin, the root theorem, and the publication gate must remain absent. | Exact exhaustive semantics are presented as a polynomial algorithm; row coverage is presented as proof completion; any checkpoint beyond the recorded M187 change is awarded; a remaining blocker or axiom is hidden; or a final theorem appears. |
