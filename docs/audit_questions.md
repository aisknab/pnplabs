...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/6e7124f7edf19b2ab077b9b52ed267cb6ecb00e3/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/6e7124f7edf19b2ab077b9b52ed267cb6ecb00e3/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,129 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 165 of 167 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the proof-bearing PCCMin total-oracle loop with one axiom-free reviewed completion-theorem pin. It proves terminating strict-gain recursion, exact-minimum output, zero residual slack, and an iteration bound only under an explicit total oracle. The oracle remains supplied and the regression fixture uses exhaustive reference minimisation; no fixed weighted checkpoint changes. An executable terminal-derived oracle, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | The supplied oracle is presented as a PCCMin construction or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M189; a remaining blocker is hidden; or a final theorem appears. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 133 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
