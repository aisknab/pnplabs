...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/783941d428aa43cc2fb6763ed5f58fd9dab9ec44/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/783941d428aa43cc2fb6763ed5f58fd9dab9ec44/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the thirty-seven-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1126 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm thirty-nine earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, all three complete fixed clauses, and the exact remaining clause-two and clause-three padding runs. Confirm that the latest machine traverses `FormulaTokensPerClause - 8` clause-three padding coordinates without emission, retains the fourth-clause coordinate with direct token `Sep`, and preserves `encodedFormula.take (2 * (FormulaWidth + 27))`. Confirm three unearned global milestones, four project axioms, and six blockers, and that emission of the fourth-clause separator, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The padding traversal is widened into separator emission, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require thirty-seven PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
