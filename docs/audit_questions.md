...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/359dcb1bf645ef15dddbfb42cd1ec789e4f5a5b7/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/359dcb1bf645ef15dddbfb42cd1ec789e4f5a5b7/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the sixteen-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 340 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm twenty-two earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor with exact fuelled traversal, the fixed 19-rule input-length tally machine, the literal framer-launch-tally prefix, and the standalone exact token appender whose first-header specialization emits two formula bits within `24*n + 48` raw steps, plus three unearned global milestones, four project axioms, and six blockers. Confirm that prefix-to-appender composition, a complete raw formula builder, `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | A specification cursor, schedule, tally, prefix, standalone appender, or output-size theorem is widened into a complete raw builder, construction-runtime, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require sixteen PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
