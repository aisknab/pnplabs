...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/74d7f531bebe302cba08ad992fd4d48d46a8b29e/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/74d7f531bebe302cba08ad992fd4d48d46a8b29e/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,103 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 159 of 161 scoped milestone rows are earned. The newest scope is the finite SaturatePositive-to-BCEL-ready composition with four reviewed theorem pins. All six declarations in the focused audit use only `Quot.sound` and `propext`; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The newest row reruns the finite classifier for an explicit direct-wire candidate, terminal model, candidate-derived anchor problem, and positive full-slack premise, then retains its positive-projection branch with a computed BCEL-ready anchor nucleus and checked proper-cut conclusions. This must not be described as deriving positivity from residual slack, constructing BN3 through BN6 data or a grouped family, deriving constant activation, establishing manuscript-wide `SaturatePositive` or BCELReady, completing global routing, or establishing unconditional ZeroSlack. Encoded-size and runtime bounds, complete PkgC, Package E, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked finite composition over supplied data is presented as manuscript-wide SaturatePositive or BCELReady, derived positivity or constant activation, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
