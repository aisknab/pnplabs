...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/1061db268348734ecbf26306a76ef1cfb609672f/status/FORMAL_RECONSTRUCTION_STATUS.json)
...
| Milestone earning | Check all 3,108 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 160 of 162 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports its risk-weighted estimate and five open global gates. The newest scope is same-candidate finite BCEL-ready and Packet carrier coherence with five reviewed theorem pins. All six declarations in the focused audit use only `Quot.sound` and `propext`; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The newest row uses a checked bijection and exact list equality to identify the computed ready nucleus with the exact grouped Packet carrier for the same candidate and model, transferring the carrier-size and existing Packet/constant-activation exclusions to one coherent family. This must not be described as equating activation weights with projection excess, deriving constant activation from positive residual slack, deriving the supplied terminal or Packet data, completing the no-lower ledger or global routing, or establishing unconditional ZeroSlack. Encoded-size and runtime bounds, complete PkgC, Package E, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked carrier identity over supplied data is presented as derived terminal or Packet construction, derived constant activation, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; row coverage is presented as proof completion; local milestones inflate a fixed checkpoint; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
