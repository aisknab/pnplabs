...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/3894e5e2dd3f34c2fe19f6eb0b9e39119ad05403/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/3894e5e2dd3f34c2fe19f6eb0b9e39119ad05403/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the sixty-nine-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,093 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm seventy-two earned scoped milestones. The newest scope covers every finite proof-bearing or executably verified chain of adjacent strict equivalent gains: semantics and the exhaustive reference minimum are invariant, and endpoint slack plus chain length is bounded by starting slack, with a four-step specialization for the complete locked-NAND candidate. Three global milestones, four project axioms, and six blockers remain; gain generation and completeness, justified stopping, `ZeroSlack`, exact minimization, polynomial checker/PCCMin runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | A verified-chain bound is presented as a gain generator, completeness theorem, stopping theorem, minimizer, polynomial runtime theorem, CNF-SAT decider, assumption discharge, or final theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require sixty-nine PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
