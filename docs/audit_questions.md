...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/f3dd861bf522e9fca2a3c1ba4a4c3e78f5454b4a/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/f3dd861bf522e9fca2a3c1ba4a4c3e78f5454b4a/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,032 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 149 of 151 scoped milestone rows are earned. The newest scope is the finite terminal Packet-budget no-lower composition with four reviewed theorem pins. Its focused six-declaration audit has all six declarations using `Quot.sound` plus `propext`; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every finite direct-wire candidate, one Boolean recomputes the complete canonical budget-support ledger and the checked five-row Packet no-lower ledger over that same candidate. Acceptance excludes every feasible strict-equivalent gain and a positive Packet conclusion. The caps and Packet-side data remain supplied. This is a finite two-branch composition and must not be described as manuscript HN/BUD grammar, polynomial HResolve or BudgetResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A finite two-branch composition is presented as the complete no-lower ledger or a polynomial algorithm; supplied caps, Packet data, or missing grammar, blocker, routing, runtime, or global premises are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
