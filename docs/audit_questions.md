...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/b2e0b1e97744e365528b7c1a06e208a6481d7d1a/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/b2e0b1e97744e365528b7c1a06e208a6481d7d1a/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 102-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,741 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 127 of 129 scoped milestone rows are earned. The newest scope is Packet selector-faithfulness routing with eleven reviewed theorem pins. Its focused 20-declaration audit has five empty, nine `propext`-only, and six `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The grouped family, ten payload fields, finite rank tags, route-clear checks, HB table and exact binding, exhaustive realizer claims, activity bits, dependency rows, and rank map must remain proof-bearing inputs. Positive slack, blocker semantics, semantic dependency completeness, construction from terminal data, selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A table, rank map, payload field, route-clear check, realizer claim, or family is hidden or presented as terminal-derived; a checked conditional faithfulness binding and contradiction is presented as unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 102 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
