...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/29279d1a9608a97832b7ad05e69a3fea05f39dd3/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/29279d1a9608a97832b7ad05e69a3fea05f39dd3/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 129-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,083 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 155 of 157 scoped milestone rows are earned. The newest scope is the Selector/HB ZeroSlack sidecar with seven reviewed theorem pins, all using only `Quot.sound` plus `propext`; the focused eight-declaration audit has one `propext`-only certificate declaration and seven `Quot.sound` plus `propext` theorem closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For arbitrary finite supplied grouped BN6, typed-realizer, and exact-rank HB dependency tables, the checked certificate yields all-canonical-selector nonfaithfulness, exact typed-bottom rows, complete no-outcome closure, all-node HN/BUD inactivity, and well-founded dependencies. The grouped family and tables remain supplied. This must not be described as selector faithfulness or compatibility, HN/BUD blocker semantics, semantic dependency completeness, the BCEL contradiction, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked sidecar over supplied tables is presented as derived terminal data, selector compatibility, blocker semantics, semantic dependency completeness, unconditional ZeroSlack, or a polynomial algorithm; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 129 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
