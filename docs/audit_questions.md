...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/38ad00f1798b703371c4d32d1507b43268f0ea9f/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/38ad00f1798b703371c4d32d1507b43268f0ea9f/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-one-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,639 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and fifteen earned scoped milestones. The newest scope is total fail-closed Packet source-payload realization with 11 reviewed theorem pins and a 21-declaration focused audit: 12 use only `propext`, and 9 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The grouped BN6 family must remain proof-bearing input. The unary code still denotes only a list position, and recovered source data is not a manuscript gain-or-blocker selector realizer. Payload serialization, a replacement circuit, manuscript-level selector faithfulness and compatibility, a gain or typed blocker route, derivation or grouping from terminal data, circuit-size bounds, polynomial generation and runtime, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | Source-payload materialization is presented as a replacement circuit, a manuscript gain-or-blocker realizer, payload serialization, circuit-size-bounded or polynomially generated, derived from terminal candidates, manuscript-level faithful or compatible, or a completed route; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 91 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
