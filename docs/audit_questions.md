...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/ff003dc1e86367a072e783d7fb8e32d374feed05/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/ff003dc1e86367a072e783d7fb8e32d374feed05/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-nine-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,719 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twenty-four earned scoped milestones. The newest scope is a checked HN/BUD active-dependency closure with seven reviewed theorem pins and a 13-declaration focused audit: six have empty closures and seven use `Quot.sound` with `propext`; none uses only `propext`, a project axiom, or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. Activity bits, dependency rows, rank mapping, selector family, faithfulness predicate, realizer claims, grouped family, and blueprints must remain proof-bearing inputs. Blocker activity, blocker semantics, semantic dependency completeness, gain exclusion, lower-seed closure, construction from terminal data, selector compatibility, rank-complete selector silence, the full HB negative closure, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An activity table, dependency row, rank mapping, faithfulness predicate, claim, blueprint, family, or semantic premise is hidden or presented as derived; checked local closure is presented as blocker activity, blocker semantics, semantic dependency completeness, gain exclusion, lower-seed closure, the full HB negative closure, global selector silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 99 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
