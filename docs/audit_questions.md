...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/edcf84c74280b455ca656ee57653d6fb07cfcc30/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/edcf84c74280b455ca656ee57653d6fb07cfcc30/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 132-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,114 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 161 of 163 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports 30%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the finite BCEL and Packet activation-coherence obstruction with six reviewed theorem pins. All twelve declarations in the focused audit use only `Quot.sound` and `propext`; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The newest row recomputes the selected nucleus defect, checks the Packet cut value and every canonical nonempty proper cut, and returns a proof-bearing mismatch because full numerical coherence conflicts with the same-family Packet exclusion. This must not be described as proving or constructing activation coherence, deriving the supplied terminal or Packet data, completing the no-lower ledger or global routing, or establishing unconditional ZeroSlack. Exhaustive cut enumeration may be exponential. Encoded-size and runtime bounds, complete PkgC, Package E, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A finite diagnostic over supplied data is presented as a repaired numerical bridge, derived activation coherence, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; row coverage is presented as proof completion; the local milestone inflates a fixed checkpoint; the supplied-input or exponential-enumeration boundary is hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 132 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
