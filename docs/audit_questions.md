...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/fa7436c2bb0efcc77f1a8b193b918c6d97d3d3b5/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/fa7436c2bb0efcc77f1a8b193b918c6d97d3d3b5/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-two-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,651 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and sixteen earned scoped milestones. The newest scope is the checked Packet selector candidate-gain scan with 12 reviewed theorem pins and a 20-declaration focused audit: 3 have empty closure, 2 use only `propext`, and 15 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The candidate implementations and grouped BN6 family must remain proof-bearing inputs. Cell-local no-gain must not be presented as an HN, budget, or lower-rank manuscript blocker or a global minimality result. Candidate construction, a replacement circuit, manuscript-level selector faithfulness and compatibility, a payload-mass-to-charge connection, a complete gain-or-typed-blocker route, derivation or grouping from terminal data, circuit-size bounds, polynomial generation and runtime, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A supplied candidate scan is presented as candidate construction, a complete manuscript gain-or-blocker realizer, circuit-size-bounded or polynomially generated, derived from terminal candidates, manuscript-level faithful or compatible, or a global minimality or blocker result; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 92 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
