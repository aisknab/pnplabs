...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/54305498ca55425a39cfee7892b40181e250bbdd/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/54305498ca55425a39cfee7892b40181e250bbdd/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,088 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 156 of 158 scoped milestone rows are earned. The newest scope is the Packet/budget no-lower ZeroSlack sidecar with five reviewed theorem pins, all using only `Quot.sound` plus `propext`; the focused six-declaration audit has one empty-axiom structure declaration and five `Quot.sound` plus `propext` theorem closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For arbitrary finite supplied same-candidate Packet and terminal-budget data, the checked certificate proves every governed budget-feasible support semantically minimum, excludes every such strict equivalent gain, and excludes a positive Packet conclusion for the supplied family. The caps, candidate-derived model, grouped family, payloads, ranks, claims, environment, dependency rows, and rank maps remain supplied. This must not be described as a complete manuscript no-lower result, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked sidecar over supplied data is presented as derived terminal data, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
