...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/821f05facdda1d41e6c1fb7b2e5206b2e2703715/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/821f05facdda1d41e6c1fb7b2e5206b2e2703715/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 136-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,134 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 170 of 172 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the BN6 computed-faithfulness checked Packet/HB bridge with one reviewed completion-theorem pin whose closure uses only Lean-standard `Quot.sound` and `propext`. Under one explicit constant-activation premise, positive residual slack reaches the general BN6 Packet theorem; checked route-clear payload data computes a faithful selector in the canonicalized table; and the M193 checked HB contradiction supplies conditional ZeroSlack before the checked loop is reused. Constant activation, the terminal family, carrier lower bound, payload and rank construction, route-clear proof, claim and HB tables, resolvers, normalizer, blocker semantics, and exhaustive reference fixture remain supplied; no fixed weighted checkpoint changes. Executable terminal-derived stages, manuscript-wide SaturatePositive and BCELReady, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied stages or data are presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M194; a remaining blocker is hidden; or a final theorem appears. |
