...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/81155a482de1e7cf1cd974cc8078dbd94d88d8ae/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/81155a482de1e7cf1cd974cc8078dbd94d88d8ae/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,628 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and fourteen earned scoped milestones. The newest scope is the canonical fail-closed Packet selector-handle codec with 11 reviewed theorem pins and a 16-declaration focused audit: 9 use only `propext`, and 7 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The grouped BN6 family must remain proof-bearing input. The unary code-length bound is by its explicit grouped-footprint list, not encoded circuit size. Polynomial enumeration and runtime, atom or payload encoding, manuscript-level selector faithfulness, compatibility, a realizer, a route, derivation or grouping from terminal data, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | The codec is presented as circuit-size-bounded, polynomially enumerable, payload-complete, derived from terminal candidates, manuscript-level faithful or compatible, a realizer, or a route; a polynomial generator, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
