...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/7080186d2b2701c9b472071cc14c04292a5dff6f/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/7080186d2b2701c9b472071cc14c04292a5dff6f/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,013 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 146 of 148 scoped milestone rows are earned. The newest scope is the terminal-derived HResolve support resolver with ten reviewed theorem pins. Its focused 15-declaration audit has one empty closure and 14 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every finite direct-wire candidate, Lean constructs the complete duplicate-free canonical terminal support family, saturates every seed, and computes either semantic-minimum evidence or a strict-equivalent-gain witness. The exhaustive search may be exponential. This must not be described as manuscript HN grammar, BWL exactness, H-disjointness, blocker or `NoHereditary` semantics, polynomial or full HResolve, the complete no-lower ledger, BudgetResolve, unconditional HB closure, global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A complete finite reference resolver is presented as a polynomial algorithm or the manuscript's full HResolve system; missing grammar, blocker, no-lower, runtime, or global premises are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
