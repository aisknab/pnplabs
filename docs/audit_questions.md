...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/28829559f3c2a151fb1983fa408e979296cdde76/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/28829559f3c2a151fb1983fa408e979296cdde76/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the eighty-nine-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,606 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twelve earned scoped milestones. The newest scope is exact finite grouped-footprint payload-selector-universe membership with 6 reviewed theorem pins and a 9-declaration focused audit: 3 use only `propext`, and 6 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The grouped BN6 family must remain proof-bearing input. The universe must remain exactly its duplicate-free grouped-footprint list, not the manuscript's encoded or polynomial selector universe. Selector faithfulness, compatibility, a realizer, a route, derivation from terminal data, polynomial enumeration and size bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | The finite footprint universe is presented as the manuscript's encoded or polynomial selector universe, as derived from terminal candidates, as faithful or compatible, or as a realizer or route; a polynomial generator, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 89 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
