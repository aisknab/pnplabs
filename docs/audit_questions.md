...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/f0bad8053fa334d4d996abd8ee5b796138526ec8/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/f0bad8053fa334d4d996abd8ee5b796138526ec8/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the seventy-nine-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,426 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm ninety-three earned scoped milestones. The newest scope computes a canonical minimum-cardinality positive anchor nucleus and returns the exact first anchor-algebra, proper-cut defect, or local-route failure, or exact constant-cut and local BN2 conclusions for every proper cut. Three global milestones, four project axioms, and six blockers remain. The terminal dependency system and positive whole-support projection defect must remain explicit premises. Their derivation, the complete global route system, manuscript activation and charge classes, `SaturatePositive`, Package E, BCELReady, later BCEL/BN2-BN6 conclusions, `ZeroSlack`, PCCMin exactness and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | The local anchor classifier is presented as deriving either premise, establishing global BCELReady, a global gain-completeness theorem, polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require seventy-nine PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
