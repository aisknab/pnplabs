...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/fe6c20fe6929c9e57c5ab7c483f7c56bfa9e9832/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/fe6c20fe6929c9e57c5ab7c483f7c56bfa9e9832/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 119-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,985 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 142 of 144 scoped milestone rows are earned. The newest scope is checked Packet semantic/HN activity binding with five reviewed theorem pins. Its focused nine-declaration audit has zero empty, four `propext`-only, and five `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The executable binding checker requires any failure of simultaneous frontier, obligation, activation, and direction agreement to imply HN-node activity at the table-owned handle rank. With the supplied checked well-founded HB no-outcome closure, all four fields agree; with the separately checked budget/HB binding and executable selector silence, every semantic first route is excluded and exact residual nondecrease is the sole local outcome. The grouped family, typed coordinates, activation atoms, direction and budget values, rank map, activity environment, dependency table, bindings, and closure premise remain proof-bearing inputs. They must not be described as terminal-derived or as proving HN blocker semantics, semantic dependency completeness, complete Packet adequacy, a decreasing transition, a no-lower contradiction, unconditional HB negative closure, or global route completion. Positive slack, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; the checked binding is presented as HN blocker semantics, semantic dependency completeness, complete Packet adequacy, a decreasing transition, no-lower contradiction, unconditional HB negative closure, or a complete global route system; positive slack or unconditional `ZeroSlack` is claimed; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 119 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
