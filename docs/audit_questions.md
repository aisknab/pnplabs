...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/c912b110f14386d335d58bb89b1bb9bb643bffb4/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/c912b110f14386d335d58bb89b1bb9bb643bffb4/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,130 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 166 of 168 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the proof-bearing PCCMin normalization and oracle composition with one axiom-free reviewed completion-theorem pin. It composes a supplied non-increasing semantic normalizer with the supplied total oracle, lifts later gains to strict gains from the original implementation, and transports exact-minimum and ZeroSlack endpoints into the checked loop. Both stages remain supplied and the regression fixtures use exhaustive reference minimisation; no fixed weighted checkpoint changes. Executable terminal-derived stages, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Either supplied stage is presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M190; a remaining blocker is hidden; or a final theorem appears. |
