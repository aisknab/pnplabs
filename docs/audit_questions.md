...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/72e2509cb18080700a764dde8309060a7d9bb21c/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/72e2509cb18080700a764dde8309060a7d9bb21c/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 103-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,756 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 129 of 131 scoped milestone rows are earned. The newest scope is the total Packet selector first-route outcome with seven reviewed theorem pins. Its focused 9-declaration audit has no empty, six `propext`-only, and three `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The classifier returns no route exactly on payload acceptance and an earliest typed route exactly on rejection. Every positive Packet under the canonicalized HB table and accepted executable selector silence therefore exposes a route without route-clear or binding premises. The grouped family, ten payload fields, finite rank tags and assignment, exhaustive realizer claims, activity bits, dependency rows, and rank map remain proof-bearing inputs. Route semantics, decreasing complete global coverage, positive slack, blocker semantics, semantic dependency completeness, construction from terminal data, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A rank map, payload field, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; a typed first route is presented as externally sound, decreasing globally complete routing, unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
