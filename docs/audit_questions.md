...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/7cba7335d300b31617fde64a2c7a96c758c6fdf5/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/7cba7335d300b31617fde64a2c7a96c758c6fdf5/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-six-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,694 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twenty-one earned scoped milestones. The newest scope is the checked Packet typed-realizer contract with five reviewed theorem pins and a 20-declaration focused audit: 10 have empty closures, five use only `propext`, and five use `Quot.sound` with `propext`; no reviewed closure uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The selector family, finite rank assignment, faithfulness predicate, realizer claims, activity tables, grouped family, and blueprints must remain proof-bearing inputs. Finite indices must not be presented as manuscript packet ranks, activity as blocker semantics, or rejection as bot reclassification. Blocker and blueprint construction from Packet or terminal data, selector faithfulness or compatibility, HB acyclicity, global selector silence, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A rank, faithfulness, claim, activity table, blueprint, family, or semantic premise is hidden or presented as derived; activity is presented as blocker semantics or the checked finite table as global selector silence or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 96 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
