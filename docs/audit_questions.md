...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/dd69b94a762eb830a3b91503faffde3984cce84f/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/dd69b94a762eb830a3b91503faffde3984cce84f/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,486 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm ninety-eight earned scoped milestones. The newest scope fixes the residual terminal rank as ten natural coordinates, proves executable/propositional comparison equivalence, supplies every priority witness, and proves accessibility, induction, and well-foundedness. Three global milestones, four project axioms, and six blockers remain. The route-to-outcome map, strict decrease for every current route, global route completeness, proof-bearing problem, and positive initial full-slack premise must not be hidden. Manuscript-wide `SaturatePositive`, manuscript activation and charge classes, Package E, BCELReady, later BCEL/BN2-BN6 conclusions, `ZeroSlack`, PCCMin exactness and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | RankWF is presented as a proof that current routes decrease or as complete global routing; a local route is presented as a complete global outcome, full Package E acceptance, verified global gain, or global route completeness; a global gain-completeness theorem, polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require eighty-two PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
