...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/b1b93861a2205deec70452df3db8d66a5cf8a8d5/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/b1b93861a2205deec70452df3db8d66a5cf8a8d5/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 125-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,042 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 150 of 152 scoped milestone rows are earned. The newest scope is the terminal HResolve maximal H-disjoint family with ten reviewed theorem pins. Two pins have empty axiom closures and eight use `Quot.sound` plus `propext`; the focused 18-declaration audit has ten empty and eight `Quot.sound` plus `propext` closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every arbitrary finite duplicate-free family of supplied hereditary footprints, the constructor checks eight exact interference domains, selects a governed duplicate-free maximal pairwise H-disjoint subfamily, and gives every rejected governed candidate its selected first blocker route. The footprints remain supplied. This must not be described as terminal derivation, manuscript HN grammar, BWL exactness, ParseOrExit, leaf tightness, the full NoHereditary sidecar, blocker-to-rank semantics, full or polynomial HResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A maximal H-disjoint subfamily over supplied footprints is presented as full HResolve or a polynomial algorithm; the supplied footprint boundary or missing grammar, blocker-rank, routing, runtime, or global premises are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 125 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
