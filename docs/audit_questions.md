...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/36a65d294276659f964e0b75cf102be2089fe1de/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/36a65d294276659f964e0b75cf102be2089fe1de/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 108 reviewed theorem kinds, kernel-type fingerprints, empty axiom closures, and the full Lean-source digest. | Same-name type weakening, an added axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm nine earned scoped milestones, including `CNFSAT ∈ NP`, bounded internal pipeline-stage bridges, and the separate terminal packer, plus three unearned global milestones, four project axioms, and seven blockers. Confirm that packer-to-bridge composition, target termination, complete raw refinement, an external input-size polynomial, CNF-SAT in P, and NP-completeness remain false. | A supplied-trace/internal-handoff result or standalone packer is widened into a complete compiler, a local/conditional result is presented as global, or a blocker/axiom is hidden. |
