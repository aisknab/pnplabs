...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/337e07e725f72ddd832c2a7885d505e9044838be/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/337e07e725f72ddd832c2a7885d505e9044838be/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 127-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,071 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 153 of 155 scoped milestone rows are earned. The newest scope is the terminal HResolve ZeroSlack sidecar with seven reviewed theorem pins. Two pins have empty axiom closures and five use only `Quot.sound` plus `propext`; the focused 8-declaration audit has three empty and five `Quot.sound` plus `propext` closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every arbitrary finite supplied governed family, the sidecar recomputes duplicate-free total blocked coverage, excludes exact and gain routes for every listed candidate, and binds them to semantic minimum and strict equivalent gain propositions. The governed family, implementation map, predicates, decidability, and blocker semantics remain supplied. This must not be described as hereditary-candidate derivation, HN grammar soundness or completeness, BWL, ParseOrExit, leaf tightness, H0-H4 blocker semantics, full or polynomial HResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked sidecar over supplied predicates is presented as terminal derivation, H0-H4 blocker semantics, full HResolve, unconditional ZeroSlack, or a polynomial algorithm; the supplied family, implementation map, predicates, decidability, blocker semantics, or missing grammar, routing, runtime, or global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
