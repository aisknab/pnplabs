...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/83b2a786c1960e15d193404eb7a5eef14fd0cb89/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/83b2a786c1960e15d193404eb7a5eef14fd0cb89/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 109-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,818 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 134 of 136 scoped milestone rows are earned. The newest scope is canonical Packet charge-route reflection with 19 reviewed theorem pins. Its focused 27-declaration audit has one empty, 24 `propext`-only, and two `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. Each selected canonical source atom has strictly positive mass, so the internal charge field is set by construction while the canonical source route, authoritative rank, and exact RankWF descent computation are retained. The first route can return none of charge, exactRoute, or rank; a final descent route carries actual nondecrease; and the positive Packet/HB endpoint carries exact failure evidence without route-clear or binding premises. The finite rank map, before/after ranks, six remaining semantic Boolean fields, grouped family, exhaustive realizer claims, activity bits, dependency rows, and rank map remain proof-bearing inputs. Positive source mass must not be described as full external charge-surplus, budget, replacement, or selector-compatibility semantics. External semantics for the six remaining routes, decreasing complete global coverage, a guaranteed decreasing transition, a no-lower ledger, positive slack, blocker semantics, semantic dependency completeness, construction from terminal data, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A rank map, payload field, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; positive source mass is presented as full external charge-surplus or replacement semantics, proof that a decreasing transition exists, or proof that the remaining routes have external semantics and decreasing globally complete coverage; unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack` is claimed; a no-lower ledger, circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 107 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
