...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/289ccde2874be3f2f4684470602cad073858fac0/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/289ccde2874be3f2f4684470602cad073858fac0/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 115-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,945 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 139 of 141 scoped milestone rows are earned. The newest scope is typed Packet direction-route reflection with 28 reviewed theorem pins. Its focused 35-declaration audit has two empty, 17 `propext`-only, and 16 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For explicit typed source and selector direction values with decidable equality, the canonical payload computes direction acceptance from exact equality while retaining computed BN5 frontier, obligation, and BN4 activation, grouped colour, positive charge, the internal source route, authoritative rank, and exact RankWF descent computation. A direction first route is prior frontier, obligation, and activation equality with typed-direction inequality, the first route can return none of colour, charge, exactRoute, or rank, a final descent route carries actual nondecrease, and the positive Packet/HB endpoint carries exact failure evidence without route-clear or binding premises. The direction values, BN5 coordinates, finite rank map, before/after ranks, budget, grouped family, exhaustive realizer claims, activity bits, and dependency rows remain proof-bearing inputs. The direction values must not be described as constructed from terminal data or as proving the manuscript's complete Dir(u) or Packet adequacy bridge. External semantics for the sole remaining budget route, decreasing complete global coverage, a guaranteed decreasing transition, a no-lower ledger, positive slack, blocker semantics, semantic dependency completeness, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A direction value, BN5 coordinate, rank map, payload field, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; supplied direction values are presented as a complete Dir(u) or Packet adequacy proof; route exclusion is presented as proof that a decreasing transition exists or that the budget route has external semantics and decreasing globally complete coverage; unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack` is claimed; a no-lower ledger, circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 115 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
