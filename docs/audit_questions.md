...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/e355f1c93ef17f7d8069cc128b204a351b4792b7/status/FORMAL_RECONSTRUCTION_STATUS.json)
...
The current canonical download is the 142-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,140 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 176 of 178 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is a singleton-and-pair sparse V53 proper-cut route with one reviewed completion-theorem pin whose closure uses only Lean-standard `Quot.sound` and `propext`. The duplicate-free quadratic-size family is equivalent to every nonempty proper-cut equation; the total classifier returns the first exact small-cut mismatch or derives the complete equation. At the checked boundary, a mismatch is reflected through the direct raw ledger, while coherence yields conditional ZeroSlack. The terminal problem, positive premise, checked finite BCEL-ready certificate, raw positive cells and payloads, tables, ranks, route-clear proof, resolvers, normalizer, blocker semantics, upstream construction, and complete encoded-size bounds remain supplied. The local quadratic list bound is not a complete polynomial construction or runtime theorem, a mismatch is not a gain or global rank decrease, and no fixed checkpoint changes. Executable terminal-derived stages, complete PkgC/BN3 through BN6 integration, manuscript-wide SaturatePositive and BCELReady, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied stages or data are presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M200; the local list bound is presented as a complete runtime theorem; a mismatch is presented as a decreasing route; a remaining blocker is hidden; or a final theorem appears. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 142 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
