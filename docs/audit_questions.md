...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/0a21d196359069a09be43c489a8fe4a95f5c8cf6/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/0a21d196359069a09be43c489a8fe4a95f5c8cf6/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,730 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 126 of 128 scoped milestone rows are earned. The newest scope is executable HB selector-silence induction with seven reviewed theorem pins. Its focused nine-declaration audit has three empty closures and six `Quot.sound` plus `propext` closures; none uses only `propext`, a project axiom, or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The grouped family, selector table, finite ranks, faithfulness predicate, exhaustive realizer claims, activity bits, dependency rows, and rank map must remain proof-bearing inputs. Blocker semantics, semantic dependency completeness, construction from terminal data, selector faithfulness and compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A table, rank map, faithfulness predicate, realizer claim, or family is hidden or presented as derived; an all-row typed-bottom check is presented as selector faithfulness, compatibility, unconditional HB negative closure, independently constructed global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
