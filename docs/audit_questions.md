...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/9f26c50f8e8bfbda9cb15a8142c6191482148220/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/9f26c50f8e8bfbda9cb15a8142c6191482148220/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 117-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,980 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 141 of 143 scoped milestone rows are earned. The newest scope is checked Packet budget/HB activity binding with five reviewed theorem pins. Its focused seven-declaration audit has zero empty, two `propext`-only, and five `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The executable binding checker requires every typed source/selector budget mismatch to imply HB budget-node activity at the table-owned handle rank. When that binding and the supplied checked well-founded HB no-outcome closure pass, every canonical handle has equal typed budgets, the Packet first-route classifier excludes the budget route, and the positive endpoint is confined to frontier, obligation, activation, direction, or exact residual nondecrease. The grouped family, binding, typed budget values, rank map, activity environment, dependency table, and closure premise remain proof-bearing inputs. They must not be described as terminal-derived or as constructing the manuscript's Bud(u), implementing BudgetResolve, proving complete Packet adequacy or blocker semantics, establishing unconditional HB negative closure, or excluding the remaining routes. Positive slack, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; the checked binding is presented as a Bud(u) construction, BudgetResolve, complete Packet adequacy, blocker semantic completeness, unconditional HB negative closure, or complete route exclusion; positive slack or unconditional `ZeroSlack` is claimed; a no-lower ledger, circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 117 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
