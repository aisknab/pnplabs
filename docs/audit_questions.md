...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/02a8fc9ea8fb7c338b9c6a484020fe6dfc754872/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/02a8fc9ea8fb7c338b9c6a484020fe6dfc754872/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 141-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,139 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 175 of 177 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is a sparse V53 constant-cut basis with one reviewed completion-theorem pin whose closure uses only Lean-standard `Quot.sound` and `propext`. The basis is equivalent to every nonempty proper-cut equation; the total classifier avoids enumerating carrier subsets; acceptance derives constant activation and conditional ZeroSlack. The terminal problem, positive premise, checked finite BCEL-ready certificate, raw positive cells and payloads, tables, ranks, route-clear proof, resolvers, normalizer, blocker semantics, upstream construction, and complete encoded-size bounds remain supplied. Rejection is a structural diagnostic, not a gain, and no fixed checkpoint changes. Executable terminal-derived stages, complete PkgC/BN3 through BN6 integration, manuscript-wide SaturatePositive and BCELReady, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied stages or data are presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M199; basis rejection is presented as a decreasing route; a remaining blocker is hidden; or a final theorem appears. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 141 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
