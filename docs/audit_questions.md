...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/19f43501ad87d4c5611ba109d53157fd0bd1dfdb/status/FORMAL_RECONSTRUCTION_STATUS.json)
...
The current canonical download is the eighty-one-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,459 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm ninety-six earned scoped milestones. The newest scope recognizes only an exact candidate-derived interface consumer, proves transparent balance or returns a proof-bearing local E-route, and records the exact first interface-exposure event and complete transparent prefix. Three global milestones, four project axioms, and six blockers remain. The observer and projection must remain explicit inputs. Only the finite terminal forms of `projectionPositivityNotLostSilently`, `transparentSaturationCostBalanced`, `firstNontransparentStepRecorded`, and `interfaceExposureRoutesToE` are closed among the five SaturatePositive obligations. `originKernelObligationClosureRouted`, full `SaturatePositive`, the complete global route system, manuscript activation and charge classes, Package E, BCELReady, later BCEL/BN2-BN6 conclusions, `ZeroSlack`, PCCMin exactness and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | The local route is presented as full Package E acceptance, verified global gain, or global route completeness; the remaining SaturatePositive obligation is treated as discharged; full SaturatePositive or global BCELReady is claimed; a global gain-completeness theorem, polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require eighty-one PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
