...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/9835168ab80564fce8a7a66c0a2c2d0d9027ff07/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/9835168ab80564fce8a7a66c0a2c2d0d9027ff07/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-seven-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,701 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twenty-two earned scoped milestones. The newest scope is exact-rank acyclicity for a supplied finite HN and budget dependency graph with seven reviewed theorem pins and a 22-declaration focused audit: 12 have empty closures, one uses only `propext`, and nine use `Quot.sound` with `propext`; no reviewed closure uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The graph, edges, rank mapping, selector family, faithfulness predicate, realizer claims, activity tables, grouped family, and blueprints must remain proof-bearing inputs. Dependency completeness, blocker semantics, active-row coverage, construction from terminal data, selector compatibility, rank-complete selector silence, the full HB negative closure, encoded-size and runtime bounds, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A graph, edge, rank mapping, faithfulness, claim, activity table, blueprint, family, or semantic premise is hidden or presented as derived; activity is presented as blocker semantics or checked graph acyclicity as dependency completeness, the full HB negative closure, global selector silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 97 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
