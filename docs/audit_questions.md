...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/9f2e7f022d1483702a1ead06392ac2dafe86120b/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/9f2e7f022d1483702a1ead06392ac2dafe86120b/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 135-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,133 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 169 of 171 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports an unchanged 35%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the checked Packet/HB conditional ZeroSlack bridge with one reviewed completion-theorem pin whose closure uses only Lean-standard `Quot.sound` and `propext`. Complete checked Packet claims and exact-rank silence feed executable selector-silence induction; checked HB no-outcome closure then eliminates every canonical faithful handle, and one explicit positive-residual-slack-to-faithful-selector premise supplies the conditional ZeroSlack contradiction before the M192 selector construction and loop are reused. The positive-slack bridge, terminal family, rank assignment, claim table, resolvers, normalizer, blocker semantics, and exhaustive reference fixture remain supplied; no fixed weighted checkpoint changes. Executable terminal-derived stages, unconditional ZeroSlack, encoded-size polynomial runtime and certificate bounds, a target decider, SAT NP-hardness or CNFSAT NP-completeness transport, the root theorem, and the publication gate must remain absent. | Supplied stages or data are presented as executable PCCMin or unconditional ZeroSlack; row coverage is presented as proof completion; a fixed checkpoint is awarded for M193; a remaining blocker is hidden; or a final theorem appears. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 135 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
