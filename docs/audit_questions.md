...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/ed0ba30a44abb27dbd4e5914541b9fcc8bd82950/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/ed0ba30a44abb27dbd4e5914541b9fcc8bd82950/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 121-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,003 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 145 of 147 scoped milestone rows are earned. The newest scope is the finite supplied HResolve coverage ledger with ten reviewed theorem pins. Its focused ten-declaration audit has zero empty, four `propext`-only, and six `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. Every supplied candidate receives one fixed-priority exact, gain, blocked, or unresolved route; the generated ledger is sound and complete for that enumeration; and a separately checked duplicate-free `NoHereditary` sidecar excludes exact and gain for every listed candidate. The candidate universe and exact, gain, and blocker predicates remain proof-bearing inputs. This must not be described as terminal-derived HN enumeration, established predicate semantics, full HResolve, the complete no-lower ledger, BudgetResolve, normalization, other named obstruction routing, saturation, replay, unconditional HB closure, global ZeroSlack, or complete global routing. Positive slack, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; the finite supplied ledger is presented as full HResolve or the complete global route system; positive slack is claimed; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 121 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
