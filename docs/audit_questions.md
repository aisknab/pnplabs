...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/5f2d880b953cfc1f2d26b6fc0d6a97d5a1c2b8cf/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/5f2d880b953cfc1f2d26b6fc0d6a97d5a1c2b8cf/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-five-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,689 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and twenty earned scoped milestones. The newest scope is the checked Packet unit-charge blueprint realizer with 13 reviewed theorem pins and a 30-declaration focused audit: 6 have empty closures, 11 use only `propext`, and 13 use `Quot.sound` with `propext`; no reviewed closure uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The grouped family, candidates, replacement blueprints, occurrence pairings, and unmatched lists must remain proof-bearing inputs. Blueprint construction from Packet or terminal data, selector faithfulness or compatibility, encoded-size and runtime bounds, typed blockers or HB/rank closure, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A blueprint, family, pairing, unmatched list, or semantic premise is hidden or presented as derived; family-local validator silence is presented as a typed blocker, global no-gain result, semantic minimum, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, typed blocker or rank closure, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 95 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
