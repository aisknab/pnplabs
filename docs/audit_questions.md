...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/5a2cf2b0bea9568d7361336fa5f8f197246a2f9c/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/5a2cf2b0bea9568d7361336fa5f8f197246a2f9c/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the forty-five-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1445 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm forty-six earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, four complete fixed clauses, and traversal through all remaining padding of the first scheduled constraint. Confirm that the latest machine emits no token, preserves `encodedFormula.take (2 * (FormulaWidth + 36))`, advances to `FormulaVariableSlotBound + 1 + FormulaClauseSlotsPerConstraint * FormulaTokensPerClause`, and proves the retained direct token is the `Sep` beginning the second scheduled constraint. Confirm three unearned global milestones, four project axioms, and six blockers, and that emission of that separator, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The bounded first-constraint padding run is widened into separator emission, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require forty-five PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
