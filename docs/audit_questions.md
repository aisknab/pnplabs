...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/5055eaaa82a57ee5ce0793f07e2f49ae82189169/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/5055eaaa82a57ee5ce0793f07e2f49ae82189169/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-three-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,661 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and seventeen earned scoped milestones. The newest scope is the exhaustive Packet selector-universe gain scan with 10 reviewed theorem pins and a 19-declaration focused audit: 9 use only `propext`, and 10 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The candidate implementations and grouped BN6 family must remain proof-bearing inputs. Supplied-universe no-gain must not be presented as an HN, budget, or lower-rank manuscript blocker, selector faithfulness or compatibility, or a global minimality result. Replacement-candidate construction, a payload-mass-to-charge connection, a complete gain-or-typed-blocker route, derivation or grouping from terminal data, circuit-size bounds, polynomial enumeration and runtime, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A supplied-universe scan is presented as candidate construction, a complete manuscript gain-or-blocker realizer, circuit-size-bounded or polynomially generated, derived from terminal candidates, manuscript-level faithful or compatible, or a global minimality or blocker result; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 93 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
