...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/77b275efe586e68339f6dd61abb5a2d3abcdce68/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/77b275efe586e68339f6dd61abb5a2d3abcdce68/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,676 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and nineteen earned scoped milestones. The newest scope is the finite Packet charge-surplus realizer kernel with 8 reviewed theorem pins and a 10-declaration focused audit: 2 have empty closures, 2 use only `propext`, and 6 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The finite ledgers, exact occurrence pairing, unmatched positive charge, gate accounting, and semantic equivalence must remain proof-bearing inputs. Replacement or charge-ledger construction from Packet or terminal data, selector faithfulness or compatibility, encoded-size and runtime bounds, typed blockers or HB/rank closure, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit ledger, pairing, witness, accounting, or semantic premise is hidden or presented as derived; the finite kernel is presented as a complete realizer or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, typed blocker or rank closure, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
