...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/bdbbabc67f20b5d4f673e11098ba86a9d10a1cf1/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/bdbbabc67f20b5d4f673e11098ba86a9d10a1cf1/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 2,200 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm eighty-one earned scoped milestones. The newest scope exhaustively searches every canonical primitive-record seed for a finite direct-wire candidate and explicit terminal dependency system. It saturates and physically completes each seed, extracts its exact open support, computes exact local gain, returns a proof-bearing nonempty proper positive-gain support whenever one exists in that universe, and returns `none` exactly when none exists there. The dependency system remains explicit rather than circuit-derived, and the search is exhaustive rather than polynomial. Three global milestones, four project axioms, and six blockers remain. Global gain completeness, full manuscript support completion, square legitimacy, BCEL/BN2–BN6, route completeness, `ZeroSlack`, PCCMin exactness and polynomial runtime, SAT NP-hardness or CNF-SAT NP-completeness, abstract threshold discharge, CNF-SAT in P, and `P = NP` must remain absent. | A bounded exhaustive seed search is presented as a circuit-derived dependency frontier, global gain-completeness theorem, polynomial minimizer, complete support square or gain route, CNF-SAT decider, assumption discharge, or final theorem; or a blocker/axiom is hidden. |
