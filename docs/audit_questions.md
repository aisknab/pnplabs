...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/2d66e5a7c3468b51e019c638f43780e7acfb8f93/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/2d66e5a7c3468b51e019c638f43780e7acfb8f93/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 124-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,028 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 148 of 150 scoped milestone rows are earned. The newest scope is the finite terminal budget no-lower ledger with eight reviewed theorem pins. Its focused 13-declaration audit has one empty closure and 12 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every finite direct-wire candidate, candidate-derived saturation model, and supplied natural resource caps, Lean classifies every canonical support as exact, strict gain, or `NoBudget`, materializes the complete route ledger, and proves that acceptance makes every budget-feasible governed support a semantic minimum while excluding every feasible strict-equivalent gain. The caps remain supplied and the exhaustive search may be exponential. This closes only the finite terminal-derived budget branch and must not be described as manuscript HN/BUD grammar, BWL or a budget-envelope dynamic program, blocker semantics, polynomial BudgetResolve, Packet or complete no-lower composition, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An exhaustive finite reference ledger is presented as a polynomial algorithm or manuscript BudgetResolve; supplied caps or missing grammar, blocker, no-lower composition, runtime, or global premises are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 124 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
