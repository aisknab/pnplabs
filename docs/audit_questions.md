...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/ed439e1477e69315006fe7f8bbecac278c024bbf/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/ed439e1477e69315006fe7f8bbecac278c024bbf/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the thirty-six-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1089 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm thirty-eight earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, all three complete fixed clauses, and the exact remaining clause-two padding run. Confirm that the latest machine emits the clause-three `Finish`, advances the retained coordinate to `thirdClauseStart + 8`, proves the next direct token is padding, and emits `encodedFormula.take (2 * (FormulaWidth + 27))`. Confirm three unearned global milestones, four project axioms, and six blockers, and that clause-three padding traversal, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The fixed third-clause completion is widened into clause-three padding traversal, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require thirty-six PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
