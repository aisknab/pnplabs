...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/d9940d34f6273831dd889f1e23073ce9bff1b039/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/d9940d34f6273831dd889f1e23073ce9bff1b039/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the ninety-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,617 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and thirteen earned scoped milestones. The newest scope is canonical finite Packet selector handles with 11 reviewed theorem pins and a 16-declaration focused audit: 4 use only `propext`, and 12 use `Quot.sound` with `propext`. Two global milestones, four project axioms, and five blockers remain. The grouped BN6 family must remain proof-bearing input. The handles must remain input-relative positions in its exact duplicate-free grouped-footprint list, not the manuscript's bit encoding or a polynomially enumerable selector universe. Manuscript-level selector faithfulness, compatibility, a realizer, a route, derivation or grouping from terminal data, polynomial encoding bounds and runtime, complete PkgC, global outcome integration, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | The input-relative handles are presented as the manuscript's bit encoding or a polynomial selector universe, as derived from terminal candidates, as manuscript-level faithful or compatible, or as a realizer or route; a polynomial generator, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 90 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
