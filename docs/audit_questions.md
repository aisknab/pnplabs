...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/a9559242cd6171e217e9917c9c70f99a98ca7757/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/a9559242cd6171e217e9917c9c70f99a98ca7757/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,092 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 157 of 159 scoped milestone rows are earned. The newest scope is the BCEL/Packet no-lower ZeroSlack sidecar with four reviewed theorem pins. The focused five-declaration audit has one empty-axiom structure, one `propext`-only theorem, and three `Quot.sound` plus `propext` theorem closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For a supplied M180 certificate with at least two grouped-carrier anchors, BCEL constant activation would construct the positive BN6 Packet excluded by that same-family no-lower certificate. The family and all inherited terminal, budget, Packet, realizer, dependency, and rank inputs remain supplied. This must not be described as deriving constant activation from positive residual slack, constructing BCELReady, completing the manuscript no-lower ledger, establishing unconditional global ZeroSlack, or completing global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked conditional contradiction over supplied data is presented as derived BCELReady or constant activation, complete no-lower coverage, unconditional ZeroSlack, or a polynomial algorithm; the supplied-input boundary or remaining global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
