...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/6b0b51ad3fda5bed69ca765b485b166746da8cfd/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/6b0b51ad3fda5bed69ca765b485b166746da8cfd/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the forty-nine-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1547 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm forty-nine earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, four complete fixed clauses, traversal through all remaining padding of the first scheduled constraint, the second-constraint `Sep` and sign, and the first unary `T` of its first variable index. Confirm that the latest machine preserves `encodedFormula.take (2 * (FormulaWidth + 39))`, advances to `FormulaVariableSlotBound + 1 + FormulaClauseSlotsPerConstraint * FormulaTokensPerClause + 3`, and proves the direct next token is the second unary `T`. Confirm three unearned global milestones, four project axioms, and six blockers, and that the following second unary `T`, the rest of the first literal, traversal of the second constraint, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The one-token first-unary-unit step is widened into more unary-index emission, a completed literal or second-constraint traversal, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require forty-nine PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
