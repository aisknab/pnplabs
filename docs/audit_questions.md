...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/23f53b6efccee3ff50987cf55338b8b01ddad343/status/FORMAL_RECONSTRUCTION_STATUS.json)
...
The current canonical download is the sixty-five-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,030 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm sixty-eight earned scoped milestones. The newest scope proves exact all-input behavior for one literal strict-v0 target emitter, an explicit polynomial non-timeout bound, a quadratic output-size bound, polynomial-time machine/function witnesses, recursive `RawRefinement`, and strict parser/emitter composition. Confirm three unearned global milestones, four project axioms, and six blockers. A packaged concrete `PolynomialReduction`, abstract threshold discharge, remaining Cook-Levin formula body, complete raw builder, CNF-SAT in P, and NP-completeness must remain absent. | Exact target emission is presented as a complete reduction; a bounded formula prefix is presented as a complete reduction; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require sixty-five PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
