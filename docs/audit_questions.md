...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/055325ee54abddefc595ccb826ea410dc8be2231/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/055325ee54abddefc595ccb826ea410dc8be2231/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 116-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,975 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 140 of 142 scoped milestone rows are earned. The newest scope is typed Packet budget-route reflection with 30 reviewed theorem pins. Its focused 37-declaration audit has two empty, 17 `propext`-only, and 18 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For explicit typed source and selector budget values with decidable equality, the canonical payload computes budget acceptance from exact equality while retaining computed frontier, obligation, activation, direction, grouped colour, positive charge, the internal source route, authoritative rank, and exact RankWF descent computation. A budget first route is the four prior equalities with typed-budget inequality; the first route can return none of colour, charge, exactRoute, or rank; a final descent route carries actual nondecrease; and the positive Packet/HB endpoint carries exact failure evidence without route-clear or binding premises. Every local Packet classifier field is computed, but the coordinates, direction and budget values, finite rank map, before/after ranks, grouped family, exhaustive realizer claims, activity bits, and dependency rows remain proof-bearing inputs. They must not be described as constructed from terminal data or as proving the manuscript's complete BN5, Dir(u), Bud(u), or Packet adequacy bridge. Local Packet budget coherence must not be identified with BudgetResolve or HB budget activity. A guaranteed decreasing transition, no-lower ledger, positive slack, blocker semantics, semantic dependency completeness, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; computed local classifier fields are presented as complete Packet adequacy or global route exclusion; local budget equality is presented as the complete Bud(u) envelope, BudgetResolve, or HB budget activity; route exclusion is presented as proof that a decreasing transition exists or routing is globally complete; unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack` is claimed; a no-lower ledger, circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 116 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
