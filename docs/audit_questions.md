...
| Milestone earning | Check all 2,487 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm ninety-nine earned scoped milestones. The newest scope publishes the uniform all-bitstring concrete locked-NAND polynomial reduction with one reviewed theorem pin using only `Quot.sound` and `propext`. Two global milestones, four project axioms, and five blockers remain. The reduction must not be described as a polynomial-time target decider, CNF-SAT NP-hardness or NP-completeness, activation of the legacy string-handle bridge, or P = NP. The route-to-outcome map, strict decrease for every current route, global route completeness, proof-bearing problem, and positive initial full-slack premise must not be hidden. Manuscript-wide `SaturatePositive`, manuscript activation and charge classes, Package E, BCELReady, later BCEL/BN2-BN6 conclusions, `ZeroSlack`, PCCMin exactness and polynomial runtime, CNF-SAT in P, and `P = NP` must remain absent. | The concrete reduction is presented as a target decider, hardness theorem, or final result; RankWF is presented as a proof that current routes decrease or as complete global routing; a local route is presented as a complete global outcome, full Package E acceptance, verified global gain, or global route completeness; a global gain-completeness theorem, polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
