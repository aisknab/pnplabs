...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/6d598d9ef6a866f475bcc0fa0e5d1f65061524d4/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/6d598d9ef6a866f475bcc0fa0e5d1f65061524d4/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 122-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,020 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 147 of 149 scoped milestone rows are earned. The newest scope is the finite terminal budget-envelope resolver with seven reviewed theorem pins. Its focused 15-declaration audit has one empty closure and 14 `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For every finite direct-wire candidate, candidate-derived saturation model, and supplied natural gate and saturated-record caps, Lean scans every canonical seed and returns a feasible semantic minimum, a feasible strict equivalent gain, or complete `NoBudget` exclusion. The caps remain supplied and the exhaustive search may be exponential. This must not be described as manuscript HN/BUD grammar, BWL or a budget-envelope dynamic program, blocker semantics, polynomial BudgetResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An exhaustive finite reference resolver is presented as a polynomial algorithm or manuscript BudgetResolve; supplied caps or missing grammar, blocker, no-lower, runtime, or global premises are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 122 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
