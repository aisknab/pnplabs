...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/170775d279865370c239919326f1f336cf254b70/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/170775d279865370c239919326f1f336cf254b70/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the fifty-one-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1615 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm fifty-one earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, four complete fixed clauses, traversal through all remaining padding of the first scheduled constraint, the second-constraint `Sep` and sign, and all three unary `T` tokens of its first variable index. Confirm that the latest machine preserves `encodedFormula.take (2 * (FormulaWidth + 41))`, advances to `FormulaVariableSlotBound + 1 + FormulaClauseSlotsPerConstraint * FormulaTokensPerClause + 5`, and proves the direct next token is the terminating `F`. Confirm three unearned global milestones, four project axioms, and six blockers, and that the following terminating `F`, the rest of the first literal, traversal of the second constraint, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The one-token third-unary-unit step is widened into a completed literal or second-constraint traversal, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require fifty-one PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
