...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/98da0c0ad5614d69f5f5b50f629a759dc5c29fb9/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/98da0c0ad5614d69f5f5b50f629a759dc5c29fb9/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 128-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 3,076 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 154 of 156 scoped milestone rows are earned. The newest scope is the terminal Budget ZeroSlack sidecar with five reviewed theorem pins, all using only `Quot.sound` plus `propext`; the focused 6-declaration audit has six `Quot.sound` plus `propext` closures. None uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. For arbitrary finite direct-wire candidates and supplied natural caps, the sidecar records the failed exhaustive search over the complete canonical terminal support universe, excludes every budget-feasible support, and separately binds exact and gain routes to semantic minimum and strict equivalent gain propositions. The natural caps remain supplied and the search may be exponential. This must not be described as B0-B4 Budget grammar or blocker semantics, full or polynomial BudgetResolve, the complete no-lower ledger, unconditional global ZeroSlack, or complete global routing. Encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A checked sidecar over supplied caps is presented as B0-B4 blocker semantics, full BudgetResolve, unconditional ZeroSlack, or a polynomial algorithm; the supplied caps, exhaustive-search boundary, missing grammar, routing, runtime, or global obligations are hidden; a CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 128 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
