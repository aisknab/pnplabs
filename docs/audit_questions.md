...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/b69c7419158154187c2adc6991ebe31aef6538f9/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/b69c7419158154187c2adc6991ebe31aef6538f9/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 104-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,762 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 130 of 132 scoped milestone rows are earned. The newest scope is exact Packet selector first-route failure semantics with six reviewed theorem pins. Its focused 8-declaration audit has one empty, five `propext`-only, and two `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. Every returned route is equivalent to the unique earliest failed supplied field, with every preceding field accepted; rejection is equivalent to exact failure existence, and the positive Packet/HB endpoint carries both route and proof. The grouped family, ten payload fields, finite rank tags and assignment, exhaustive realizer claims, activity bits, dependency rows, and rank map remain proof-bearing inputs. External field semantics, decreasing complete global coverage, positive slack, blocker semantics, semantic dependency completeness, construction from terminal data, full external selector compatibility, unconditional HB negative closure, unconditional `ZeroSlack`, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | A rank map, payload field, realizer claim, activity bit, dependency row, or family is hidden or presented as terminal-derived; an exact field-failure result is presented as external manuscript semantics, decreasing globally complete routing, unconditional positive slack, compatibility, HB negative closure, independently constructed global silence, or unconditional `ZeroSlack`; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
