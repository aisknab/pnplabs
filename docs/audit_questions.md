...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/63ba53d76da0878b0d3b2834b891afcc6ef0fe0e/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/63ba53d76da0878b0d3b2834b891afcc6ef0fe0e/docs/FORMAL_RECONSTRUCTION.md).
...
| Milestone earning | Check all 3,120 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm formal artefact coverage is 162 of 164 scoped milestone rows and is not labelled proof completion. Confirm the fixed-weight ledger independently reports 32%, a 20% to 40% uncertainty range, and five open global gates. The newest scope is the concrete report-facing locked-NAND compatibility link with six reviewed theorem pins and no project axiom at its checked endpoint. It replaces a caller-supplied reduction field with direct reuse of the concrete all-bitstring reduction and removes `PNP.LockedNANDThreshold` from the active project-axiom inventory. Exactly the named `reductions-final-target-compatibility` and `axiom-remove-locked-nand-threshold` checkpoints become earned; three project axioms and five blockers remain. A target decider, SAT NP-hardness or CNFSAT NP-completeness transport, unconditional residual minimisation, ZeroSlack, exact polynomial PCCMin, the root theorem, and the publication gate must remain absent. | Compatibility names are presented as a target solver or hardness proof; row coverage is presented as proof completion; any checkpoint beyond the two recorded M186 changes is awarded; a remaining blocker or axiom is hidden; or a final theorem appears. |
