...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/a632bd845a69a7c442e1e6fcecb00981854b2f1c/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/a632bd845a69a7c442e1e6fcecb00981854b2f1c/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the fifty-four-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 1689 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm fifty-three earned scoped milestones, including `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin generated-CNF semantics, the actual encoded-formula size polynomial, the answer-independent rectangular schedule, the direct coordinate cursor, four complete fixed clauses, traversal through all remaining padding of the first scheduled constraint, the complete first literal of the second scheduled constraint, and its width-selected successor token. Confirm that the latest machine preserves `encodedFormula.take (2 * (FormulaWidth + 43))`, advances to `FormulaVariableSlotBound + 1 + FormulaClauseSlotsPerConstraint * FormulaTokensPerClause + 7`, and emits `Finish` at width one or positive `T` at wider widths. Confirm three unearned global milestones, four project axioms, and six blockers, and that the following padding or unary opportunity, traversal of the second constraint, a general dynamic formula cursor, the remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, packaged polynomial reduction, CNF-SAT in P, and NP-completeness remain absent. | The one-token successor step is widened into second-constraint traversal, a general cursor, complete formula builder, construction-runtime refinement, or reduction theorem; a local/conditional result is presented as global; or a blocker/axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require fifty-four PDF pages. | An alias differs, the old 56-page hash returns, or report text conflicts with generated status. |
