...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/7a3308520a68b5553cc95a7b56c560a20508111a/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/7a3308520a68b5553cc95a7b56c560a20508111a/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the 120-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,989 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm 143 of 145 scoped milestone rows are earned. The newest scope is one checked local Packet descent no-lower row with four reviewed theorem pins. Its focused six-declaration audit has zero empty, two `propext`-only, and four `Quot.sound` plus `propext` closures; none uses a project axiom or `Classical.choice`. Two global milestones, four project axioms, and five blockers remain. The executable checker scans every canonical handle and accepts exactly when no fully computed first route is residual nondecrease. A supplied positive Packet conclusion, checked semantic/HN and budget/HB bindings, executable selector silence, and checked well-founded HB no-outcome closure force rejection; accepting that same local row is contradictory. The grouped family, rank map, residual ranks, typed fields, realizer and dependency tables, bindings, silence, and closure checks remain proof-bearing inputs. They must not be described as terminal-derived or as the complete no-lower ledger, HResolve, BudgetResolve, normalization, named descent routing, saturation, replay, unconditional HB closure, global ZeroSlack, or complete global routing. Positive slack, encoded-size and runtime bounds, complete PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | An explicit input is hidden or presented as terminal-derived; the local row is presented as the complete no-lower ledger, HResolve, BudgetResolve, normalization, named descent routing, saturation, replay, unconditional HB closure, ZeroSlack, or a complete global route system; positive slack is claimed; a circuit-size bound, polynomial generation, complete blocker route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require 120 PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
