...
> notice are [`status/FORMAL_RECONSTRUCTION_STATUS.json`](https://github.com/aisknab/pnp/blob/d77a5faf194b86fd1175065a0930fd485e16ace5/status/FORMAL_RECONSTRUCTION_STATUS.json)
> and [`docs/FORMAL_RECONSTRUCTION.md`](https://github.com/aisknab/pnp/blob/d77a5faf194b86fd1175065a0930fd485e16ace5/docs/FORMAL_RECONSTRUCTION.md).
> Claims and release records below are historical assertion-checker audit targets only.

The current canonical download is the eighty-five-page inventory-derived report. Every numbered section or
...
| Milestone earning | Check all 2,548 reviewed theorem kinds, kernel-type fingerprints, permitted Lean-standard axiom closures, absence of project axioms, and the full Lean-source digest. | Same-name type weakening, an unapproved axiom, an absent theorem, or source drift still earns a milestone. |
...
| Non-claim boundary | Confirm one hundred and five earned scoped milestones. The newest scope is the finite BN6 grouped hypergraph-packet bridge with 8 reviewed theorem pins using only `Quot.sound` and `propext`. Two global milestones, four project axioms, and five blockers remain. The survivor family, grouping, payload data, PkgC singletonization proofs, and BCEL constant-cut equation must remain explicit inputs. Their derivation from terminal candidates, the full historical BN6 and Packet selector and realizer results, a decreasing complete global route system, manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, and `P = NP` must remain absent. | The BN6 bridge is presented as constructing PkgC, deriving or grouping survivors from terminal candidates, proving the full historical BN6 or Packet result, or proving a global result; a polynomial minimizer, complete gain route, CNF-SAT decider, assumption discharge, or final theorem appears; or a blocker, premise, or axiom is hidden. |
| Canonical report | Compare both PDF aliases and both TeX aliases with the current merged-core files; require eighty-five PDF pages. | An alias differs, the old 57-page direct-claim manuscript hash returns, or report text conflicts with generated status. |
