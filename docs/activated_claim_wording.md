...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Thirty-eight narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines reach all three complete fixed clauses and traverse every remaining second-clause padding coordinate without emitting a token. `BuilderThirdClausePrefix.machine` composes the complete clause-three second-literal prefix with a selected 59-rule `Finish` appender and the existing 45-rule cursor advance behind two total symbol-preserving bridges. Every raw bitstring emits the complete third clause, advances the retained coordinate to `thirdClauseStart + 8`, retains bits equal to `encodedFormula.take (2 * (FormulaWidth + 27))`, and proves the executed direct schedule token is `Finish` while the retained next token is padding. Its literal table has `3126` plus eight inherited/generated unary-evaluator rule counts; its external compiled bound is `BuilderThirdClauseSecondLiteralPrefix.rawTimeBound + 498 + 24 * inputLength + 12 * FormulaWidth + 12 * BuilderThirdClauseSeparatorStep.cursorWord.length`. The 57-declaration audit has 14 empty closures, 10 using only `[propext]`, and 33 using only `[Quot.sound, propext]`, with no project or choice axiom. This executes exactly the fixed third-clause terminator and retains its first padding coordinate. It does not traverse clause-three padding, emit the remaining formula body, implement a general dynamic formula cursor, supply the complete raw builder or builder `FunctionProgram.RawRefinement`, or package a polynomial reduction. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the thirty-six-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
