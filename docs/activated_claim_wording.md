...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Ninety-nine narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone publishes `PNP.Main.locked_nand_threshold`, a uniform all-bitstring polynomial many-one reduction from concrete CNF-SAT to the concrete locked-NAND threshold language. Its witness is the composed finite parser, compiler, and emitter pipeline, and its compiled closure uses only `Quot.sound` and `propext`. It is not a polynomial-time decider for the target and does not prove CNF-SAT NP-hardness or NP-completeness, map the finite terminal routes into the complete global outcome system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, PCCMin, polynomial runtime, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
