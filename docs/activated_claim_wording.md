...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and six narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone is the finite PkgC separating-consumer restoration dichotomy. From an explicit finite minimal-consumer antichain, Lean canonically selects the first disjoint nonsingleton pair or proves exact V54 singletonization. A selected pair yields canonical exact-coordinate quotient units, and an explicit restoration universe yields complete multiplicity coverage or a strict Hall deficit with a deterministic local Q route. It does not derive the inputs from terminal candidates, connect coverage back to a BN4 or BN5 contradiction, embed the local route in the complete global outcome system, prove full PkgC route silence or the full historical PkgC result, map the finite terminal routes into a decreasing complete global outcome system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
