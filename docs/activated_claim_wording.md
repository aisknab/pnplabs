...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Twenty-six narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete unary width header, and the body-start separator. `BuilderFirstLiteralPrefix.machine` composes the body-start prefix with a structurally generated unary next-slot evaluator and two reusable appender copies. Every raw bitstring emits `T^FormulaWidth F Sep T F`, the canonical first positive literal for variable zero; `firstLiteralTokens_eq_canonical_formula_prefix` identifies the exact formula prefix, `nextTokenSlot_eq_formulaVariableSlotBound_add_four` retains the following coordinate, and an external `NatPolynomial` bounds compiled execution. All 73 public declarations are completely axiom-audited; all 52 reviewed theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. The dynamic cursor controller, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged polynomial reduction remain absent. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the twenty-one-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
