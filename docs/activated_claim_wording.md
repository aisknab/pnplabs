...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and fourteen narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone gives every canonical input-relative selector handle in an arbitrary finite explicit grouped BN6 family a unary bitstring whose total decoder rejects missing delimiters, trailing data, and out-of-range indices. Lean proves round trip, injectivity, canonical successful decoding, exact and explicit-list-bounded length, unique accepted codes, and retained payload, carrier, size, cell, and atom evidence. The grouped family remains proof-bearing input and its size is not bounded by encoded circuit size. The milestone does not prove polynomial enumeration or runtime, encode atom or payload data, establish manuscript-level selector faithfulness or compatibility, construct a selector realizer or route, derive or group BN6 survivors from terminal data, complete PkgC, embed outcomes in the complete global route system, map finite terminal routes into a decreasing complete global outcome system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
