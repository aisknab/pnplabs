...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Thirty-five narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines reach the complete first and second clauses, traverse every remaining second-clause padding coordinate without emitting a token, and then emit the separator beginning clause three. `BuilderThirdClauseSeparatorStep.machine` composes the complete clause-two padding run with the reused selected 59-rule `Sep` appender, two total nine-symbol bridges, and the existing fixed 45-rule cursor advance. Every raw bitstring emits the canonical token prefix through that `Sep`, advances the retained coordinate by one, retains bits equal to `encodedFormula.take (2 * (FormulaWidth + 20))`, and proves direct schedule lookup at the following coordinate is `F`. Its literal table has `2272` plus eight inherited/generated unary-evaluator rule counts; its external compiled bound is `BuilderSecondClausePaddingRun.rawTimeBound + 330 + 24 * inputLength + 12 * FormulaWidth + 12 * cursorWord.length`. The 56-declaration audit covers 48 new public declarations and eight reviewed suffix interfaces: 14 have empty closure, 11 use only `[propext]`, and 31 use only `[Quot.sound, propext]`, with no project or choice axiom. This executes exactly one populated transition beginning clause three. It does not emit the following `F` or remaining formula body, implement a general dynamic formula cursor, supply the complete raw builder or builder `FunctionProgram.RawRefinement`, or package a polynomial reduction. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the thirty-three-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
