...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Eighty-five narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include the concrete machine, Cook-Levin, locked-NAND, strict-codec, parser/emitter, polynomial-reduction, residual-chain, terminal full-carrier, mode-firewall, exhaustive projection-minimum, projection-transfer, terminal-saturation, terminal physical-support, terminal support-extraction, governed proper-positive support-search, saturated support-square, governed support-completion, governed frontier-pushout, and governed projection-square results described by the generated publication map. The newest milestone proves that, for every finite direct-wire candidate, explicit terminal dependency system, computed saturated support square, and forgetful terminal projection, the exact physical frontier is retained, all ten role profiles are filtered exactly, and projection commutes with both the shared meet and the side-only frontier pushout. The dependency system remains explicit rather than derived from the circuit. This structural commutation result is not side-tight four-corner minima, BN2 square legitimacy, `SaturatePositive`, Package E, BCEL/BN2-BN6, complete obstruction routing, a complete residual route, `ZeroSlack`, PCCMin, polynomial runtime, SAT in P, assumption discharge, or `P = NP`. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
