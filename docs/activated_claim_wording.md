...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Eighty-two narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include the concrete machine, Cook–Levin, locked-NAND, strict-codec, parser/emitter, polynomial-reduction, residual-chain, terminal full-carrier, mode-firewall, exhaustive projection-minimum, projection-transfer, terminal-saturation, terminal physical-support, terminal support-extraction, governed proper-positive support-search, and saturated support-square results described by the generated publication map. The newest milestone proves that, for every finite direct-wire candidate, explicit terminal dependency system, and pair of finite terminal seeds, Lean computes saturated left and right supports, their canonical closed meet, and their closed saturated-union join. It proves exact greatest-lower-bound and least-upper-bound laws, seed extensionality, computed physical compatibility, exact gate count, open-support semantics, and whole-circuit recovery for all four corners. The dependency system remains explicit rather than derived from the circuit. This finite closed-corner algebra is not the manuscript's obstruction routing, frontier pushout, projection-compatible square, side-tight four-corner minima, BN2 square legitimacy, `SaturatePositive`, Package E, BCEL/BN2-BN6, complete residual route, `ZeroSlack`, PCCMin, polynomial runtime, SAT in P, assumption discharge, or `P = NP`. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the seventy-five-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
