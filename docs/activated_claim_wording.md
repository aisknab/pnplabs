...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Ninety-three narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone proves that, for every finite direct-wire candidate, explicit terminal dependency system, computed governed proper-positive support, forgetful projection, executable ambient observer, and positive whole-support projection defect, Lean computes the canonical minimum-cardinality positive anchor nucleus and returns either an insufficient nucleus, the exact first anchor-algebra mismatch, the exact first proper-cut defect mismatch, the first proof-bearing full-before-quotient local route, or exact constant-cut and local BN2 conclusions for every proper cut. This result assumes the dependency system and positive whole-support projection defect. It does not derive either premise, identify manuscript activation or charge equivalence classes, connect each local failure to the complete global route system, establish `SaturatePositive`, Package E, BCELReady or later BCEL/BN2-BN6 conclusions, complete obstruction routing or a residual route, prove `ZeroSlack`, PCCMin, polynomial runtime, SAT in P, assumption discharge, or `P = NP`. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
