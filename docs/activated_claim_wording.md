...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Twenty-three narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. A fixed 19-rule work machine proves the local input-length tally stage, while a reusable fixed 59-rule machine appends any selected token. `BuilderFirstTokenPrefix.machine` contains all 116 executable input-prefix rules, nine total symbol-preserving bridge rules, and all 59 appender rules in one collision-free 184-rule table. Every raw bitstring reaches the exact preserved workspace with output `[CNFToken.t]`; the two emitted bits equal `encodedFormula.take 2`, and the compiled raw bound is `18*n*n + 87*n + 147`. Prefix-endpoint, malformed tally/output, and one-step-short configurations fail closed. Its 25 reviewed publication theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. The remaining width header, dynamic cursor controller, complete raw formula builder, builder `FunctionProgram.RawRefinement`, and packaged polynomial reduction remain absent. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the eighteen-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
