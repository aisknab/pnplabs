...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Thirty-six narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines reach the complete first and second clauses, traverse every remaining second-clause padding coordinate without emitting a token, emit the separator beginning clause three, and then emit the complete first negative literal of clause three. `BuilderThirdClauseFirstLiteralPrefix.machine` composes the complete third-clause separator prefix with the reused 235-rule two-`F` appender/cursor suffix behind one total nine-symbol bridge. Every raw bitstring emits the canonical token prefix through the negative literal on variable zero, advances the retained coordinate to `thirdClauseStart + 3`, retains bits equal to `encodedFormula.take (2 * (FormulaWidth + 22))`, and proves the literal sign, unary-zero terminator, and following direct schedule token are all `F`. Its literal table has `2516` plus eight inherited/generated unary-evaluator rule counts; its external compiled bound is `BuilderThirdClauseSeparatorStep.rawTimeBound + 732 + 48 * inputLength + 24 * FormulaWidth + 24 * cursorWord.length`. The 87-declaration/interface audit has 24 empty closures, 18 using only `[propext]`, and 45 using only `[Quot.sound, propext]`, with no project or choice axiom. This executes exactly the fixed first negative literal of clause three. It does not emit the following `F`, complete clause three, emit the remaining formula body, implement a general dynamic formula cursor, supply the complete raw builder or builder `FunctionProgram.RawRefinement`, or package a polynomial reduction. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the thirty-four-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
