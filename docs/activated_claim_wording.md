...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. 161 narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. This is formal artefact coverage of 161 of 163 current scoped rows, not proof completion. The risk-weighted proof-completion estimate is 30%, with a current uncertainty range of 20% to 40%. It is not the probability that the route is correct and not a time estimate. The newest milestone is the finite BCEL and Packet activation-coherence obstruction. It recomputes the selected nucleus defect, checks the Packet cut value and every canonical nonempty proper-cut activation, and returns a proof-bearing mismatch because full numerical coherence conflicts with the existing same-family Packet exclusion. This is a diagnostic, not a proof or construction of activation coherence. The terminal problem, positive full-slack premise, family, map, activation cells and masses, payloads, budget, realizer and dependency tables, and rank data remain supplied, and exhaustive cut enumeration may be exponential. It does not construct the remaining global data, close ZeroSlack or PCCMin, establish polynomial runtime, put SAT in P, or prove P = NP. Four project-specific axioms and all five global proof gates remain open. The concrete target is present but inactive, `PNP.Main.p_eq_np` is absent, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
