...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and ten narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone is a finite PkgC ambient-BN4 residual reduction. From an explicit ambient ledger, typed restorer, exact embedding, and explicit remainder, Lean preserves the executable residual at every complete key and makes the complete canonical executable residual ledger equal the remainder. Every remainder key is covered, a fail-closed classifier constructs the reduction without caller-provided proof bits, and an empty remainder yields an empty ambient residual ledger. Those inputs remain proof-bearing and are not derived from an arbitrary terminal candidate, and the remainder is not proved empty or route-producing. The result does not embed outcomes in the complete global route system, prove full PkgC route integration or silence, map finite terminal routes into a decreasing complete global outcome system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
