...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Thirty-three narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines reach the complete first clause and both clause-two literals. `BuilderSecondClausePrefix.machine` extends that prefix with one selected 59-rule `Finish` appender, two total symbol-preserving bridges, and one copy of the fixed 45-rule cursor advance. Every raw bitstring emits the complete second clause, retains its first padding coordinate, and emits bits equal to `encodedFormula.take (2 * (FormulaWidth + 19))`. Its literal table has `2098` plus six inherited/generated unary-evaluator rule counts, and the external compiled bound is `BuilderSecondClauseSecondLiteralPrefix.rawTimeBound + 390 + 24*n + 12*FormulaWidth + 12*cursorWord.length`. The 57-declaration audit covers 55 new public declarations and two reviewed cursor dead-loop facts: 15 have empty closure, 10 use only `[propext]`, and 32 use only `[Quot.sound, propext]`, with no project or choice axiom. This is one fixed clause prefix, not a general dynamic formula cursor. It does not traverse clause-two padding, reach clause three, or emit the remaining formula body; the complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged polynomial reduction remain absent. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the thirty-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
