...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Twenty-seven narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete unary width header, and the body-start separator. `BuilderFirstClausePrefix.machine` composes the complete first-literal prefix with a structurally generated unary next-slot evaluator and a fixed eight-token tail. Every raw bitstring emits `T^FormulaWidth F Sep T F T T F T T T F Finish`, the canonical formula prefix through the complete positive first clause on variables zero, one, and two; `firstClauseTokens_eq_canonical_formula_prefix` identifies the exact formula prefix, `nextTokenSlot_eq_formulaVariableSlotBound_add_twelve` retains the following coordinate, and an external `NatPolynomial` bounds compiled execution. All 77 public declarations are completely axiom-audited, and the combined 78-declaration audit includes the predecessor halt-separation theorem; all 41 reviewed theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. The retained coordinate is not a dynamic cursor. The remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged polynomial reduction remain absent. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the twenty-two-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
