...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Thirty narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include universal concrete CNF-SAT verifier correctness, exact accept/reject behavior, no timeout at a polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include literal raw-machine compilation, exact Cook-Levin semantics through `encodedFormula_mem_CNFSAT_iff_language`, the external encoded-size bound `encodedFormula_size_le`, an answer-independent rectangular formula schedule, and a direct formula cursor. Fixed finite machines reach the complete first-clause prefix and execute its remaining padding run. `BuilderSecondClauseSeparatorStep.machine` then composes that run with a selected 59-rule `Sep` appender, two total nine-symbol bridges, and the existing fixed 45-rule cursor advance. Every raw bitstring emits the canonical separator beginning clause two, advances the retained unary coordinate by one, proves the following direct token is `F`, and emits bits equal to `encodedFormula.take (2 * (FormulaWidth + 13))`. Its literal table has `1366` plus six inherited/generated unary-evaluator rule counts, and the external compiled bound is `BuilderFirstClausePaddingRun.rawTimeBound + 246 + 24*n + 12*FormulaWidth + 12*cursorWord.length`. The combined 56-declaration audit covers 54 new declarations and two predecessor dispatch facts; every audited declaration has empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. This is one fixed populated transition, not a general dynamic formula cursor. The following `F`, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged polynomial reduction remain absent. The results do not prove CNF-SAT NP-completeness, CNF-SAT in P, or `P = NP`. Global locked-NAND construction, premise instantiation, candidate-universe completeness, ZeroSlack, PCCMin, the residual-band minimizer, polynomiality, and `PNP.Main.p_eq_np` remain unfinished. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the twenty-seven-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
