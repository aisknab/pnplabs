...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and thirty-eight narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone computes the canonical Packet activation field from equality of the nested BN4 activation atoms in explicit typed source and selector terminal BN5 coordinates while retaining grouped colour, positive charge, the canonical source route, authoritative handle rank, and exact residual descent computation from the ten-coordinate RankWF comparison. It proves that atom equality is equivalent to equality of the canonical BN4 activation predicates on every cut. An activation route is prior frontier and obligation equality with activation-atom inequality, colour, charge, exactRoute, and rank remain impossible, and the positive Packet/HB endpoint carries exact failure evidence without route-clear or binding premises. The BN5 coordinates, finite rank map, before/after ranks, direction, budget, grouped family, realizer claims, blocker activity, and dependency rows remain proof-bearing inputs rather than terminal-derived constructions. The milestone does not prove the manuscript's complete BN4, BN5, or Packet adequacy bridge, the two remaining routes' external manuscript semantics or decreasing complete global coverage, existence of a decreasing transition, a no-lower ledger, full external selector compatibility or complete route silence, blocker semantics or semantic dependency completeness, unconditional `HB.NegativeClosure` or `ZeroSlack`, PCCMin or polynomial runtime, complete global PkgC, manuscript-wide `SaturatePositive`, Package E, BCELReady, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the 114-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
