...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Ninety-five narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone proves that, for every finite direct-wire candidate, executable ambient observer, forgetful projection, and finite terminal seed, Lean derives the candidate-specific dependency system and deterministic rule-labelled saturation trace. It then proves exact support and full-circuit cost balance throughout a transparent history, with preserved slack, positivity, and nondecreasing projection defect, or records the exact first nontransparent event and complete transparent prefix. Together with the preceding positivity firewall, this closes the finite terminal forms of `projectionPositivityNotLostSilently`, `transparentSaturationCostBalanced`, and `firstNontransparentStepRecorded`. The observer and projection remain explicit inputs, and a nontransparent event is recorded rather than routed. The result does not discharge `interfaceExposureRoutesToE` or `originKernelObligationClosureRouted`; identify manuscript activation or charge equivalence classes; connect each local failure to the complete global route system; establish full `SaturatePositive`, Package E, BCELReady or later BCEL/BN2-BN6 conclusions; complete obstruction routing or a residual route; prove `ZeroSlack`, PCCMin, polynomial runtime, SAT in P, assumption discharge, or `P = NP`. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the eighty-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
