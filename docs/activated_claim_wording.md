...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and eighteen narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone accepts an explicit proof-bearing global gain-coverage certificate for an arbitrary finite explicit grouped BN6 family whose payloads are direct-wire implementations. Under precisely that premise, the exhaustive Packet scan returns a canonical source-atom strict equivalent gain with proved residual-slack descent or a proof-bearing semantic minimum and zero residual slack, while every Packet branch remains literal. The certificate, candidate implementations, and grouped family remain proof-bearing inputs, and the certificate is not derived from terminal data. The milestone does not prove selector faithfulness or compatibility, construct or polynomially enumerate replacement candidates, establish encoded-size or runtime bounds, produce typed blockers or HB/rank closure, complete global PkgC, embed outcomes in a decreasing complete global route system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
...
Every such surface must be labelled historical or superseded. The current canonical report is instead the ninety-four-page inventory-derived non-claiming report. The activated-run intake is frozen. No green matrix badge is a current status signal.
