...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. One hundred and nineteen narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. The newest milestone accepts arbitrary finite support and replacement charge ledgers with exact multiplicity-preserving occurrence pairing, pairwise weight preservation, and one unmatched positive support charge. It derives strict occurrence count and strict total replacement weight, then uses exact NAND-gate accounting and separately proved semantic equivalence to construct a genuine strict equivalent gain and strict residual descent without assuming either strict inequality. The ledgers, pairing, witness, accounting, and semantics remain proof-bearing inputs. The milestone does not construct a replacement or its ledger from Packet or terminal data, prove selector faithfulness or compatibility, establish encoded-size or runtime bounds, produce typed blockers or HB/rank closure, complete global PkgC, embed outcomes in a decreasing complete global route system, establish manuscript-wide `SaturatePositive`, Package E, BCELReady, unconditional `ZeroSlack`, polynomial PCCMin, CNF-SAT in P, or `P = NP`. Four project-specific axioms and five formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
