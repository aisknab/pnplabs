...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root and exports a canonical theorem inventory. Seventy-seven narrowly scoped milestones are earned from reviewed theorem types, approved axiom closures, and the complete Lean-source digest. They include the concrete machine, Cook–Levin, locked-NAND, strict-codec, parser/emitter, polynomial-reduction, residual-chain, terminal full-carrier, mode-firewall, and exhaustive projection-minimum results described by the generated publication map. The newest milestone proves the exact signed Section 5.2 transfer identity for four supplied terminal-profile corners that share one computed observer and one explicit projection. It identifies projection excess with the quotient delta minus the full delta and derives the manuscript's constant-cut equation: when the meet and both side defects vanish while the join defect is `D`, the excess is exactly `D` and is positive whenever `D` is positive. This is arithmetic over four supplied corners; it does not construct or certify a proper governed support square, prove `SaturatePositive`, discharge Package E or BCEL/BN2–BN6, generate a complete residual route, prove `ZeroSlack` or PCCMin, establish polynomial runtime, put SAT in P, remove a project assumption, or prove `P = NP`. Four project-specific axioms and six formal blockers remain. The concrete target is present but inactive, the publication gate is false, and the abstract string-handle `PNP.PEqualsNP` bridge is publication-ineligible.
