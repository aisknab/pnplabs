...
Its 12 reviewed theorem pins use only empty closure, `propext`, or `Quot.sound` with `propext`; the focused 20-declaration audit has 3 empty, 2 `propext`-only, and 15 `Quot.sound` plus `propext` declarations, with no project axiom.
