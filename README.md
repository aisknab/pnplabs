...
Its 19 reviewed theorem pins have 17 `propext`-only closures and two using `Quot.sound` with `propext`. The focused 27-declaration audit has one empty closure, 24 `propext`-only closures, and two `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 83b2a786c1960e15d193404eb7a5eef14fd0cb89
npm ci
lake build PNP
npm run check
