...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,498** exported public declarations across **309** modules, including **15,649** theorem-kind declarations and **7,690** assumption-free theorem-kind declarations. It excludes **15,151** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,133 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 169 of 171 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, proof-bearing normalization/oracle composition, rank-ordered proof-bearing PCCOracle orchestration, the checked Packet-backed rank-selector construction, and the checked Packet/HB conditional ZeroSlack bridge. The newest row derives complete canonical selector silence from checked Packet claims, exact-rank silence, executable selector-silence induction, and checked HB no-outcome closure, then uses one explicit positive-residual-slack-to-faithful-selector premise to obtain conditional ZeroSlack. It does not derive that positive-slack bridge, the grouped terminal family, rank assignment, claim table, resolvers, normalizer, or blocker semantics from arbitrary input; prove unconditional ZeroSlack or encoded-size polynomial runtime; put CNFSAT in P; create the eligible root theorem; or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-25-193` it reports two separate metrics:

- Formal artefact coverage: **169 of 171** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M193 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint because the terminal family, rank assignment, claim table, normalizer, resolvers, blocker semantics, and positive-slack bridge remain supplied. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Checked Packet/HB conditional ZeroSlack bridge**

For every finite direct-wire implementation and supplied checked Packet/HB table, Lean now derives that every canonical handle is nonfaithful from complete checked claims, exact-rank silence, executable selector-silence induction, and checked HB no-outcome closure. One explicit positive-residual-slack-to-faithful-selector implication then yields conditional ZeroSlack, and the adapter reuses the M192 selector construction and checked well-founded exact loop.

The positive-slack bridge, grouped terminal family, rank assignment, claim table, HResolve and BudgetResolve algorithms, normalizer, blocker semantics, and encoded-size bounds remain supplied, and the exhaustive reference fixture is not polynomial. This is not a terminal-data construction, unconditional ZeroSlack, an encoded-size polynomial PCCMin theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 169 of 171 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.
...
cd pnp
git checkout 9f2e7f022d1483702a1ead06392ac2dafe86120b
npm ci
