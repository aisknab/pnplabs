...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **6,849** exported public declarations across **62** modules, including **3,104** theorem-kind declarations and **2,560** assumption-free theorem-kind declarations. It excludes **1,220** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 309 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects; it preserves distinct out-of-range, valid-padding, and populated states and proves exact prefix, full, one-step-short, terminal, and excess-fuel traversal. A separate fixed 19-rule work machine is definitionally connected to the total input framer, preserves every source bit, and appends an exact unary input-length tally. Its exact cost is `2*n*n + 4*n + 2` work steps and `12*n*n + 24*n + 12` compiled raw steps; malformed internal scan symbols and one-step-short fuel time out. Its ten reviewed publication theorem types have either empty axiom closure or `[Quot.sound, propext]`, with no project or choice axiom. This new stage emits no formula bits and is not a complete raw formula builder, `FunctionProgram.RawRefinement`, or packaged `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 7c831a37844deaef9036ceb97205bd04d0556d21
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current fourteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, and the literal 19-rule input-length tally machine with exact work/raw polynomials. They explicitly withhold formula-bit emission from that machine, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
