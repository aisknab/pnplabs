...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **9,474** exported public declarations across **84** modules, including **5,036** theorem-kind declarations and **3,178** assumption-free theorem-kind declarations. It excludes **3,398** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1221 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Forty-one scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and three complete fixed clauses. `BuilderFourthClauseFirstLiteralPrefix.machine` composes the complete fourth-clause separator prefix with the reused 357-rule `F T F` appender/cursor suffix through one outer total nine-symbol bridge. Its literal table has `3666` plus ten inherited/generated unary-evaluator rule counts. Every raw bitstring emits the complete first negative literal on variable one in clause four, preserves `encodedFormula.take (2 * (FormulaWidth + 31))`, and advances the retained coordinate to `FormulaVariableSlotBound + 1 + 3 * FormulaTokensPerClause + 4`, whose direct next token is `F`. Its compiled run is bounded by `BuilderFourthClauseSeparatorStep.rawTimeBound + 1422 + 72*n + 36*FormulaWidth + 36*cursorWord.length`. The combined 115-declaration audit covers all 97 new public declarations, 16 reviewed reused suffix interfaces, and two cursor dead-state facts using only the approved Lean-standard closure, with no project axiom or `Classical.choice`. All six unlaunched endpoints, malformed appender tally/output and cursor scratch in all three token stages, and one-step-short fuel time out. This emits only the fixed first negative literal in clause four. It observes but does not emit the following `F`, does not complete clause four, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 3112ca90d74d58116dc53ea5300082d0caa63c0e
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current forty-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, all three complete fixed clauses, exact clause-two and clause-three padding traversal, the fourth-clause separator, and the complete first negative literal in clause four. They explicitly withhold emission of the following `F`, completion of clause four, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
