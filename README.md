...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **8,811** exported public declarations across **79** modules, including **4,530** theorem-kind declarations and **3,062** assumption-free theorem-kind declarations. It excludes **2,974** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 960 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-six scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and both complete fixed clauses. `BuilderThirdClauseFirstLiteralPrefix.machine` composes the complete third-clause separator prefix with the reused 235-rule two-`F` appender/cursor suffix behind one total nine-symbol bridge. Its table has `2516` plus eight inherited/generated unary-evaluator rule counts. Every raw bitstring emits the complete negative literal on variable zero in clause three, advances the retained unary coordinate to `thirdClauseStart + 3`, proves the emitted bits equal `encodedFormula.take (2 * (FormulaWidth + 22))`, and proves the literal sign, unary-zero terminator, and following direct schedule token are all `F`. Its compiled run is bounded by `BuilderThirdClauseSeparatorStep.rawTimeBound + 732 + 48*n + 24*FormulaWidth + 24*cursorWord.length`. All 87 audited declarations and interfaces use only the approved Lean-standard closure: 24 have empty closure, 18 use only `[propext]`, and 45 use only `[Quot.sound, propext]`. This is exactly the first fixed negative literal in clause three. It does not emit the following `F`, complete clause three, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 9db71188ff4dafbefc09f5f0120fdf37c229bb4d
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-four-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, both complete fixed clauses, exact clause-two padding traversal, and the complete negative literal on variable zero in clause three. They explicitly withhold emission of the following `F`, completion of clause three, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
