...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **25,099** exported public declarations across **231** modules, including **13,423** theorem-kind declarations and **7,025** assumption-free theorem-kind declarations. It excludes **14,705** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,397 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Ninety-two scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, exact open-support extraction, governed proper-positive support search, saturated support-square closure, governed completion of every saturated square corner, exact governed frontier gluing, exact commutation of that structure with every forgetful terminal projection, side-tight four-corner minimum arithmetic, checked structural carrier transport, compatibility of independently attained four-corner optima with one reversible ambient carrier and shared observer projection, deterministic classification of coherent optimum transport or its exact first failure, conditional side-tight coherent completion, the complete finite tight-basis maximum under exact local route silence, and computed terminal BN2 square legitimacy for explicit finite dependency data.

The newest milestone works for every finite square computed from two seed lists under one explicit terminal dependency system, direct-wire candidate, observer, and forgetful projection. Lean constructs the exact compatible governed frontier and projection square, keeps full and quotient minimum quantities on one carrier, and returns either the complete local conclusion under exact route silence or the deterministic full-then-quotient proof-bearing first route.

This is a bounded mathematical step, not the final theorem. It packages computed local structural legitimacy, not the manuscript's unrestricted square-legitimacy conclusion. It does not derive the terminal dependency system from an arbitrary circuit, prove universal route silence, connect every local failure to the complete global no-outcome route system, identify a BCEL anchor square, formalize complete obstruction routing, establish SaturatePositive, complete residual routes, prove `ZeroSlack`, prove PCCMin exactness or polynomial runtime, discharge an assumption, place CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout ca60f66498eaa4a6242e15c86fa86a2fe62e78f9
npm ci
lake build PNP
