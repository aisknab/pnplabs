...
Its 16 reviewed theorem pins have 14 `propext`-only closures and two using `Quot.sound` with `propext`. The focused 24-declaration audit has one empty closure, 21 `propext`-only closures, and two `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 26547c1a3c05de0c907021320352a46a8145dbaa
npm ci
lake build PNP
npm run check
