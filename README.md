...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,075** exported public declarations across **305** modules, including **15,543** theorem-kind declarations and **7,642** assumption-free theorem-kind declarations. It excludes **15,145** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,129 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 165 of 167 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, and the proof-bearing PCCMin total-oracle loop. The newest row proves a terminating, semantics-preserving minimisation loop for every finite direct-wire implementation under an explicit total oracle, including exact-minimum and zero-residual-slack output facts and a strict-gain iteration bound. It does not construct that oracle, derive terminal routes, prove unconditional ZeroSlack or encoded-size polynomial runtime, put CNFSAT in P, create the eligible root theorem, or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-24-189` it reports two separate metrics:

- Formal artefact coverage: **165 of 167** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 adds formal artefact coverage but does not close the fixed `pccmin-executable-loop` checkpoint because its oracle remains supplied. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Proof-bearing PCCMin total-oracle loop**

For every finite direct-wire implementation and explicit proof-bearing total oracle, Lean now runs transparent well-founded recursion through strict semantics-preserving gains until the oracle supplies an exact-minimum or ZeroSlack terminal result. The loop preserves semantics, returns a global minimum with zero residual slack, and bounds strict-gain iterations by the starting residual slack.

The total oracle remains an explicit proof-bearing argument, and the regression fixture constructs it with exhaustive reference minimisation. This is not a uniform executable PCCMin oracle, a terminal-family or route derivation, unconditional ZeroSlack, a raw-machine implementation, an encoded-size polynomial construction or runtime theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 165 of 167 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin and all eight declarations in the focused audit are axiom-free.
