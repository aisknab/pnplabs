...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,909** exported public declarations across **316** modules, including **15,792** theorem-kind declarations and **7,690** assumption-free theorem-kind declarations. It excludes **15,171** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,138 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 174 of 176 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, proof-bearing normalization/oracle composition, rank-ordered proof-bearing PCCOracle orchestration, the checked Packet-backed rank-selector construction, the checked Packet/HB conditional ZeroSlack bridge, the BN6 computed-faithfulness bridge, the same-candidate BCEL activation-route classifier, the BCEL-derived BN6 family skeleton, canonical positive-cell grouping, and exact raw cut-ledger conservation through that grouping. The newest row proves that canonical duplicate-footprint coalescing preserves the exact direct raw crossing-mass sum on every cut and exposes the remaining activation mismatch directly against the supplied raw ledger. It does not derive the terminal problem, positive premise, checked finite BCEL-ready certificate, raw positive cells or payloads, tables, ranks, route-clear proof, resolvers, normalizer, or blocker semantics from arbitrary input; turn the raw-ledger mismatch into a constructed gain or derive constant activation; prove manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, or encoded-size polynomial runtime; put CNFSAT in P; create the eligible root theorem; or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-27-198` it reports two separate metrics:

- Formal artefact coverage: **174 of 176** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
cd pnp
git checkout ebebf9356116e3b8dca9fbae6999826c026fa18b
npm ci
