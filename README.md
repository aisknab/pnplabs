...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,056** exported public declarations across **302** modules, including **15,507** theorem-kind declarations and **7,617** assumption-free theorem-kind declarations. It excludes **15,141** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 3,114 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 161 of 163 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, and the new finite activation-coherence obstruction. The newest row computes the selected nucleus defect, checks the Packet cut value and every canonical nonempty proper cut, and returns a proof-bearing mismatch because full numerical coherence conflicts with the same-family Packet exclusion. The terminal problem, positive full-slack premise, family, map, activation data, and downstream certificates remain supplied, and exhaustive cut enumeration may be exponential. This is a diagnostic, not a construction of the missing numerical bridge, unconditional ZeroSlack, PCCMin, polynomial runtime, SAT in P, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-23-185` it reports two separate metrics:

- Formal artefact coverage: **161 of 163** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
This milestone starts from the accepted finite carrier-coherence certificate, computes the exact selected nucleus defect, and compares it with the declared Packet cut value and every canonical nonempty proper Packet-cut activation. Acceptance reconstructs full finite activation coherence and maps each cut equation to projection excess. The existing same-family Packet exclusion proves acceptance impossible, and a deterministic classifier returns either the declared total mismatch or the first proof-bearing cut mismatch.

The result proves that numerical coherence fails on the current supplied finite family; it does not prove or construct coherence. The terminal problem, positive full-slack premise, family, map, activation cells and masses, payloads, budget, realizer and dependency tables, and rank data remain supplied, and exhaustive cut enumeration may be exponential. It does not construct this data from every input, complete global routing or the no-lower system, prove unconditional SaturatePositive, BCELReady, ZeroSlack, PCCMin or polynomial runtime, put SAT in P, or prove P = NP. The canonical tracker keeps 161 of 163 formal artefact rows separate from the unchanged 30% fixed-weight proof-completion estimate.

Its 6 reviewed theorem pins, and all twelve declarations in the focused audit, use only Lean-standard `Quot.sound` and `propext`, with no project axiom or `Classical.choice`.
