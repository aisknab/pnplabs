...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **24,464** exported public declarations across **223** modules, including **13,166** theorem-kind declarations and **6,953** assumption-free theorem-kind declarations. It excludes **14,594** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,263 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Eighty-four scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, exact open-support extraction, governed proper-positive support search, saturated support-square closure, governed completion of every saturated square corner, and exact governed frontier gluing for the two sides of every computed square.

The newest governed frontier-pushout milestone works for every finite direct-wire candidate, explicitly supplied terminal dependency system, and computed saturated support square. Lean constructs the governed boundary, interface, and role-preserving profile gluing from the two side completions alone. It proves that the independently completed join frontier is exactly this gluing, the meet profile is the exact shared overlap, and every side physical coordinate is retained externally or witnessed as internalized.

This is a bounded mathematical step, not the final theorem. The dependency system remains explicit rather than derived from the circuit. The result proves exact gluing for computed saturated support squares, not the manuscript's projection compatibility, side-tight four-corner minima, square legitimacy, complete obstruction routing, complete residual route, `ZeroSlack`, PCCMin exactness or polynomial runtime, assumption discharge, CNF-SAT in P, or `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 31937a36ecc21413e334e5e7f7f27058f0dfecc7
npm ci
lake build PNP
