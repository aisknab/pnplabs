...
Its 5 reviewed theorem pins all use only `Quot.sound` with `propext`. The focused seven-declaration audit has two `propext`-only closures and five `Quot.sound` plus `propext` closures, with no empty closure, project axiom, or `Classical.choice`.
...
cd pnp
git checkout 9f26c50f8e8bfbda9cb15a8142c6191482148220
npm ci
lake build PNP
npm run check
