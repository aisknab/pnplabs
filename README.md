...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **6,571** exported public declarations across **60** modules, including **2,964** theorem-kind declarations and **2,524** assumption-free theorem-kind declarations. It excludes **1,101** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 286 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Eighteen scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The new answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Its reviewed closure is `[Quot.sound, propext]`, with no project or choice axiom. This is still a pure schedule specification: no slot is interpreted as a constant-time raw-machine action, and a raw finite formula builder, construction-runtime polynomial, `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` remain absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout d1b0ae18d8b03968a0a8af1fe44093f48b821664
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, and the exact answer-independent rectangular formula schedule. They explicitly withhold a constant-time raw interpretation of schedule slots, a raw formula builder and construction-runtime polynomial, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
