...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **29,988** exported public declarations across **303** modules, including **15,497** theorem-kind declarations and **7,613** assumption-free theorem-kind declarations. It excludes **15,141** private compiler auxiliaries and records **three** project axioms.

The inventory-derived publication map binds 3,120 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 162 of 164 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, and the concrete report-facing locked-NAND compatibility link. The newest row makes the report-facing SAT, locked-NAND, P/NP-class, verifier, decider, and reduction interfaces exact compatibility names for the concrete finite-pipeline model and reuses the checked all-bitstring CNFSAT reduction directly. It removes one duplicate project axiom and one caller-supplied reduction edge. It does not provide a target decider, SAT NP-hardness, residual-band minimisation, unconditional ZeroSlack, PCCMin, polynomial runtime, the eligible root theorem, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-23-186` it reports two separate metrics:

- Formal artefact coverage: **162 of 164** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
- Risk-weighted proof completion estimate: **32%**, with a current uncertainty range of **20% to 40%**. This is a conservative estimate of complete proof burden retired, not the probability that the route is correct and not a time estimate.

The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 1/10 for the root theorem and axiom elimination. M186 earned exactly two fixed one-point checkpoints: final concrete SAT-to-target compatibility and removal of `PNP.LockedNANDThreshold` as an active project-specific axiom. All five global gates remain open, three project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Concrete report-facing locked-NAND compatibility**

This milestone identifies the report-facing SAT language definitionally with concrete `CNFSAT` and the report-facing locked-NAND language with concrete `EncodedLockedNANDThreshold`. The associated verifier, decider, reduction, P/NP-class, and P-equals-NP interfaces now reuse the concrete finite-pipeline definitions, and the report bridge consumes the existing checked all-bitstring polynomial reduction instead of a caller-supplied reduction field.

The result removes duplicate interface trust; it does not put the concrete locked target or CNFSAT in P. It does not prove SAT NP-hardness or CNFSAT NP-completeness transport, derive the residual-band construction, close unconditional SaturatePositive, BCELReady, ZeroSlack or PCCMin, prove the complete polynomial runtime and certificate bounds, create `PNP.Main.p_eq_np`, or prove `P = NP`. The canonical tracker keeps 162 of 164 formal artefact rows separate from the 32% fixed-weight proof-completion estimate. No global gate closes.

Its 6 reviewed theorem pins close under the Lean-standard allowlist, and the checked compatibility endpoint has no project-specific axiom in its dependency closure.
