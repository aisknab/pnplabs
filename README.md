...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,418** exported public declarations across **67** modules, including **3,473** theorem-kind declarations and **2,764** assumption-free theorem-kind declarations. It excludes **2,032** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 414 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-four scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. Fixed finite machines provide input framing, unary tallying, and reusable two-bit token appending. `BuilderFirstTokenPrefix.machine` retains its exact 184-rule first-token trace and quadratic bound. The new `BuilderCompleteHeader.machine` composes that prefix with a structurally generated unary `NatPolynomial` evaluator, a 16-rule controller, two 59-rule appender copies, and five total bridges in pairwise-disjoint state images. Its literal table has exactly `363 + BuilderUnaryPolynomial.ruleCount(widthPolynomial verifier)` rules. Every raw bitstring emits exactly `FormulaWidth` copies of `T` followed by `F`; `finalTokenBits_eq_encodedFormula_header` identifies the output bits with `encodedFormula.take (2 * (FormulaWidth + 1))`, and an external `NatPolynomial` bounds compiled execution. The evaluator's 74 and composition's 83 public declarations are completely audited; all 48 reviewed theorem types use only empty closure, `[propext]`, or `[Quot.sound, propext]`. The dynamic cursor controller, formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` are absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 62c35edef4bb64f2c665051e61d07c7b17f70ad3
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current nineteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, the literal input preparation and token-appender machines, the composed first-token prefix, the unary polynomial evaluator, and the complete width-header composition. They explicitly withhold a dynamic cursor controller, formula-body emission, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
