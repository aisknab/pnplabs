...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,348** exported public declarations across **307** modules, including **15,609** theorem-kind declarations and **7,688** assumption-free theorem-kind declarations. It excludes **15,145** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,131 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 167 of 169 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, proof-bearing normalization/oracle composition, and rank-ordered proof-bearing PCCOracle orchestration. The newest row proves the manuscript stage order over arbitrary finite canonical rank rows: HResolve precedes BudgetResolve, each selector is scanned in rank order, gains return immediately, and complete typed silence is required before the supplied ZeroSlack closure. It does not construct the normalizer, resolvers, rows, realizer, blocker semantics, or ZeroSlack proof; prove unconditional ZeroSlack or encoded-size polynomial runtime; put CNFSAT in P; create the eligible root theorem; or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-25-191` it reports two separate metrics:

- Formal artefact coverage: **167 of 169** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M191 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint because the normalizer, resolvers, selector rows, realizer, blocker semantics, and ZeroSlack closure remain supplied. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Rank-ordered proof-bearing PCCOracle orchestration**

For every finite direct-wire implementation and explicit proof-bearing rank-ordered oracle builder, Lean now enforces the manuscript's orchestration order. HResolve is tried before BudgetResolve, every selector in every canonical finite rank row is scanned, the first gain returns immediately, and ZeroSlack is admitted only after complete typed silence. The resulting total oracle plugs into the already checked normalization and well-founded exact loop.

The normalizer, HResolve and BudgetResolve algorithms, rank rows, selector universe, realizer, typed blocker semantics, and final ZeroSlack closure remain explicit proof-bearing inputs, and the exhaustive reference fixture is not polynomial. This is not a terminal-data construction, unconditional ZeroSlack, an encoded-size polynomial PCCMin theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 167 of 169 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.
...
cd pnp
git checkout 0dbad6e14d82bf1f6f1d7b315dc896bdceb325a6
npm ci
