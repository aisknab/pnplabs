...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **24,260** exported public declarations across **220** modules, including **13,074** theorem-kind declarations and **6,927** assumption-free theorem-kind declarations. It excludes **14,574** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,200 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Eighty-one scoped milestones are earned. They cover the concrete machine model, Cook–Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, and exact open-support extraction.

The newest governed proper-positive support milestone searches the complete canonical finite universe of primitive-record seeds for every finite direct-wire candidate and explicitly supplied terminal dependency system. For each seed, Lean saturates and physically completes the support, extracts its exact open semantics, and computes exact local gain from the exhaustive semantic reference minimum. The search returns a proof-bearing nonempty proper support with positive gain whenever one exists in that canonical seed universe, and returns `none` exactly when none exists there.

This is a bounded mathematical step, not the final theorem. The dependency system remains explicit rather than derived from the circuit, and the search is exhaustive rather than polynomial. It does not prove global gain completeness, full manuscript support completion or square legitimacy, a complete residual route, the manuscript's `ZeroSlack` certificate, PCCMin exactness or polynomial runtime, discharge a project assumption, put CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout bdbbabc67f20b5d4f673e11098ba86a9d10a1cf1
npm ci
lake build PNP
