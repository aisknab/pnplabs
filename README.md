...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,689** exported public declarations across **311** modules, including **15,702** theorem-kind declarations and **7,690** assumption-free theorem-kind declarations. It excludes **15,157** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,135 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 171 of 173 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, proof-bearing normalization/oracle composition, rank-ordered proof-bearing PCCOracle orchestration, the checked Packet-backed rank-selector construction, the checked Packet/HB conditional ZeroSlack bridge, the BN6 computed-faithfulness bridge, and the same-candidate BCEL activation-route classifier. The newest row removes M194's explicit constant-activation premise at that checked boundary: it returns a proof-bearing carrier, cut-value, or proper-cut activation mismatch, or derives coherent constant activation, identifies it with the BCEL projection excess, and reuses M194 for conditional ZeroSlack. It does not derive the terminal problem, positive premise, checked finite BCEL-ready certificate, grouped family, payloads, tables, ranks, route-clear proof, resolvers, normalizer, or blocker semantics from arbitrary input; turn mismatch diagnostics into constructed gains; prove manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, or encoded-size polynomial runtime; put CNFSAT in P; create the eligible root theorem; or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-26-195` it reports two separate metrics:

- Formal artefact coverage: **171 of 173** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M195 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint because the terminal family, payload and rank construction, normalizer, resolvers, blocker semantics, route-clear proof, and encoded-size polynomial construction remain supplied. The new M195 finite powerset classifier is not proved polynomial, and its mismatch diagnostics are not constructed gains. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.
...
The terminal problem, positive premise, checked finite BCEL-ready certificate, grouped Packet family, payloads, claim table, rank map, route-clear equation, dependency table, checked HB closure, HResolve and BudgetResolve algorithms, normalizer, blocker semantics, and encoded-size bounds remain supplied. The proper-cut classifier enumerates a finite powerset and is not proved polynomial. Its mismatch results are typed diagnostic routes, not constructed gains or globally decreasing transitions. This is not manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, an encoded-size polynomial PCCMin theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 171 of 173 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin and all 15 declarations in the focused audit are free of project-specific axioms. The endpoint uses only Lean-standard `Quot.sound` and `propext`.
...
cd pnp
git checkout b9523fe7ea939cd90dba5671aa627986e1f22f18
npm ci
