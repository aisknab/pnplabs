...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,626** exported public declarations across **69** modules, including **3,640** theorem-kind declarations and **2,827** assumption-free theorem-kind declarations. It excludes **2,214** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 508 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-six scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and the separator that starts the body. The new `BuilderFirstLiteralPrefix.machine` composes the body-start machine with a structurally generated unary evaluator and two appender copies. Its literal table has exactly `585 + BuilderUnaryPolynomial.ruleCount(widthPolynomial verifier) + BuilderUnaryPolynomial.ruleCount(BuilderBodyStartPrefix.nextTokenSlotPolynomial verifier) + BuilderUnaryPolynomial.ruleCount(nextTokenSlotPolynomial verifier)` rules. Every raw bitstring emits `T^FormulaWidth F Sep T F`, the canonical first positive literal for variable zero; `firstLiteralTokens_eq_canonical_formula_prefix` identifies the exact formula prefix, and `nextTokenSlot_eq_formulaVariableSlotBound_add_four` retains the following coordinate under an external `NatPolynomial` bound. All 73 public declarations are completely audited; all 52 reviewed theorem types use only empty closure, `[propext]`, or `[Quot.sound, propext]`. The dynamic cursor controller, remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` are absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 977748fa95202c86cf1235e7336e76d908a11eb1
npm ci
lake build PNP
