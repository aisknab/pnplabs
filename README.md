...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **9,117** exported public declarations across **81** modules, including **4,764** theorem-kind declarations and **3,122** assumption-free theorem-kind declarations. It excludes **3,151** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1089 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-eight scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and three complete fixed clauses. `BuilderThirdClausePrefix.machine` composes the complete clause-three second-literal prefix with a selected 59-rule `Finish` appender and the existing 45-rule cursor advance behind two total symbol-preserving bridges. Its fixed suffix has 113 rules, and its global table has `3126` plus eight inherited/generated unary-evaluator rule counts. Every raw bitstring emits the complete third clause, advances the retained unary coordinate to `thirdClauseStart + 8`, proves the emitted bits equal `encodedFormula.take (2 * (FormulaWidth + 27))`, proves the executed opportunity is `Finish`, and proves the retained next opportunity is padding. Its compiled run is bounded by `BuilderThirdClauseSecondLiteralPrefix.rawTimeBound + 498 + 24*n + 12*FormulaWidth + 12*BuilderThirdClauseSeparatorStep.cursorWord.length`. All 55 new public declarations plus two reviewed cursor dead-loop facts use only the approved Lean-standard closure: 14 have empty closure, 10 use only `[propext]`, and 33 use only `[Quot.sound, propext]`. This exactly completes the fixed third clause and retains its first padding coordinate. It does not traverse clause-three padding, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout ed439e1477e69315006fe7f8bbecac278c024bbf
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-six-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, all three complete fixed clauses, and exact clause-two padding traversal. They explicitly withhold clause-three padding traversal, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
