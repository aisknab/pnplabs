...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **31,278** exported public declarations across **321** modules, including **15,972** theorem-kind declarations and **7,705** assumption-free theorem-kind declarations. It excludes **15,185** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,141 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 177 of 179 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The newest row classifies every finite list of supplied active V54 consumer systems over one common carrier into the first exact same-key PkgC cancellation realization or an all-singletonized branch. The latter derives raw BN6 positive-cell supports and support-size facts from active consumer data, preserves payload order, and conserves exact activation weight on every cut. The terminal problem, source systems, active cuts, positive payload atoms, typed restorer, upstream BN3 through BN5 construction, checked BCEL/Packet integration, and complete encoded-size bounds remain supplied or open. The result does not turn a returned cancellation into a verified global gain or rank-decreasing transition, and it does not establish unconditional ZeroSlack, executable polynomial PCCMin, CNFSAT in P, the eligible root theorem, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-27-201` it reports two separate metrics:

- Formal artefact coverage: **177 of 179** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M201 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint. M199 removes one endpoint's proper-cut powerset scan, M200 supplies a uniform duplicate-free quadratic test family and exact first-mismatch classifier, and M201 derives raw BN6 positive cells from supplied active V54 consumers after classifying exact PkgC cancellation versus singletonization. None constructs the complete terminal-derived upstream inputs, establishes a decreasing global route, or proves complete encoded-size polynomial construction and runtime. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**PkgC same-key cancellation or derived BN6 positive cellization**

For every finite list of supplied active V54 consumer systems over one common carrier and a supplied typed restorer, Lean returns the first exact same-key PkgC cancellation realization or proves all systems singletonized. The latter branch derives each raw BN6 positive-cell support and support-size fact from its active consumer data, preserves payload order, and proves exact activation-weight conservation on every cut.

The terminal problem, source V54 consumer systems, active cuts, positive payload atoms, typed restorer, upstream BN3 through BN5 construction, checked BCEL/Packet integration, and complete encoded-size bounds remain supplied or open. A returned cancellation is exact proof-bearing PkgC evidence, not a verified global gain or rank-decreasing transition. This is not complete PkgC through BN6 integration, manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, an encoded-size polynomial PCCMin theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 177 of 179 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin and all 19 declarations in the focused audit are free of project-specific axioms. The endpoint uses only Lean-standard `Quot.sound` and `propext`.
