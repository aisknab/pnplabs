...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,054** exported public declarations across **64** modules, including **3,249** theorem-kind declarations and **2,636** assumption-free theorem-kind declarations. It excludes **1,333** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 365 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-three scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. A fixed 19-rule tally machine proves the local input-length stage, and the literal `BuilderInputPrefix.machine` composes that tally with the all-input framer. The reusable 59-rule `BuilderTokenAppender.machine` appends any selected token exactly. The new `BuilderFirstTokenPrefix.machine` places all 116 prefix rules, nine symbol-preserving bridge rules, and all 59 appender rules in one collision-free 184-rule table. Every raw bitstring reaches the exact preserved workspace with output `[CNFToken.t]`; the two emitted bits equal `encodedFormula.take 2`, and compiled execution is bounded by `18*n*n + 87*n + 147`. Malformed prefix/appender tally or output phases, the pre-launch prefix endpoint, and one-step-short fuel time out. All 37 public declarations are audited; its 25 reviewed theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. The remaining width header, dynamic cursor controller, complete raw formula builder, builder `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` are absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout a09062d02e0c06704f57efe1bdb99024fbcfc618
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current eighteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, the literal input-length tally machine, the independently audited token appender, and the composed 184-rule first-token prefix with raw bound `18*n*n + 87*n + 147`. They explicitly withhold the remaining width header, a dynamic cursor controller, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
