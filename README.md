...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **12,138** exported public declarations across **104** modules, including **7,074** theorem-kind declarations and **3,654** assumption-free theorem-kind declarations. It excludes **4,532** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1,952 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Sixty-one scoped milestones are earned. In addition to the concrete CNF-SAT verifier, raw-machine compilers, and bounded Cook-Levin formula-building prefix, `PNP.DirectWire.LockedNANDTrace` now defines an exact `X/T/O/R/L/z` carrier for every finite topologically ordered NAND circuit. Its width is the number of inputs plus six slots per gate plus one fresh final lock coordinate, and its distinguished trace contains exactly three checks per gate. `tracePredicate_coherentExtension` constructs a coherent trace from ordinary circuit evaluation; `trace_sound_of_predicate_true` recovers that evaluation from any accepted trace; `traceEquivalence`, `satisfiable_iff_trace_extension`, and `exists_coherent_trace` package both directions. The 71-declaration audit has 18 empty closures, 13 using only `propext`, and 40 using only `propext` and `Quot.sound`, with no project axiom or `Classical.choice`. This milestone does not assemble the complete exposed candidates, prove cross-instance `BaselineDistinct` or final-output laws, construct the uniform polynomial builder, or establish the locked-NAND threshold. The Cook-Levin formula builder and packaged reduction also remain incomplete. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout f1ebb93c5683592eaa70e0b77ed1969a1def6180
npm ci
lake build PNP
