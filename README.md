...
Its 30 reviewed theorem pins have 12 `propext`-only closures and 18 using `Quot.sound` with `propext`. The focused 37-declaration audit has two empty closures, 17 `propext`-only closures, and 18 `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 055325ee54abddefc595ccb826ea410dc8be2231
npm ci
lake build PNP
npm run check
