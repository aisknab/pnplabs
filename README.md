...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **24,934** exported public declarations across **228** modules, including **13,352** theorem-kind declarations and **7,015** assumption-free theorem-kind declarations. It excludes **14,691** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,352 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Eighty-nine scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, exact open-support extraction, governed proper-positive support search, saturated support-square closure, governed completion of every saturated square corner, exact governed frontier gluing, exact commutation of that structure with every forgetful terminal projection, side-tight four-corner minimum arithmetic, checked structural carrier transport, compatibility of independently attained four-corner optima with one reversible ambient carrier and shared observer projection, and a deterministic classifier for coherent optimum transport or its exact first failure.

The newest milestone works for every finite computed terminal support square, explicit observer, terminal projection, and full or quotient coherence mode. Lean checks the meet-left, meet-right, left-join, and right-join legs in a fixed order. It returns either one coherent canonical optimum tuple with exact transport, retained-semantics, side-tight, and incidence facts, or the exact first transport, semantic, profile, charge-profile, or mode mismatch.

This is a bounded mathematical step, not the final theorem. It classifies success or failure but does not prove that every square succeeds, construct the later no-outcome route for a failed leg, prove `sideTightCompletionExists` or BN2 square legitimacy, derive the required dependency data from the circuit, formalize obstruction routing, establish SaturatePositive, complete residual routes, prove `ZeroSlack`, prove PCCMin exactness or polynomial runtime, discharge an assumption, place CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 34713d47d7e00298aeb532dc9d5c69e57d11f296
npm ci
lake build PNP
