...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,006** exported public declarations across **64** modules, including **3,213** theorem-kind declarations and **2,620** assumption-free theorem-kind declarations. It excludes **1,333** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 340 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-two scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects; it preserves distinct out-of-range, valid-padding, and populated states and proves exact prefix, full, one-step-short, terminal, and excess-fuel traversal. A fixed 19-rule tally machine first proved the local input-length stage. The literal `BuilderInputPrefix.machine` composes the all-input framer, an explicit launch transition, and that tally machine with collision-free state namespaces. `BuilderInputPrefix.workRunExact` reaches the represented final tape after exactly `totalInputFramerWorkSteps(input) + 1 + 2*n*n + 4*n + 2` work steps, while its compiled raw execution has the explicit external bound `18*n*n + 63*n + 93`. The new standalone 59-rule `BuilderTokenAppender.machine` appends any one fixed token exactly for every represented input, unary tally, prior output, and exterior tape. Its first-header specialization emits the first two direct formula bits within the compiled raw bound `24*n + 48`; malformed phase symbols and one-step-short fuel time out. Its seventeen reviewed theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. The token appender is not yet rule-table-composed with the input prefix and is not a complete raw formula builder, `FunctionProgram.RawRefinement`, or packaged `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 359dcb1bf645ef15dddbfb42cd1ec789e4f5a5b7
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current sixteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, the literal input-length tally machine, the executable framer-launch-tally prefix, and a standalone exact token appender whose first-header specialization emits the first two formula bits within `24*n + 48` raw steps. They explicitly withhold prefix-to-appender rule-table composition, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
