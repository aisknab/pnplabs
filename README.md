...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **12,233** exported public declarations across **104** modules, including **7,146** theorem-kind declarations and **3,669** assumption-free theorem-kind declarations. It excludes **4,738** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1,963 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Sixty-two scoped milestones are earned. In addition to the concrete CNF-SAT verifier, raw-machine compilers, bounded Cook-Levin formula-building prefix, and arbitrary-circuit carrier/trace equivalence, `PNP.DirectWire.LockedNANDGlobalCandidates` now assembles two exact circuit families for every finite topologically ordered NAND circuit. The source-derived baseline has `B` gates and `B` outputs; the extension has `B + 4` gates and `B + 1` outputs. The reviewed theorems prove the size equations, preserved source and initial-conjunction semantics, exact final-conjunction semantics, absence of internal constants, and independence of every baseline output from the fresh final lock. The 59-declaration audit of the eleven pinned theorem types closes only over `propext` and `Quot.sound`, with no project axiom or `Classical.choice`. This milestone does not prove cross-instance `BaselineDistinct`, either conditional final-output branch law, the locked-NAND threshold, residual slack at most four, or a uniform polynomial bitstring builder. The Cook-Levin formula builder and packaged reduction also remain incomplete. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout a8916280a02c3d2357f5b81917baa17926e51047
npm ci
lake build PNP
