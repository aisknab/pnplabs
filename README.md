...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **24,675** exported public declarations across **226** modules, including **13,260** theorem-kind declarations and **6,984** assumption-free theorem-kind declarations. It excludes **14,607** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,316 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Eighty-seven scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, exact open-support extraction, governed proper-positive support search, saturated support-square closure, governed completion of every saturated square corner, exact governed frontier gluing, exact commutation of that structure with every forgetful terminal projection, side-tight four-corner minimum arithmetic, and checked transport of the computed four-corner structure into one common carrier.

The newest carrier milestone works for every finite computed saturated terminal support square. Lean places the meet, left, right, and join endpoints into one common coordinate system, proves that the boundary, interface, and profile lists have no duplicates, transports the meet and join profiles exactly, and classifies every present side physical coordinate as retained or constructively internalized through fail-closed queries.

This is a bounded mathematical step, not the final theorem. The carrier contains the computed structural endpoints and profiles, not four optimum realizers. It does not construct one coherent basis that realizes all four minima, prove coherent completion or square legitimacy, derive the required dependency data from the circuit, formalize obstruction routing, establish SaturatePositive, complete residual routes, prove `ZeroSlack`, prove PCCMin exactness or polynomial runtime, discharge an assumption, place CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 45e828437ed335a62dbc4e9889e65ee383c53139
npm ci
lake build PNP
