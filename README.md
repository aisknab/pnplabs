...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **23,601** exported public declarations across **210** modules, including **12,818** theorem-kind declarations and **6,776** assumption-free theorem-kind declarations. It excludes **14,273** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,093 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Seventy-two scoped milestones are earned. The earlier typed locked-NAND theorem proves that the exhaustive reference minimum crosses from `B` to at least `B + 1` exactly when the source circuit is satisfiable and leaves residual slack at most four. Its strict version-zero codec and pure fail-closed transformation have exact semantic theorems. A literal 228-state, 2,052-rule source parser accepts exactly valid strict-v0 circuits and clears malformed input, while a fixed 1,387,921-rule grammar-only target emitter has exact all-input output, explicit polynomial runtime and quadratic output bounds, polynomial-time machine/function witnesses, leaf `RawRefinement`, and strict composition with the parser. That composition is packaged as a concrete polynomial reduction from `EncodedNANDSAT` to `EncodedLockedNANDThreshold`. The semantic CNF-to-NAND layer transforms strict canonical CNF formulas into intrinsically topological NAND circuits with exact satisfiability preservation, exact gate count, a quadratic serialized-output bound, and fail-closed malformed input. The fixed all-input compiler milestone realizes that transformation with one 135,070-rule three-node finite work graph. Lean proves exact all-bitstring output, one external polynomial runtime bound, a non-timeout `PolynomialTimeFunction`, literal `RawRefinement`, a direct `PolynomialReduction` from `CNFSAT` to `EncodedNANDSAT`, and composition to `EncodedLockedNANDThreshold`. The newest milestone proves that every finite proof-bearing or executably verified chain of adjacent strict equivalent circuit gains preserves semantics and the exhaustive reference minimum, while its endpoint residual slack plus its length is at most its starting residual slack. The locked-NAND residual bound specializes this to at most four verified gain steps. This bounds a disclosed chain only: it does not find a gain, prove route or candidate-list completeness, justify stopping early, construct `ZeroSlack`, compute an exact minimizer, establish polynomial checker or PCCMin runtime, discharge a project assumption, put CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
- The bundled canonical PDF and TeX are the current sixty-nine-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook–Levin semantic equivalence, the bounded formula-building prefix, global locked-NAND baseline output conditions, both whole-carrier final-output branches, the typed semantic threshold, strict encoding round trips, the pure encoded semantic construction, the exact polynomial-time strict-v0 source parser and target emitter, their packaged concrete `EncodedNANDSAT`-to-`EncodedLockedNANDThreshold` polynomial reduction, the fixed all-input CNF-to-NAND compiler with its direct and composed polynomial reductions, and the universal verified residual-gain-chain bound with its locked-family four-step specialization. They explicitly withhold gain generation and completeness, a stopping theorem, `ZeroSlack`, exact residual minimization and polynomial runtime, the rest of the Cook–Levin formula body, a complete raw formula builder, SAT NP-hardness or CNF-SAT NP-completeness transport, the abstract threshold discharge, CNF-SAT in P, and P = NP.
