...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **25,515** exported public declarations across **232** modules, including **13,564** theorem-kind declarations and **7,043** assumption-free theorem-kind declarations. It excludes **14,779** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,426 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Ninety-three scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal carriers and support construction through computed BN2 square legitimacy, and the computed terminal BCEL anchor nucleus with fail-closed proper-cut classification.

The newest milestone works for every finite direct-wire candidate, explicit terminal dependency system, computed governed proper-positive support, forgetful projection, executable ambient observer, and positive whole-support projection defect. Lean computes the canonical minimum-cardinality positive anchor nucleus, then returns an insufficient nucleus, the exact first anchor-algebra mismatch, the exact first proper-cut defect mismatch, the first proof-bearing full-before-quotient local route, or exact constant-cut and local BN2 conclusions for every proper cut.

This is a bounded mathematical step, not the final theorem. It assumes both the terminal dependency system and a positive whole-support projection defect. It does not derive either premise, identify the manuscript's activation or charge equivalence classes, connect every local failure to the complete global no-outcome route system, establish SaturatePositive, Package E, BCELReady or later BCEL/BN2-BN6 conclusions, formalize complete obstruction routing, prove `ZeroSlack`, prove PCCMin exactness or polynomial runtime, discharge an assumption, place CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout f0bad8053fa334d4d996abd8ee5b796138526ec8
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current seventy-nine-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the bounded formula-building prefix, global locked-NAND baseline output conditions, strict encoding and concrete reductions, the verified residual and terminal-support chain through computed BN2 square legitimacy, and the canonical positive BCEL anchor nucleus with its exact proper-cut mismatch, route, constant-cut, and local-conclusion branches. They explicitly disclose that the terminal dependency system and positive whole-support projection defect remain premises, and withhold the complete global route system, SaturatePositive, Package E, BCELReady, global gain completeness, the manuscript's `ZeroSlack` construction, PCCMin exactness and polynomial runtime, the rest of the Cook-Levin formula body, SAT NP-hardness or CNF-SAT NP-completeness transport, the abstract threshold discharge, CNF-SAT in P, and P = NP.
