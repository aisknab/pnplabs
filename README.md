...
Its 28 reviewed theorem pins have 12 `propext`-only closures and 16 using `Quot.sound` with `propext`. The focused 35-declaration audit has two empty closures, 17 `propext`-only closures, and 16 `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 289ccde2874be3f2f4684470602cad073858fac0
npm ci
lake build PNP
npm run check
