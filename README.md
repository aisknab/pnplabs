...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **8,473** exported public declarations across **76** modules, including **4,267** theorem-kind declarations and **3,013** assumption-free theorem-kind declarations. It excludes **2,779** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 841 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-three scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and the canonical first clause. `BuilderSecondClausePrefix.machine` extends the complete clause-two second-literal prefix with one selected 59-rule `Finish` appender, two total symbol-preserving bridges, and one copy of the fixed 45-rule cursor advance. Every raw bitstring emits `T^FormulaWidth F Sep T F T T F T T T F Finish Sep F F F T F Finish`, the canonical prefix through the complete second clause, and retains `secondClauseStart + 7` at the first in-range padding coordinate. The emitted bits equal `encodedFormula.take (2 * (FormulaWidth + 19))`; direct schedule facts prove the executed token is `Finish` and the retained next opportunity is padding. Its table has `2098` plus six inherited/generated unary-evaluator rule counts, and its compiled run is bounded by `BuilderSecondClauseSecondLiteralPrefix.rawTimeBound + 390 + 24*n + 12*FormulaWidth + 12*cursorWord.length`. All 55 new public declarations plus two reviewed cursor dead-loop facts are axiom-audited: 15 have empty closure, 10 use only `[propext]`, and 32 use only `[Quot.sound, propext]`. This is one fixed clause prefix, not a general dynamic formula cursor. It does not traverse clause-two padding, reach clause three, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 151e25f791def55a7855b20bd805cfb088c4d853
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, the literal input preparation and token-appender machines, the complete first clause, and the complete fixed second clause. They explicitly withhold a general dynamic formula cursor, traversal of clause-two padding, clause three and the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
