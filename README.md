...
Its 12 reviewed theorem pins have ten `propext`-only closures and two using `Quot.sound` with `propext`. The focused 20-declaration audit has one empty closure, 17 `propext`-only closures, and two `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
