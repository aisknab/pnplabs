...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,845** exported public declarations across **71** modules, including **3,788** theorem-kind declarations and **2,886** assumption-free theorem-kind declarations. It excludes **2,380** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 585 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-eight scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and the canonical first clause. `BuilderDynamicTokenCursorStep.machine` composes that first-clause machine, nine total launch rules, and a fixed 45-rule cursor advance. Every raw bitstring preserves `T^FormulaWidth F Sep T F T T F T T T F Finish`, consumes the proved first valid-padding opportunity, and advances the retained unary coordinate from `FormulaVariableSlotBound + 12` to `+ 13`. The suffix costs exactly `2*cursorWord.length + 8` work steps including launch and is bounded by `BuilderFirstClausePrefix.rawTimeBound + 48 + 12*cursorWord.length` compiled steps. All 45 public declarations and all 31 reviewed theorem types use only empty closure, `[propext]`, or `[Quot.sound, propext]`. This is one padding transition, not a general dynamic cursor loop or arbitrary raw slot decoder; it emits no token. The remaining formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` are absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 8a4645c90af934b3a54396aea810cc9d6d944724
npm ci
lake build PNP
