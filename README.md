...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **31,035** exported public declarations across **318** modules, including **15,849** theorem-kind declarations and **7,699** assumption-free theorem-kind declarations. It excludes **15,175** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,139 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 175 of 177 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The newest row gives every finite sparse V53 hypergraph with at least two anchors a shape-specific constant-cut basis: a full-span weight for two anchors, three singleton cuts for three anchors, and full-span support plus weight for four or more. Lean proves this basis equivalent to the complete equation on every nonempty proper cut, and a total classifier uses it without enumerating carrier subsets. Accepted evidence derives constant activation and the existing conditional ZeroSlack bridge; rejection remains one typed structural diagnostic. The terminal problem, positive premise, checked finite BCEL-ready certificate, raw cells and payloads, tables, ranks, route-clear proof, resolvers, normalizer, blocker semantics, and complete encoded-size bounds remain supplied. The result neither forces basis acceptance nor turns rejection into a gain, and it does not establish unconditional ZeroSlack, executable polynomial PCCMin, CNFSAT in P, the eligible root theorem, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-27-199` it reports two separate metrics:

- Formal artefact coverage: **175 of 177** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M199 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint because the raw positive cells, payload and rank construction, normalizer, resolvers, blocker semantics, route-clear proof, and encoded-size polynomial construction remain supplied. M199 removes this endpoint's proper-cut powerset scan, but it does not prove the upstream construction polynomial, force basis acceptance, or turn a rejected basis into a decreasing gain. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Canonical sparse V53 constant-cut basis and checked conditional ZeroSlack bridge**

For every finite sparse V53 hypergraph with at least two anchors, Lean proves that the shape-specific basis above is equivalent to the full constant equation over all nonempty proper cuts. At the checked same-candidate BN6/BCEL/HB boundary, accepted basis evidence derives constant activation and conditional ZeroSlack under checked selector silence.

The terminal problem, positive premise, checked finite BCEL-ready certificate, raw positive cells and payloads, claim table, rank map, route-clear equation, dependency table, checked HB closure, HResolve and BudgetResolve algorithms, normalizer, blocker semantics, and complete encoded-size bounds remain supplied. A rejected basis is a typed structural diagnostic, not a constructed gain or globally decreasing transition. Removing the endpoint scan does not prove that upstream terminal or BCEL construction avoids subset enumeration or that the complete construction is polynomial. This is not complete PkgC/BN3 through BN6 integration, manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, an encoded-size polynomial PCCMin theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 175 of 177 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin and all 22 declarations in the focused audit are free of project-specific axioms. The endpoint uses only Lean-standard `Quot.sound` and `propext`.
...
- The bundled canonical PDF and TeX are the current 141-page inventory-derived formal status report. They report the established concrete, reduction, residual, Packet, HB, PCCPack, PCCMin-oracle, BN6/BCEL, canonical grouping, raw cut-ledger, and sparse V53 constant-cut-basis results. The newest row replaces this endpoint's exhaustive proper-cut classifier with a total shape-specific basis equivalent to every proper-cut equation; basis acceptance derives constant activation and conditional ZeroSlack. Raw cells and terminal data, the upstream constructions, rejection-to-gain routing, complete encoded-size bounds, unconditional ZeroSlack, executable polynomial PCCMin, a target decider, SAT hardness transport, and the root theorem remain open. The report retains the axiom-free logical boundary witness showing that arbitrary per-cut BN2 realizability alone cannot imply a stable family.

  The report explicitly discloses that the BN3 repair and upstream supplied constructions may enumerate subsets; the BN4 ledger, BN5 payload and shadow universe, PkgC consumer antichain, raw positive support-and-payload data, rank and route data, resolver algorithms, blocker tables, and exact-rank mapping remain explicit inputs. M196 derives the BN6 family skeleton from a supplied checked BCEL nucleus, M197 canonically groups supplied raw cells, M198 preserves their exact raw cut ledger, and M199 replaces the endpoint's proper-cut powerset check with an equivalent sparse constant-cut basis. A rejected basis is still only a structural diagnostic, not a constructed decreasing route. Full external selector compatibility, complete terminal-derived construction, blocker semantics, semantic dependency completeness, a decreasing global route system, manuscript-wide SaturatePositive, Package E, BCELReady, unconditional global `ZeroSlack`, PCCMin exactness and polynomial runtime, the rest of the Cook-Levin formula body, SAT NP-hardness or CNF-SAT NP-completeness transport, CNF-SAT in P, and P = NP remain outside the earned scope.
