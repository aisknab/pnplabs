...
Its 9 reviewed theorem pins have seven `propext`-only closures and two using `Quot.sound` with `propext`. The focused 17-declaration audit has one empty closure, 14 `propext`-only closures, and two `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
