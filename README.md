...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 through M210 add formal artefact coverage without closing another fixed checkpoint. M199 through M207 strengthen the sparse BCEL, PkgC and residual-route chain; M208 provides a uniform polynomial raw controller for the complete input-dependent Cook-Levin token schedule, M209 adds the first fixed arbitrary-coordinate router for its top-level header boundary, and M210 adds semantic post-header clause and within-clause decoding with exact remainder extraction. Raw division, body-token emission, raw refinement and the packaged reduction remain open. None constructs every remaining terminal-derived object, establishes complete global route coverage, or proves the complete Cook-Levin reduction and residual algorithm with encoded-size polynomial runtime. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Uniform Cook-Levin arbitrary-slot post-header decoder**

For every concrete polynomial-time verifier and every coordinate below its complete direct token schedule, Lean now refines the uniform outer route with a structurally recursive decoder for every post-header coordinate. Every body result carries typed finite clause and within-clause coordinates that reconstruct the exact shifted remainder; the unique finish and out-of-range cases are characterized; and the interpreted route agrees with direct token lookup. A checked reader recovers that exact remainder from every M209 raw comparison result while preserving the M209 exact raw trace.

M210 adds an unbounded semantic clause and within-clause decoder and extracts the exact shifted remainder from M209's checked raw result. It does not implement raw division or raw body-token emission, complete the raw formula builder or its FunctionProgram.RawRefinement, package the concrete Cook-Levin PolynomialReduction, establish CNFSAT NP-hardness or NP-completeness transport or CNFSAT in P, close a global gate, create the eligible root theorem, or prove P = NP. No fixed weighted checkpoint changes, so the risk-weighted estimate remains 35% while formal artefact coverage becomes 186 of 188. All five global gates remain open.

Its 1 reviewed theorem pin and all 22 declarations in the focused audit are free of project-specific axioms and `Classical.choice`. The endpoint uses only Lean-standard `Quot.sound` and `propext`.
