...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **25,059** exported public declarations across **230** modules, including **13,401** theorem-kind declarations and **7,025** assumption-free theorem-kind declarations. It excludes **14,705** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,385 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Ninety-one scoped milestones are earned. They cover the concrete machine model, Cook-Levin semantics and the bounded formula-builder prefix, the typed locked-NAND threshold, strict codecs, fixed parsers and emitters, concrete polynomial reductions, verified residual-gain chains, the global semantic stopping specification, terminal full and quotient carriers, exhaustive projection minima, the signed four-corner transfer identity, finite terminal saturation, physical support completion, exact open-support extraction, governed proper-positive support search, saturated support-square closure, governed completion of every saturated square corner, exact governed frontier gluing, exact commutation of that structure with every forgetful terminal projection, side-tight four-corner minimum arithmetic, checked structural carrier transport, compatibility of independently attained four-corner optima with one reversible ambient carrier and shared observer projection, deterministic classification of coherent optimum transport or its exact first failure, conditional side-tight coherent completion, and the complete finite tight-basis maximum under exact local route silence.

The newest milestone works for every finite computed terminal support square, explicit observer, and full or quotient mode. Lean retains every profile-constrained minimum implementation at each corner, tries the full finite Cartesian product, filters it with the arbitrary-family coherence query, and proves under exact local route silence that the signed maximum equals the selected delta.

This is a bounded mathematical step, not the final theorem. It proves a local all-finite maximum under computed route silence, not universal route silence. It does not connect every local route to the complete global no-outcome route system, prove BN2 square legitimacy, derive the required dependency data from the circuit, formalize complete obstruction routing, establish SaturatePositive, complete residual routes, prove `ZeroSlack`, prove PCCMin exactness or polynomial runtime, discharge an assumption, place CNF-SAT in P, or prove `P = NP`. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout fc47845928f2cafb4f7ebbafed38e5e7a8a6c25a
npm ci
lake build PNP
