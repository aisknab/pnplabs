...
Its 4 reviewed theorem pins all use only `Quot.sound` with `propext`. The focused six-declaration audit has two `propext`-only closures and four `Quot.sound` plus `propext` closures, with no empty closure, project axiom, or `Classical.choice`.
