...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **5,076** exported public declarations across **46** modules, including **2,125** theorem-kind declarations and **2,024** assumption-free theorem-kind declarations. It excludes **956** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 124 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Nine scoped milestones are earned. They include a concrete universal CNF-SAT verifier with exact accept/reject behavior, no timeout at its polynomial fuel bound, and `PNP.Concrete.FinalUniversalDesign.cnfSATInNP : InNP CNFSAT`. They also include collision-free pipeline state namespaces, exact framer-to-simulator and verdict-to-handoff transitions for supplied target traces, the standalone terminal raw-output packer, and a local bridge from represented handoff output into distinct accepting and rejecting packer copies. That suffix preserves raw `outputBits` within compiled bound `18*m^2 + 36*m + 12`; its 44 reviewed declarations have empty axiom closure. The earlier ordinary-input framer/simulator/handoff trace has not been transported into the extended terminal-bridge machine. Target termination, complete raw-machine refinement, and an external encoded-input-size polynomial remain absent. CNF-SAT in P, CNF-SAT NP-completeness, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
- The bundled canonical PDF and TeX are the current ten-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, bounded internal pipeline-stage bridges, and the local handoff-to-terminal suffix, while explicitly withholding transport of the earlier ordinary-input trace into the extended machine, target termination, complete raw refinement, CNF-SAT in P, NP-completeness, and P = NP.
