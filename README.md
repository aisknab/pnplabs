...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **7,500** exported public declarations across **68** modules, including **3,536** theorem-kind declarations and **2,795** assumption-free theorem-kind declarations. It excludes **2,124** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 454 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-five scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects. Fixed finite machines provide input framing, unary tallying, and reusable two-bit token appending. `BuilderCompleteHeader.machine` emits the complete answer-independent width header. The new `BuilderBodyStartPrefix.machine` composes that machine with a structurally generated unary evaluator for the next token slot and the reusable appender. Its literal table has exactly `440 + BuilderUnaryPolynomial.ruleCount(widthPolynomial verifier) + BuilderUnaryPolynomial.ruleCount(nextTokenSlotPolynomial verifier)` rules. Every raw bitstring emits `T^FormulaWidth F Sep`; `bodyStartTokens_eq_canonical_prefix` identifies the tokens with the canonical prefix, and `nextTokenSlot_eq_formulaVariableSlotBound_add_two` retains the next coordinate under an external `NatPolynomial` bound. All 58 public declarations are completely audited; all 40 reviewed theorem types use only empty closure, `[propext]`, or `[Quot.sound, propext]`. The dynamic cursor controller, subsequent formula body, complete raw builder, builder `FunctionProgram.RawRefinement`, and packaged `PolynomialReduction` are absent. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout a23dd28052c4bbeee281ed0ce691ef7350ef2215
npm ci
lake build PNP
