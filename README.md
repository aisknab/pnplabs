...
Its 26 reviewed theorem pins have 14 `propext`-only closures and 12 using `Quot.sound` with `propext`. The focused 36-declaration audit has five empty closures, 19 `propext`-only closures, and 12 `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
