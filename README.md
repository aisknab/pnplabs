...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **8,680** exported public declarations across **78** modules, including **4,425** theorem-kind declarations and **3,043** assumption-free theorem-kind declarations. It excludes **2,892** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 912 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-five scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and both complete fixed clauses. `BuilderThirdClauseSeparatorStep.machine` composes the complete clause-two padding run with a selected 59-rule `Sep` appender, two total nine-symbol bridges, and the fixed 45-rule cursor advance. Its table has `2272` plus eight inherited/generated unary-evaluator rule counts. Every raw bitstring emits the canonical separator beginning clause three, advances the retained unary coordinate by one, proves the emitted bits equal `encodedFormula.take (2 * (FormulaWidth + 20))`, and proves the following direct schedule token is `F`. Its compiled run is bounded by `BuilderSecondClausePaddingRun.rawTimeBound + 330 + 24*n + 12*FormulaWidth + 12*cursorWord.length`. All 48 new public declarations plus eight reviewed suffix interfaces are axiom-audited: 14 have empty closure, 11 use only `[propext]`, and 31 use only `[Quot.sound, propext]`. This is exactly one populated separator transition at the beginning of clause three. It does not emit the following `F`, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 71f0e479cfd23126abfe19bda4763f514cf4d3e5
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-three-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, both complete fixed clauses, exact clause-two padding traversal, and emission of the fixed `Sep` beginning clause three. They explicitly withhold emission of the following `F`, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
