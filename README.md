...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **12,245** exported public declarations across **105** modules, including **7,158** theorem-kind declarations and **3,676** assumption-free theorem-kind declarations. It excludes **4,997** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1,968 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Sixty-three scoped milestones are earned. In addition to the concrete CNF-SAT verifier, raw-machine compilers, bounded Cook-Levin formula-building prefix, arbitrary-circuit carrier/trace equivalence, and exact source-derived candidate assembly, `PNP.DirectWire.LockedNANDGlobalCandidates` now proves global baseline output conditions for every finite topologically ordered NAND circuit. Every exposed baseline output is nonconstant, is not a positive input projection, and is semantically distinct from every other exposed baseline output; the exact exhaustive reference minimum is therefore `B`. The complete 64-declaration module audit has three empty closures, two using only `propext`, and 59 using only `propext` and `Quot.sound`, with no project axiom or `Classical.choice`. This milestone does not prove either whole-carrier final-output branch law, the locked-NAND threshold, residual slack at most four, or a uniform polynomial bitstring builder. The Cook-Levin formula builder and packaged reduction also remain incomplete. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout aed2c360982d1e356b462b9e27d976b23a2305a4
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current sixty-three-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, all four complete fixed clauses, traversal through the remaining padding of the first scheduled constraint, the complete first literal of the second scheduled constraint, its width-selected successor token, the first six width-selected opportunities, the seventh padding-or-unary opportunity, and the global locked-NAND baseline output conditions and exact reference minimum. They explicitly withhold the rest of the following literal, the rest of the second constraint, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, the two locked-NAND final-output laws, the uniform locked-NAND builder, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
