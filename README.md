...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,783** exported public declarations across **312** modules, including **15,730** theorem-kind declarations and **7,690** assumption-free theorem-kind declarations. It excludes **15,157** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,136 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 172 of 174 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, proof-bearing normalization/oracle composition, rank-ordered proof-bearing PCCOracle orchestration, the checked Packet-backed rank-selector construction, the checked Packet/HB conditional ZeroSlack bridge, the BN6 computed-faithfulness bridge, the same-candidate BCEL activation-route classifier, and the BCEL-derived BN6 family skeleton. The newest row constructs the family carrier, duplicate-freedom, cut value, and positivity from the checked BCEL nucleus, making M195's carrier and cut-value mismatch branches impossible by construction. It leaves an exact proper-cut activation mismatch or derives conditional ZeroSlack under checked selector silence. It does not derive the terminal problem, positive premise, checked finite BCEL-ready certificate, grouped cells, payloads, tables, ranks, route-clear proof, resolvers, normalizer, or blocker semantics from arbitrary input; turn the remaining mismatch into a constructed gain; prove manuscript-wide SaturatePositive or BCELReady, unconditional ZeroSlack, or encoded-size polynomial runtime; put CNFSAT in P; create the eligible root theorem; or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-26-196` it reports two separate metrics:

- Formal artefact coverage: **172 of 174** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
cd pnp
git checkout eb631703727b4121447eab01060712343b5e22f1
npm ci
