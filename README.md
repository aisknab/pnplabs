...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **6,901** exported public declarations across **63** modules, including **3,143** theorem-kind declarations and **2,585** assumption-free theorem-kind declarations. It excludes **1,228** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 323 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Twenty-one scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects; it preserves distinct out-of-range, valid-padding, and populated states and proves exact prefix, full, one-step-short, terminal, and excess-fuel traversal. A fixed 19-rule tally machine first proved the local input-length stage. The new literal `BuilderInputPrefix.machine` composes the all-input framer, an explicit launch transition, and that tally machine with collision-free state namespaces. `BuilderInputPrefix.workRunExact` reaches the represented final tape after exactly `totalInputFramerWorkSteps(input) + 1 + 2*n*n + 4*n + 2` work steps, while its compiled raw execution has the explicit external bound `18*n*n + 63*n + 93`. Malformed tally-scan symbols and one-step-short fuel time out. Its fourteen reviewed theorem types have empty closure, `[propext]`, or `[Quot.sound, propext]`, with no project or choice axiom. This prefix emits no formula bits, does not interpret direct cursor slots as raw transitions, and is not a complete raw formula builder, `FunctionProgram.RawRefinement`, or packaged `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 5617c95037b14dde93c5d21caaf09de873491210
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current sixteen-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, the literal input-length tally machine, and the executable framer-launch-tally prefix with exact work and explicit raw polynomials. They explicitly withhold formula-bit emission, direct cursor interpretation by raw transitions, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
