...
Its 8 reviewed theorem pins have five `propext`-only closures and three using `Quot.sound` with `propext`. The focused nine-declaration audit has one empty closure, five `propext`-only closures, and three `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
