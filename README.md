...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **30,144** exported public declarations across **306** modules, including **15,562** theorem-kind declarations and **7,657** assumption-free theorem-kind declarations. It excludes **15,145** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,130 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 166 of 168 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, typed PCCPack reflection, the proof-bearing PCCMin total-oracle loop, and proof-bearing normalization/oracle composition. The newest row proves that a supplied non-increasing semantic normalizer composes with a supplied total oracle: later gains lift to strict gains from the original implementation, and exact-minimum or ZeroSlack endpoints transport back through normalization into the checked loop. It does not construct either stage, derive terminal routes, prove unconditional ZeroSlack or encoded-size polynomial runtime, put CNFSAT in P, create the eligible root theorem, or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-25-190` it reports two separate metrics:

- Formal artefact coverage: **166 of 168** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. M189 and M190 add formal artefact coverage but do not close the fixed `pccmin-executable-loop` checkpoint because the normalizer and oracle remain supplied. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Proof-bearing PCCMin normalization and oracle composition**

For every finite direct-wire implementation, explicit proof-bearing total normalizer, and explicit proof-bearing total oracle, Lean now composes the manuscript's normalization and oracle stages. Non-increasing semantic normalization lifts any later oracle gain to a strict semantics-preserving gain from the original implementation, while exact-minimum and ZeroSlack outcomes transport back through normalization into the already checked terminating loop.

The total normalizer and total oracle remain explicit proof-bearing arguments, and the regression fixtures use exhaustive reference minimisation. This is not a construction of either executable stage, a terminal-family or route derivation, unconditional ZeroSlack, a raw-machine implementation, an encoded-size polynomial construction or runtime theorem, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 166 of 168 formal artefact rows separate from the unchanged 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin and all ten declarations in the focused audit are axiom-free.
