...
Its 22 reviewed theorem pins have 18 `propext`-only closures and four using `Quot.sound` with `propext`. The focused 31-declaration audit has one empty closure, 26 `propext`-only closures, and four `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout dcba2fad66dcbdaaad5c3ebb939b0130e9a95bf8
npm ci
lake build PNP
npm run check
