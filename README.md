...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **6,800** exported public declarations across **61** modules, including **3,077** theorem-kind declarations and **2,543** assumption-free theorem-kind declarations. It excludes **1,128** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 299 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Nineteen scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact length `encodedFormulaSizePolynomial.eval input.size`, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. The new direct formula cursor decodes any constraint, clause, token, or bit coordinate without first constructing the complete canonical objects; it preserves distinct out-of-range, valid-padding, and populated states and proves exact prefix, full, one-step-short, terminal, and excess-fuel traversal. Its 13 reviewed theorem types use `[Quot.sound, propext]`, with no project or choice axiom. It remains a Lean specification cursor: no constant-time raw interpretation, raw finite formula builder, construction-runtime polynomial, `FunctionProgram.RawRefinement`, or packaged `PolynomialReduction` is proved. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout e4131888da7a1d204fb18b0b4988be022682ab95
npm ci
lake build PNP
