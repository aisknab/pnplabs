...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **29,422** exported public declarations across **286** modules, including **15,237** theorem-kind declarations and **7,537** assumption-free theorem-kind declarations. It excludes **15,058** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 3,003 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 145 of 147 scoped milestone rows are earned. They cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, exact earliest-route semantics, successive computation of every local Packet classifier field, checked budget/HB and semantic/HN bindings, executable selector silence, the local descent/no-lower row and composed Packet ledger, and the finite supplied HResolve route classifier and coverage sidecar. The newest result gives every candidate in an arbitrary supplied finite HResolve family one fixed-priority exact, gain, blocked, or unresolved route, proves the generated ledger sound and complete for that enumeration, and checks a duplicate-free `NoHereditary` sidecar that excludes exact and gain for every listed candidate. It does not construct the candidate universe or predicate semantics from terminal data; prove HN grammar, BWL exactness, H-disjointness, exact-minimum, strict-gain, or blocker dependency semantics; discharge full HResolve; complete the no-lower ledger; implement BudgetResolve, normalization, other named obstructions, saturation, or replay; derive unconditional HB or ZeroSlack; or prove polynomial PCCMin, SAT in P, or `P = NP`.
...
Its 10 reviewed theorem pins comprise four `propext`-only theorems and six using `Quot.sound` with `propext`. The focused ten-declaration audit has the same four/six closure distribution, with no empty closure, project axiom, or `Classical.choice`.
