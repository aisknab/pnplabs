...
cd pnp
git checkout 7445d2afa7af75375d20751b4a0aa7b87e8b8dfc
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current eighty-two-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the bounded formula-building prefix, global locked-NAND baseline output conditions, strict encoding and concrete reductions, the verified residual and terminal-support chain through computed BN2 square legitimacy, the canonical positive BCEL anchor nucleus, the zero-or-positive whole-support projection-defect classifier, candidate-derived terminal saturation cost balance, finite interface-exposure routing, origin/kernel/obligation closure routing, the finite terminal positive-saturation composition, and the fixed residual RankWF. They also pin an axiom-free logical boundary witness showing that per-cut BN2 realizability alone does not construct the cross-cut-stable BN3 realizing family. They explicitly disclose that the observer and projection remain inputs, the proof-bearing problem and positive initial full-slack premise remain explicit, no current route has been mapped into the complete global outcome system or proved to decrease the rank, and the result is not manuscript-wide SaturatePositive, Package E, BCELReady, global gain completeness, the manuscript's `ZeroSlack` construction, PCCMin exactness and polynomial runtime, the rest of the Cook-Levin formula body, SAT NP-hardness or CNF-SAT NP-completeness transport, the abstract threshold discharge, CNF-SAT in P, or P = NP.
