...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **21,020** exported public declarations across **186** modules, including **11,477** theorem-kind declarations and **5,987** assumption-free theorem-kind declarations. It excludes **11,970** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 2,053 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Seventy scoped milestones are earned. The earlier typed locked-NAND theorem proves that the exhaustive reference minimum crosses from `B` to at least `B + 1` exactly when the source circuit is satisfiable and leaves residual slack at most four. Its strict version-zero codec and pure fail-closed transformation have exact semantic theorems. A literal 228-state, 2,052-rule source parser accepts exactly valid strict-v0 circuits and clears malformed input, while a fixed 1,387,921-rule grammar-only target emitter has exact all-input output, explicit polynomial runtime and quadratic output bounds, polynomial-time machine/function witnesses, leaf `RawRefinement`, and strict composition with the parser. That composition is packaged as a concrete polynomial reduction from `EncodedNANDSAT` to `EncodedLockedNANDThreshold`. The newest milestone adds a general answer-independent compiler from strict canonical CNF formulas to intrinsically topological NAND circuits. Lean proves exact satisfiability preservation, exact gate count, a quadratic serialized-output bound, fail-closed behavior on malformed bitstrings, and semantic composition with the locked-NAND threshold builder. The new compiler is not yet a compiled finite work machine, `PolynomialTimeFunction`, or `PolynomialReduction`, so it does not establish CNF-SAT NP-hardness transport. The abstract threshold discharge, complete Cook–Levin builder, CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 95773a6583ca3d41f7b0c82090f000d9c6eb72da
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current sixty-seven-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook–Levin semantic equivalence, the bounded formula-building prefix, global locked-NAND baseline output conditions, both whole-carrier final-output branches, the typed semantic threshold, strict encoding round trips, the pure encoded semantic construction, the exact polynomial-time strict-v0 source parser and target emitter, their packaged concrete `EncodedNANDSAT`-to-`EncodedLockedNANDThreshold` polynomial reduction, and the general CNF-to-NAND semantic compiler with exact gate and output-size bounds. They explicitly withhold the rest of the Cook–Levin formula body, a complete raw formula builder, a finite-machine polynomial reduction and NP-hardness transport for the CNF compiler, the abstract threshold discharge, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
