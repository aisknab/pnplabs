...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [The canonical compiled inventory](public/pnp-theorem-inventory.json) records the checked declarations, modules, theorem kinds, exact reviewed kernel-type pins and standard-axiom closures. Its size is not proof completion. PNPLabs mirrors this already verified core evidence rather than compiling Lean again.
...
- The bundled canonical PDF and TeX are exact imports of the verified core report, including the complete earned milestone ledger. The search now derives its candidate supports directly from the circuit and finds a strict saving whenever any proper support with zero or one incoming wire admits a smaller equivalent complete local replacement. A support is a selected set of gates; proper means some original gates remain outside it. An accepted result builds the actual replacement and preserves every ordinary output and tracked computational wire field. The exact scope and limitations are linked from the current progress section above. A matching report hash establishes file identity, not theorem correctness. The report retains the axiom-free logical boundary witness showing that arbitrary per-cut BN2 realizability alone cannot imply a stable family.
