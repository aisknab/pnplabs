...
Its 23 reviewed theorem pins have 12 `propext`-only closures and 11 using `Quot.sound` with `propext`. The focused 30-declaration audit has two empty closures, 17 `propext`-only closures, and 11 `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 52b77de8a79e41527b4fbab788d433ec031da459
npm ci
lake build PNP
npm run check
