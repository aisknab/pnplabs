...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **8,599** exported public declarations across **77** modules, including **4,359** theorem-kind declarations and **3,031** assumption-free theorem-kind declarations. It excludes **2,833** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 878 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-four scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and both complete fixed clauses. `BuilderSecondClausePaddingRun.machine` composes the complete second-clause prefix with unary evaluators for `D = (FormulaVariableSlotBound - 1) * (FormulaVariableSlotBound + 6) + 5 = FormulaTokensPerClause - 7`, the fixed 25-rule padding countdown, and three total symbol-preserving bridges. Every raw bitstring traverses all remaining clause-two padding coordinates without emitting a token, reaches `FormulaVariableSlotBound + 1 + 2 * FormulaTokensPerClause`, and proves the retained direct schedule value is `Sep`. The emitted bits remain `encodedFormula.take (2 * (FormulaWidth + 19))`. Its table has `2150` plus eight inherited/generated unary-evaluator rule counts and its compiled run has an explicit external input-size polynomial bound. All 65 new public declarations plus three reviewed reused-countdown declarations are axiom-audited: 26 have empty closure, 9 use only `[propext]`, and 33 use only `[Quot.sound, propext]`. This is exactly the remaining clause-two padding run and a retained clause-three coordinate. It does not emit the retained `Sep`, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 5fe3ffec94ad37b0f62321bde9764518d3a4e548
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-two-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, both complete fixed clauses, and exact traversal of the remaining clause-two padding to the retained clause-three `Sep` coordinate. They explicitly withhold emission of that separator, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
