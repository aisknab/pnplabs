...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **9,758** exported public declarations across **86** modules, including **5,253** theorem-kind declarations and **3,229** assumption-free theorem-kind declarations. It excludes **3,571** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1334 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Forty-three scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and three complete fixed clauses. `BuilderFourthClausePrefix.machine` composes the complete fourth-clause second-literal prefix with a selected 59-rule `Finish` appender, the existing 45-rule cursor advance, and two total nine-symbol bridges. Its selected suffix has 113 rules, and its literal table has `4276` plus ten inherited/generated unary-evaluator rule counts. Every raw bitstring emits the `Finish` that completes clause four, preserves `encodedFormula.take (2 * (FormulaWidth + 36))`, and advances the retained coordinate to `FormulaVariableSlotBound + 1 + 3 * FormulaTokensPerClause + 9`, whose direct next token is padding. Its compiled run is bounded by `BuilderFourthClauseSecondLiteralPrefix.rawTimeBound + 618 + 24*n + 12*FormulaWidth + 12*BuilderFourthClauseSeparatorStep.cursorWord.length`. The audit covers all 55 new public declarations plus two cursor dead-state facts using only the approved Lean-standard closure, with no project axiom or `Classical.choice`. Both unlaunched endpoints, malformed appender tally/output, malformed cursor scratch, and one-step-short fuel time out. This emits exactly the fixed `Finish` terminator that completes clause four. It does not traverse clause-four padding, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout c9f4b9b684b18b5fed4a4256133bcfdb83f3ad75
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current forty-two-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, all three complete fixed clauses, exact clause-two and clause-three padding traversal, the fourth-clause separator, both negative literals in clause four, and the `Finish` that completes clause four. They explicitly withhold traversal of clause-four padding, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
