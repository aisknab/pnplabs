...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **31,356** exported public declarations across **322** modules, including **15,993** theorem-kind declarations and **7,705** assumption-free theorem-kind declarations. It excludes **15,185** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,142 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 178 of 180 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The newest row composes M201's source-derived PkgC/BN6 ledger directly with M200's checked sparse BCEL/Packet classifier. It retains the first exact source-member cancellation, reflects a nonempty proper singleton/pair activation mismatch back to the original supplied PkgC source ledger, or reaches conditional ZeroSlack under supplied checked selector silence. The terminal problem, checked readiness, source systems and cuts, payloads, the typed restorer, downstream tables, HB evidence, route-clear result, selector silence, upstream BN3 through BN5 construction, and complete encoded-size bounds remain supplied or open. Neither diagnostic is a verified global gain or rank-decreasing transition, and the result does not establish unconditional ZeroSlack, executable polynomial PCCMin, CNFSAT in P, the eligible root theorem, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-28-202` it reports two separate metrics:

- Formal artefact coverage: **178 of 180** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
