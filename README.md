...
Its 7 reviewed theorem pins have four `propext`-only closures and three using `Quot.sound` with `propext`. The focused nine-declaration audit has no empty closures, six `propext`-only closures, and three `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 72e2509cb18080700a764dde8309060a7d9bb21c
npm ci
lake build PNP
npm run check
