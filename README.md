...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **31,465** exported public declarations across **323** modules, including **16,023** theorem-kind declarations and **7,706** assumption-free theorem-kind declarations. It excludes **15,221** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,143 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 179 of 181 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The newest row computes multiplicity-preserving, order-independent extraction of generated PkgC cancellation cells from an arbitrary supplied ambient BN4 ledger. It returns the exact remainder, multiset embedding, and complete residual-ledger reduction, or proves that no exact embedding exists, then composes with M202's conditional source-routing branches. The ambient ledger, terminal data, checked readiness, source systems and cuts, payloads, restorer, downstream tables, HB evidence, route-clear result, selector silence, upstream construction, and complete encoded-size bounds remain supplied or open. Neither a surviving remainder nor no-embedding result is a verified global gain or rank-decreasing transition, and the result does not establish unconditional ZeroSlack, executable polynomial PCCMin, CNFSAT in P, the eligible root theorem, or `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-28-203` it reports two separate metrics:

- Formal artefact coverage: **179 of 181** current scoped publication rows earned (**98.9%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The terminal problem, checked finite BCEL-ready certificate, active source systems and cuts, positive payload atoms, typed restorer, ambient ledger, realizer table, accepted claims, rank assignment, dependency table, checked HB closure, route-clear result, and selector silence remain supplied. The computed reduction does not prove that its remainder is empty, and a no-embedding result is a compatibility failure rather than a verified gain or globally rank-decreasing transition. This milestone does not construct the ambient ledger or source data from every terminal input, construct upstream BN3 through BN5 data, prove complete PkgC through BN6 route integration, derive blocker semantics or semantic dependency completeness, prove manuscript-wide SaturatePositive or BCELReady, establish unconditional ZeroSlack, construct executable polynomial PCCMin, prove complete encoded-size bounds, put CNFSAT in P, close a global gate, create the eligible root theorem, or prove P = NP. The fixed-weight estimate remains 35%, while formal artefact coverage rises to 179 of 181. No fixed checkpoint or global gate changes.

Its 1 reviewed theorem pin and all 10 declarations in the focused audit are free of project-specific axioms. The endpoint uses only Lean-standard `Quot.sound` and `propext`.

These are finite kernels over explicit caller-supplied inputs. They do not derive the ambient BN4 ledger, terminal problem, checked readiness certificate, active source systems and cuts, payloads, typed restorer, downstream tables, HB evidence, route-clear result, selector silence, or upstream BN3 through BN5 objects from every valid terminal input. The computed extraction does not prove its remainder empty or route-producing, and its no-embedding branch is not a verified gain. The current route does not derive blocker semantics or semantic dependency completeness, build the manuscript's complete encoded selector and realizer universe, construct a decreasing complete global outcome system, prove manuscript-wide SaturatePositive or BCELReady, establish unconditional global `ZeroSlack` or PCCMin, prove complete polynomial construction, runtime, output-size, or certificate-size bounds, establish SAT hardness or CNFSAT in P, create the eligible root theorem, or prove `P = NP`. Two global milestones remain unearned: unconditional global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
