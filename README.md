...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **29,988** exported public declarations across **304** modules, including **15,507** theorem-kind declarations and **7,611** assumption-free theorem-kind declarations. It excludes **15,141** private compiler auxiliaries and records **no** project-specific axioms.

The inventory-derived publication map binds 3,128 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Exactly 164 of 166 scoped milestone rows are earned. This is **formal artefact coverage**, not proof completion. The rows cover the concrete machine model, Cook-Levin semantics and bounded formula-builder prefix, typed locked-NAND semantics and concrete reductions, verified residual and terminal chains, the finite BN3 through BN6 and PkgC construction, the complete finite Packet selector pipeline, checked realizer and HB dependency layers, no-lower and ZeroSlack sidecars, the checked finite SaturatePositive-to-BCEL-ready composition, the same-candidate carrier identity, the finite activation-coherence obstruction, the concrete report-facing locked-NAND and residual-band compatibility links, and typed PCCPack reflection. The newest row replaces the two remaining opaque package/checker declarations with transparent typed generation and structural checking over an explicit proof-bearing PCCMin loop certificate. It removes the final two project-specific axioms and the caller-supplied reflection field, but it does not construct that certificate, implement PCCMin, prove unconditional ZeroSlack or polynomial runtime, establish concrete SAT hardness, put CNFSAT in P, create the eligible root theorem, or prove `P = NP`.

## Progress tracker

[`public/pnp-proof-progress.json`](public/pnp-proof-progress.json) is the exact public mirror of the canonical core `status/PROOF_PROGRESS.json` ledger. At coordinate `PNP-FORMAL-RECONSTRUCTION-STATUS-2026-08-24-188` it reports two separate metrics:

- Formal artefact coverage: **164 of 166** current scoped publication rows earned (**98.8%** of the current evidence ledger). The denominator can grow, and this is not proof completion.
...
The fixed track scores are 13/15 for formal foundations, 15/20 for concrete reductions, 2/35 for the unconditional residual core, 1/20 for exact PCCMin, and 4/10 for the root theorem and axiom elimination. M188 earned exactly the fixed one-point `axiom-remove-generate-pccpack` and `axiom-remove-check-pccpackexp` checkpoints. All five global gates remain open, no project-specific axioms remain, `PNP.Main.p_eq_np` is absent, and the publication gate is false. Scores change only when a named fixed checkpoint changes state and can decrease when dependencies, assumptions, complexity, or blockers worsen.

**Typed PCCPack generation and structural reflection**

This milestone gives the active bridge a typed `PCCPack` record containing the exact supplied `PCCMinLoopCertificate`. Its reviewed completion theorem covers transparent canonical packaging, structural identifier acceptance, mismatch rejection, exact certificate projection, and the removal of the separate caller-supplied checker-reflection field.

The result replaces `PNP.GeneratePCCPack` and `PNP.CheckPCCPackexp` with a transparent typed record, canonical identifier generation, and structural reflection. It proves that generation preserves an explicitly supplied `PCCMinLoopCertificate`, that the canonical package is accepted, and that mismatched identifiers are rejected. The certificate remains an input: this is not a PCCMin construction, an audit of historical JavaScript package bytes or semantic strings, unconditional ZeroSlack, polynomial runtime, CNFSAT in P, SAT hardness transport, `PNP.Main.p_eq_np`, or `P = NP`. The canonical tracker keeps 164 of 166 formal artefact rows separate from the 35% fixed-weight proof-completion estimate. No global gate closes.

Its 1 reviewed theorem pin closes with only `Quot.sound` and `propext`; all 15 declarations in the focused audit are free of project-specific axioms.
