...
The pinned `leanprover/lean4:v4.31.0` toolchain compiles the explicit `PNP` library root. [`public/pnp-theorem-inventory.json`](public/pnp-theorem-inventory.json) is the exact public mirror of the compiled environment inventory: **9,251** exported public declarations across **82** modules, including **4,864** theorem-kind declarations and **3,140** assumption-free theorem-kind declarations. It excludes **3,205** private compiler auxiliaries and records **four** project axioms.

The inventory-derived publication map binds 1126 reviewed theorem candidates to kernel-type fingerprints and binds the complete Lean source/configuration closure. Thirty-nine scoped milestones are earned. In addition to the concrete CNF-SAT verifier and raw-machine compilers, `PNP.Concrete.CookLevin.VerifierTableauProblem.encodedFormula_mem_CNFSAT_iff_language` connects the generated formula exactly to the concrete verifier language, while `encodedFormula_size_le` bounds the actual canonical unary-indexed encoding by an explicit fixed-verifier polynomial in external input length. The answer-independent rectangular schedule has exact polynomial length, and `formulaBitSchedule_emit_eq_encodedFormula` proves that removing empty slots reproduces the canonical encoded formula. Fixed finite machines provide input framing, unary tallying, reusable two-bit token appending, the complete width header, and three complete fixed clauses. `BuilderThirdClausePaddingRun.machine` composes the complete third-clause prefix with two structurally generated unary-polynomial evaluators, the reused 25-rule padding countdown, and three total symbol-preserving bridges. Its global table has `3178` plus ten inherited/generated unary-evaluator rule counts. Every raw bitstring traverses all `FormulaTokensPerClause - 8` remaining third-clause padding coordinates without emitting a token, preserves `encodedFormula.take (2 * (FormulaWidth + 27))`, and reaches `FormulaVariableSlotBound + 1 + 3 * FormulaTokensPerClause`, where direct schedule lookup returns `Sep`. Its compiled run is bounded by `BuilderThirdClausePrefix.rawTimeBound + 18 + 6*countEvaluator.workSteps + 6*(D*(2*countRootPrefixLength + 8) + D*D) + 6*targetEvaluator.workSteps`. All 65 new public declarations plus three reviewed reused-countdown declarations use only the approved Lean-standard closure: 26 have empty closure, 9 use only `[propext]`, and 33 use only `[Quot.sound, propext]`. This executes exactly the remaining clause-three padding block and retains the fourth-clause boundary. It does not emit the fourth-clause `Sep`, implement a general dynamic formula cursor, emit the remaining formula body, complete the raw builder, construct a builder `FunctionProgram.RawRefinement`, or package a `PolynomialReduction`. Therefore CNF-SAT NP-completeness, CNF-SAT in P, and `P = NP` remain unproved. Three global milestones remain unearned: the global locked-NAND construction/threshold; global ZeroSlack, PCCMin and polynomial runtime; and the concrete standard P-versus-NP target/root.
...
cd pnp
git checkout 783941d428aa43cc2fb6763ed5f58fd9dab9ec44
npm ci
lake build PNP
...
- The bundled canonical PDF and TeX are the current thirty-seven-page inventory-derived formal status report. They report `CNFSAT ∈ NP`, raw-machine compilation, exact Cook-Levin semantic equivalence, the actual encoded-formula size polynomial, the exact answer-independent rectangular schedule, direct coordinate decoders with exact fuelled formula traversal, all three complete fixed clauses, and exact clause-two and clause-three padding traversal. They explicitly withhold emission of the fourth-clause separator, a general dynamic formula cursor, the remaining formula body, a complete raw formula builder, `FunctionProgram.RawRefinement`, a packaged polynomial reduction, CNF-SAT NP-completeness, CNF-SAT in P, and P = NP.
