...
Its 28 reviewed theorem pins have 12 `propext`-only closures and 16 using `Quot.sound` with `propext`. The focused 40-declaration audit has two empty closures, 21 `propext`-only closures, one `Quot.sound`-only closure, and 16 `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
...
cd pnp
git checkout 8978e4d55ef11b26276d3726d8eea38caa3397fe
npm ci
lake build PNP
npm run check
