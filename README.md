...
Its 6 reviewed theorem pins have four `propext`-only closures and two using `Quot.sound` with `propext`. The focused eight-declaration audit has one empty closure, five `propext`-only closures, and two `Quot.sound` plus `propext` closures, with no project axiom or `Classical.choice`.
